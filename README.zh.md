...
| <img src="https://api.iconify.design/lucide/text-cursor-input.svg?color=%238b949e" width="16" height="16" alt=""> CustomGreeting | 关 | 替换非 Project 首页问候语。项目外默认不换输入框（`heroOnlyOutsideProject`，默认开）。Project 聊天仍用第一句。问候语轮播：进入首页 / 定时 / 点击标题（`mode`、`order`、`intervalSec`）。可选 `imaginePhrases` 用于 Imagine 输入框。 |
