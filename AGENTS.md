...
Canonical **Beta click-to-install** URL (README badge — jsDelivr `@heads/voidpp`, no `CSP: sandbox`):
...
Do not write `userscript/Void.user.js`. The hop is gone. Old Tampermonkey installs that already ate `[20260911.8]` or `[20260911.9]` follow `@updateURL` to `VoidPP.user.js`. Anyone still on a pre-hop `@updateURL` must reinstall from the canonical file. The namespace change in `[20260925.1]` is a new script identity (`…/voidpp`); there is no in-place update from the old repo-root namespace.

Tampermonkey `@version` must stay plain numeric (`20260925.1`), never `[20260925.1]`. Bracketed versions parse as equal and Check for updates will not fire.
