...
- Overlay and lock CSS only when the editor is wholly empty (`p.is-editor-empty` or `p.is-empty:only-child`). `p.is-empty:first-child` matches a blank first line after Enter (`1` on line two) and paints the phrase as real text on focus. `::before` color is the official muted placeholder, not `--fg-primary`.
