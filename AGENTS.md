...
The transcript is `[data-testid=chat-transcript-scroller]` (`data-transcript-renderer=plane`). Rows are `absolute` + `translateY`. `scrollIntoView` on `#response-*` no-ops. Scroll with `getBoundingClientRect()` (includes the transform) and `pane.scrollTo(scrollTop + delta)`. Do not call `scrollIntoView` on a response root. Only `metadata.parentQuoteSource.sourceResponseId` is a hard source. `parentResponseId` is the previous turn — `[20260926.24]` child `6a0b9629` had an empty `sourceResponseId` and `parentResponseId` `e7ca918c`, a mounted thought card whose body does not contain the quote. Do not `land` that parent id when `blobScore` is 0, and do not add `.void-qj-hit` to a guessed `#response-*` when `findRanges` is empty. A mounted `#response-{sourceResponseId}` is not a guess — `[20260926.25]` child `17bea7f3` had hard `8a579136` already in the DOM (line 524px above mid) and `fits` still dropped it, so `preventDefault` left the official jump dead. Do not veto that hard id with `blobScore`. After `await`, re-read `#response-{id}` if the node disconnected. `[20260926.26]` `findRanges` missed the quote because Grok paints it in a `span`, not `p`/`li`, and `resolveNeedle` preferred store `parentQuotedText` over the button text. Official Jump needle is the button's visible text when it is ≥ 8 chars. `[20260926.27]` child needle `请圈 A / B / C。` lived inside an 82-character `p`; `blockFits` rejected it (`length <= line + 12`) and `CSS.highlights` stayed empty, so land painted `.void-qj-hit` on that paragraph. `clipRanges` searches raw text-node values (no `norm` remap) and paints only that sentence with `::highlight(void-qj)`. Do not outline the `p` or `#response-*` when the sentence range is missing. `[20260926.28]` one path: resolve (button text + `sourceResponseId`), mount that id without a `fits` veto, `clipRanges` on raw text, then paint. `[20260926.30]` one `pane.scrollTo` centers the sentence midline between the pane top and the query bar. A delta under 24px does not scroll. A box taller than that band pins its top. `[20260926.31]` a long `behavior:"smooth"` stops mid-way when the virtualizer writes `scrollTop`. If `|centerDelta|` is more than 1.2 viewports, jump with `behavior:"auto"`. After layout, if the pin is gone, instant-hop until `#response-{id}` is connected, then one smooth only when the remainder is at most 1.2 viewports. A hard id never falls through to `pickMessage`. Do not use `scrollIntoView`. Take over only when there is no button or the hard id is unmounted. Do not fall through to `parentResponseId` or `passageId` while a hard id exists. No range means scroll the card top and do not outline. `messageEls` reads `[id^='response-']` first; `user-message` / `assistant-message` are gone. Search the store for a non-citing message (`parentQuotedText` does not cover the needle), then instant-scroll until that row mounts. Do not treat a neighboring cite as the source.
...
- 26.25 mounted `sourceResponseId` — `fits` dropped `#response-8a579136` after `preventDefault`, so the official Jump did not scroll
- 26.26 official Jump outlined the tall source card and stayed put — `findRanges` skipped `span`, store needle was the last cite, land scrolled the host center
- 26.27 button text matched, but an 82-character `p` got `.void-qj-hit` and `CSS.highlights` stayed empty — `blockFits` dropped the sentence and `rangeCovering` never built a text `Range`
- 26.28 stacked vetoes (`fits`, `parentResponseId`, `passageId`, `preventDefault` on a mounted official Jump) — each patch broke another case
