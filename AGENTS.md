...
Snap (`saved` Map) dies only on explicit dismiss or `sendMessage` / `queueMessage` / `sendResponse` that carries a user body. Consume **before** the host send returns, and keep a short `consumedUntil` so an empty `quotedText` cannot restore the snap. Do not require `parentQuotedText` on the payload — Grok may attach the quote from `ChatPageStore` and omit that field. A query-bar Send/Submit press and Enter in the composer (not Shift+Enter) consume the same way. Do not `markConsumed` from `fetch` to `/rest/app-chat/conversations`. Nav draft-save POSTs the same shape (`message` / `text` / `fileAttachments` + `parentQuotedText`). During dest empty, `ownKey()` falls back to `lastKey` and that fetch would delete the outgoing chat's snap.
