...
- 26.21 a quote with no `sourceResponseId` is a non-button card — `user-message` is gone, so the old `sentQuote` fallback never saw it and the click was not `preventDefault`
