...
- 26.20 settings filter is `team-overview === tab.id`, not `team-management === tab.group` — Void++ tabs never joined the list. Official Jump is `useScrollToResponse(sourceResponseId)` and does not mount a virtual row — do not return the click when that id exists
