...
The in-message quote is `button[aria-label="Jump to quoted message"]`, a sibling of `[data-testid=user-message]` inside `#response-{childId}`. It is not a `blockquote` and not the composer chip. `sentQuote` must match that button. The host id is the child message — never the jump target. The source id is `metadata.parentQuoteSource.sourceResponseId`, else `response.parentResponseId` (fiber `props.response`, then `ResponseStore.byId[child]`, then `MessageStore` nodes). Needle is `parentQuotedText`, then the button text. Do not use `ChatPageStore.quotedText` for a sent card. `propsId` only sees a top-level id and the UUID regex is unanchored, so `id="response-{child}"` must be stripped to the bare uuid and still must not be used as the source. An id is not the source unless visible text (not a jump button) contains the needle — `parentResponseId` is often the previous assistant turn, not the passage. Host pick uses the first 48 characters, else the longest line clip, over `clipLen / messageLen`. Score the visible body first, then the thought body. `0.08` only drops a long parent when a tighter mounted host exists — do not drop the only hit. A target after the citing child is not the source. Store text is `content.message`, then `content.query`, then `nodeToResponse`. If an official Jump has no host, do not `preventDefault`. Host pick still uses a 48-character clip. The highlight does not: match each `p` / `li` with whitespace and backticks stripped, and paint the whole block. A line clip that dies on a link or a `code` node must not drop the rest of the quote. Do not paint `td` / `th`. Do not paint a block shorter than 8 characters that is only a substring of a longer quote line. Scroll is not `ranges[0]`: group the painted blocks by a 240px vertical gap and scroll the first line of the longest cluster. If that line box is empty, do not scroll the message root. `/project/` leaves `ResponseStore.byId` empty. Do not outline a host the needle missed.

Reverse (source → quotes) is a `.void-qj-back` badge on the source host, not a click on the message body. Selecting text there must still create a new quote. Pin it to the prose box (union of `p` / `li` / `pre` / headings inside the message), not `#response-*` row `right`. Do not `Math.max(8, top)` — `[20260926.17]` stuck a transparent hit target on the viewport corner and swallowed the official Jump card and the avatar menu. If the prose top is offscreen, hide the badge. Idle state keeps an `--fg-secondary` ring. Build the glyph with `createElementNS`, not `innerHTML`. Capture `preventDefault` only when the event target is the badge or its menu, never when it is `button[aria-label="Jump to quoted message"]`. `/project/` keeps messages in `MessageStore.conversations[cid].nodes`; `ResponseStore.byId` can be empty, so do not scan only that store. A backlink requires non-empty `parentQuotedText`. The source id is `metadata.parentQuoteSource.sourceResponseId`, else `parentResponseId` — never a bare thread `parentId` with an empty quote. Mounted jump buttons and a live composer (`quotePopupData.responseId`, else `quotedText` inside the source body) are the same index. One cite jumps; more than one opens `.void-qj-menu`. Scroll the citing card with `pane.scrollTo`, not `scrollIntoView`. Host the badge on `document.body`. React will delete a child of `#response-*`.
...
- 26.17 reverse badge used the full `#response-*` row and `Math.max(8, top)` — a transparent hit target covered the official Jump card and the avatar menu
