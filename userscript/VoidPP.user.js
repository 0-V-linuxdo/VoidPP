...
/**
 * Void++ [20260914.17] v1.0.0 — A modification for grok.com
...
`);
  ...
  function VersionInfo() {
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      as: "span",
      color: "secondary"
    }, "[20260914.17] v1.0.0"), /* @__PURE__ */ React.createElement(Dot, null), /* @__PURE__ */ React.createElement(VersionLink, {
      href: `${"https://github.com/0-V-linuxdo/VoidPP"}/commit/${"235fcdc"}`
    }, `(${"235fcdc"})`)), /* @__PURE__ */ React.createElement(Flex, {
      alignItems: "center",
      gap: "0.25rem"
    }, /* @__PURE__ */ React.createElement(Text2, {
      as: "span",
...
`);
  ...
  function currentChatIds() {
    ...
    try {
      ...
      logger20.debug("route chat ids unavailable:", e);
    }
    try {
      ...
      logger20.debug("url chat ids unavailable:", e);
    }
    return ids;
  }
  function currentAgentIds() {
    ...
    try {
      ...
      logger20.debug("route agent id unavailable:", e);
    }
    try {
      addId(ids, new URL(location.href).pathname.match(PATH_BOT)?.[1]);
    } catch (e) {
      logger20.debug("url agent id unavailable:", e);
    }
    return ids;
  }
  function currentIds() {
  ...
  function botLiveIds(ids) {
    try {
      ...
      logger20.debug("bots store unavailable:", e);
    }
  }
  function liveIds() {
    const ids = new Set;
  ...
  function errorOf(id) {
    ...
    try {
      ...
      logger20.debug("bot error lookup failed:", e);
    }
    return false;
  }
  ...
  var chatListStatus_default = definePlugin({
    ...
    start() {
      ...
      try {
        ...
        logger20.debug("bots store subscribe failed:", e);
      }
      observe();
      schedule();
    },
...
`);
  ...
  function PinnedModes() {
    ...
    return /* @__PURE__ */ React.createElement("div", {
      ...
      tooltip: m.label,
      onClick: onPin(m.id),
      className: classes(cl19("pin"), selectedModeId === m.id && cl19("on"), showLabels && cl19("labeled")),
      "aria-label": m.label
    })));
  }
...
`);
  ...
  function capVisits(ids) {
    ...
    const dirtyGlobalWs = asWorkspaceId(settings21.plain.workspaceByConv?.[HOME_KEY]);
    const seen = new Set;
    const out = [];
  ...
  function initEffect() {
    ...
    if (disk) {
      ...
      const fromSettings = {
        visits: asStringList(settings21.plain.visits),
        deniedIds: asStringList(settings21.plain.deniedIds),
        deniedAt: asStringRecord(settings21.plain.deniedAt)
      };
      effect = {
        v: 1,
  ...
  function commitVisits(next) {
    ...
    for (const [id, snap] of Object.entries(settings21.plain.projectIcons ?? {})) {
  ...
  function titleOf(id) {
    if (!id || isHomeId(id))
      return "New chat";
    if (isDenied(id))
      return usableTitle(settings21.plain.titles?.[id]) || "Untitled";
...
`) : "";
  ...
  function rememberProjectIcon(ws) {
    ...
    const prev = settings21.plain.projectIcons ?? {};
    if (snap) {
      wsIcons[ws] = snap;
      if (prev[ws] !== snap)
        settings21.store.projectIcons = { ...prev, [ws]: snap };
      return;
    }
    if (prev[ws] && isChromeSnap(prev[ws])) {
      ...
      delete next[ws];
      delete wsIcons[ws];
      settings21.store.projectIcons = next;
    }
  }
  ...
  function reconcileSidebarCache() {
    ...
    const prevWs = { ...settings21.plain.workspaceByConv };
    const prevNames = { ...settings21.plain.projectNames };
    const prevIcons = { ...settings21.plain.projectIcons };
    let wsChanged = false;
    ...
    if (iconsChanged)
      settings21.store.projectIcons = prevIcons;
  }
  ...
  function snapOf(id) {
    if (!id || isHomeId(id))
      return null;
    const snap = thumbs.get(id) ?? parseSnap(settings21.plain.pages?.[id]);
    if (!snap)
      return null;
...
`)) {
  ...
  function projectIconOf(ws) {
    const raw = ws ? wsIcons[ws] || settings21.plain.projectIcons?.[ws] || liveIconSnap(ws) || "" : "";
  ...
  function renderList(items) {
    if (!host)
      return;
    const panel = host.querySelector(`.${cl24("panel")}`);
    if (!panel)
      return;
  ...
  function patchList(items) {
    if (!host)
      return;
    const cards = [...host.querySelectorAll(`.${cl24("card")}`)];
...
`);
  ...
  async function fetchBotUsage() {
    try {
      const hook = BotsStore2.useBotsStore;
