...
/**
 * Void++ [20260920.19] v1.0.0 — A modification for grok.com
...
`);
  ...
  function VersionInfo() {
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      as: "span",
      color: "secondary"
    }, "[20260920.19] v1.0.0"), /* @__PURE__ */ React.createElement(Dot, null), /* @__PURE__ */ React.createElement(VersionLink, {
      href: `${"https://github.com/0-V-linuxdo/VoidPP"}/commit/${"a64e36a"}`
    }, `(${"a64e36a"})`)), /* @__PURE__ */ React.createElement(Flex, {
      alignItems: "center",
      gap: "0.25rem"
    }, /* @__PURE__ */ React.createElement(Text2, {
      as: "span",
...
.void-ct-card {
    ...
    background: hsl(var(--surface-l1));
    box-shadow:
        inset 0 0 0 1px hsl(var(--border-l1)),
        0 0.5rem 1.5rem hsl(0deg 0% 0% / 16%);
...
`);
  ...
  var logger19 = new Logger("CompleteToast");
  ...
  function currentIds() {
    ...
    try {
      ...
      logger19.debug("page ids unavailable:", e);
    }
    try {
      ...
      logger19.debug("route ids unavailable:", e);
    }
    try {
      ...
      logger19.debug("url ids unavailable:", e);
    }
    return ids;
  }
  ...
  function gatewayOf(cid) {
    try {
      ...
      logger19.debug("MessageStore unavailable:", e);
      return;
    }
  }
  function isLiveCid(cid) {
    ...
    try {
      ...
      logger19.debug("ResponseStore live lookup failed:", e);
    }
    return false;
  }
  ...
  function cidOf(responseId) {
    ...
    try {
      ...
      logger19.debug("gateway cid lookup failed:", e);
    }
    try {
      const { byConversationId } = ResponseStore.useResponseStore.getState();
      ...
      logger19.debug("response cid lookup failed:", e);
    }
    return "";
  }
  function convOf(cid) {
    try {
      ...
      logger19.debug("conversation lookup failed:", e);
      return;
    }
  }
  function workspaceOf(cid) {
    try {
      ...
      logger19.debug("workspace lookup failed:", e);
      return "";
    }
  }
  ...
  function applyChatPage(id, ws) {
    try {
      ...
      logger19.debug("ChatPageStore update failed:", e);
    }
  }
  function navigateTo(id) {
    ...
    try {
      ...
      logger19.error("Failed to navigate:", e);
      try {
        location.assign(hrefFor(cid, ws));
      } catch (navErr) {
        logger19.error("Fallback navigation failed:", navErr);
      }
    }
  }
  ...
  function armTimer() {
    clearTimer();
    const sec = clamp(settings12.store.duration, 0, DURATION_MAX);
    if (sec <= 0)
      return;
    const ms = sec * 1000;
    hideAt = Date.now() + ms;
  ...
  function liveCids() {
    ...
    try {
      ...
      logger19.debug("gateway live scan failed:", e);
    }
    try {
      const { byConversationId } = ResponseStore.useResponseStore.getState();
      for (const [id, list] of Object.entries(byConversationId ?? {})) {
      ...
      logger19.debug("response live scan failed:", e);
    }
    return ids;
  }
  function seedToasted() {
    try {
      ...
      logger19.debug("seed responses failed:", e);
    }
    try {
      ...
      logger19.debug("seed gateway failed:", e);
    }
  }
  function finishClosed() {
    try {
      ...
      logger19.debug("closed scan failed:", e);
    }
  }
  function syncLive() {
    if (!started2)
...
`);
  ...
  var logger22 = new Logger("BetterImagine");
...
${p.originalPrompt ?? ""}`.toLowerCase();
  ...
  function pauseVideo(video) {
    ...
    if (promise) {
      promise.then(() => {
        ...
        video.pause();
        video.currentTime = 0;
      }).catch((e) => logger22.warn("Failed to pause video:", e));
    } else {
      video.pause();
      video.currentTime = 0;
    }
  ...
  var onMouseEnter = (e) => {
    ...
    if (video)
      pending.set(video, video.play().catch((e2) => logger22.error("Failed to play video", e2)));
  };
  var onMouseLeave = (e) => {
    const video = e.currentTarget.querySelector("video");
    if (video)
...
`));
      ...
      logger22.error(`Failed to copy ${label}s`, e);
    ...
    for (const id of ids) {
      ...
      for (const video of videos) {
        ...
        try {
          ...
          logger22.error("Failed to upscale video:", id, video.id, e);
        }
      }
    }
    if (upscaled)
...
`);
  ...
  var logger23 = new Logger("BetterFiles");
  ...
  async function deleteAssets(ids) {
    ...
    for (const id of ids) {
      ...
      try {
        ...
        logger23.error("Failed to delete asset", id, e);
      }
...
`);
  ...
  function paint() {
    ...
    if (live2.length && !rowById.size)
      logger25.info("live ids with no rows", live2);
  }
  function schedule() {
    if (!started4 || raf2)
      return;
  ...
  var logger26 = new Logger("AutoRetry");
  ...
  function retry(responseId, conversationId, response) {
    ...
    logger26.info(`Retry ${count}/${max} for ${conversationId} in ${delaySec}s`);
    clearPending();
...
`);
  ...
  var logger27 = new Logger("CompactModeSelect");
  ...
  function uncloak() {
    for (const host2 of ghosts) {
      host2.classList.remove(cl21("ghost"));
  ...
  function cloak(menu) {
    ...
    if (ghosts.has(host2))
      return;
    host2.classList.add(cl21("ghost"));
...
`);
  ...
  var cl22 = classNameFactory("void-ih-");
  ...
  function getEntries() {
    const raw = settings20.plain.entries;
...
`) : el.innerText ?? el.textContent ?? "";
  ...
  function hudEl() {
    let el = document.querySelector(`.${cl22("hud")}`);
    if (el)
      return el;
    el = document.createElement("div");
    el.className = cl22("hud");
    el.setAttribute("aria-live", "polite");
    document.body.appendChild(el);
    return el;
  }
  function hideHud() {
    document.querySelector(`.${cl22("hud")}`)?.classList.remove(cl22("hud-on"));
  }
  ...
  function HistoryPanel() {
    const { entries } = settings20.use(["entries"]);
    const list = entries ?? [];
...
`);
  ...
  var logger29 = new Logger("DownloadTTS");
  ...
  function DownloadButton() {
    const [loading, onClick2] = useAsyncAction(async () => {
      try {
        ...
        logger29.error("Failed to download TTS audio:", e);
      }
    });
    return /* @__PURE__ */ React.createElement(Button, {
...
`);
  ...
  function syncClasses() {
    ...
    for (const [key, cls] of Object.entries(CSS_CLASSES)) {
      classList.toggle(cls, !!settings21.store[key]);
    }
  }
  ...
  function syncRestoreFlag() {
    if (!settings22.store.stickyOnNavigate) {
  ...
  function fightHydrate() {
    if (!settings22.store.stickyOnNavigate || applying2 || userPicking || awaitingMenu || !intent.modeId)
      return;
    const cur = snapshot();
  ...
  function onNavigate() {
    ...
    if (!intent.modeId)
      intent = snapshot();
    if (!settings22.store.stickyOnNavigate || !intent.modeId)
      return;
    setRestoreFlag(true);
    applyIntent(intent);
    syncRestoreFlag();
  ...
  function patchPayload(raw, live2) {
    if (!raw || typeof raw !== "object" || Array.isArray(raw) || !live2.modeId)
      return false;
    const rec = raw;
    if (!isChatSend(rec))
      return false;
  ...
  function holdQueueEvent(cid, event) {
    ...
    if (type === QUEUE_ADD && diverting) {
      ...
      diverting = null;
      logger30.info("held", id, "for", liveIntent().modeId);
      return true;
    }
  ...
  function flushTurn(cid, turn, parentId) {
    ...
    logger30.info("flushed", turn.id, "as", liveIntent().modeId);
  }
  ...
  function patchGwEvent(event, live2) {
    if (!event || typeof event !== "object" || Array.isArray(event) || !live2.modeId)
      return;
    const rec = event;
  ...
  function makeQueueWrapper(orig) {
    return function voidModeSyncQueue(...args) {
      const [first] = args;
      const live2 = liveIntent();
      if (!isTurnArgs(first) || !live2.modeId || inflightMode(first.convId) === live2.modeId)
        return orig.apply(this, args);
      diverting = first;
      try {
        return orig.apply(this, args);
  ...
  function rewriteIfChatPost(url, method, text) {
    ...
    if (!next || next === text)
      return null;
    applyIntent(live2);
    logger30.info("rewrite", live2.modeId, url.replace(/^https?:\/\/[^/]+/, ""));
    return next;
  }
...
`);
  ...
  var cl24 = classNameFactory("void-ci-");
  ...
  function PresetEditor({ preset, onUpdate, onClose }) {
    ...
    return /* @__PURE__ */ React.createElement("div", {
      ...
      className: cl24("textarea-wrap", { "textarea-wrap-error": overLimit })
    }, /* @__PURE__ */ React.createElement(Textarea, {
      ...
      className: cl24("editor-footer")
    }, /* @__PURE__ */ React.createElement(Text2, {
      size: "xs",
      color: overLimit ? undefined : "muted",
      className: overLimit ? cl24("error-text") : undefined
...
`);
  ...
  var logger31 = new Logger("BetterSidebar");
  var cl25 = classNameFactory("void-sidebar-");
  ...
  function applyHeaderHover() {
    if (settings25.store.titleRowHover)
  ...
  function startBotsCollapse() {
    stopBotsCollapse();
    if (!settings25.store.botsDefaultCollapsed)
      return;
    let done = false;
    const tick = () => {
      if (done)
  ...
  function resetChatsCollapsedStorage() {
    if (!settings25.store.chatsDefaultExpanded)
      return;
    try {
  ...
  function resetProjectsCollapsedStorage() {
    if (!settings25.store.projectsDefaultCollapsed)
      return;
    try {
  ...
  var ChatsPlus = ErrorBoundary.wrap(function ChatsPlusButton() {
    if (!settings25.use(["chatsPlus"]).chatsPlus)
      return null;
    return /* @__PURE__ */ React.createElement("button", {
      type: "button",
      className: BTN_CLASS,
  ...
  async function deleteConversations(ids) {
    ...
    await Promise.allSettled(ids.map((id) => fetchSoftDeleteConversation(id).catch((e) => logger31.error("Failed to delete", id, e))));
  }
  function SelectCheckbox({ id, route }) {
    const enabled = settings25.use(["batchSelect"]).batchSelect;
  ...
  var betterSidebar_default = definePlugin({
    ...
    _wrapSidebarClick(onClick2, id, route) {
      return (e) => {
        if (id && settings25.store.batchSelect && isConversationRoute(route) && (e.ctrlKey || e.metaKey)) {
          e.preventDefault();
          e.stopPropagation();
    ...
    _defaultOpen() {
      return !settings25.store.defaultCollapsed;
    },
    _botsDefaultCollapsed() {
      return settings25.store.botsDefaultCollapsed;
    },
    _chatsCollapsedInit() {
      resetChatsCollapsedStorage();
      return false;
    ...
    _projectsAutoExpand() {
      return !settings25.store.projectsDefaultCollapsed;
    },
    _onSidebarClick() {
      if (!settings25.store.clickToToggle)
        return;
      return (e) => {
...
`, "\\A ");
  ...
  function rotateMode() {
    const value = String(settings26.store.mode ?? "refresh");
    if (value === "interval" || value === "manual")
      return value;
    return "refresh";
  }
  function rotateOrder() {
    return settings26.store.order === "random" ? "random" : "sequential";
  }
  function intervalMs() {
    return clamp(Number(settings26.store.intervalSec ?? 10), 1, 3600) * 1000;
  }
  ...
  function pickNextIndex(listLen, advance) {
    if (listLen <= 0)
      return 0;
    const current = Number(settings26.store.greetIndex ?? -1);
    const last = Number(settings26.store.lastRandom ?? -1);
    if (listLen === 1) {
      if (current !== 0)
        settings26.store.greetIndex = 0;
      if (last !== 0)
        settings26.store.lastRandom = 0;
      return 0;
    }
    if (!advance)
    ...
    if (rotateOrder() === "random") {
      ...
      settings26.store.greetIndex = next2;
      settings26.store.lastRandom = next2;
      return next2;
    }
    const prev = current >= -1 && current < listLen ? current : -1;
    const next = (prev + 1) % listLen;
    settings26.store.greetIndex = next;
    return next;
  }
...
`);
  ...
  var logger32 = new Logger("MessageTimestamps");
  var STAMP_MAX = 5000;
  ...
  function stamps() {
    if (cache)
      return cache;
    cache = new Map;
    const raw = settings27.plain.stamps;
  ...
  function conversationIdOf(id, rec) {
    ...
    try {
      ...
      logger32.debug("conversation id lookup failed", e);
    }
    return "";
  }
  ...
  function gatewayRecords(cid) {
    ...
    try {
      ...
      logger32.debug("message store unavailable", e);
    }
    return out;
  }
  ...
  function extraKeys(rec, id) {
    ...
    try {
      ...
      logger32.debug("stable key lookup failed", e);
    }
    return keys3;
  }
  ...
  function storeRecords(id) {
    try {
      ...
      logger32.debug("response store unavailable", e);
      return [];
    }
  }
  function borrowedMs(id, cid) {
    ...
    try {
      ...
      logger32.debug("node neighbor lookup failed", e);
    }
  ...
  function conversationCreateTime(id) {
    try {
      ...
      logger32.debug("conversation time lookup failed", e);
    }
    return null;
  }
  function fullRecord(id, rec) {
    ...
    try {
      ...
      logger32.debug("byId lookup failed", e);
    }
    return rec;
  }
    ...
    pageWindow.fetch = function voidMessageTimestampsFetch(input, init) {
      ...
      return promise.then((res) => {
        try {
          ...
          logger32.debug("fetch ingest failed", e);
        }
        return res;
      });
    };
      ...
      if (RESPONSE_URL.test(url)) {
        this.addEventListener("load", () => {
          try {
            ...
            logger32.debug("xhr ingest failed", e);
          }
        }, { once: true });
      }
  ...
  function hookListResponses() {
    ...
    try {
      ...
      origList = null;
      logger32.debug("chatListResponses wrap skipped", e);
    }
  }
  function unhookListResponses() {
    ...
    try {
      ...
      logger32.debug("chatListResponses unwrap skipped", e);
    }
    origList = null;
  }
  ...
  var messageTimestamps_default = definePlugin({
    ...
    start() {
      try {
        ...
        logger32.warn("Failed to hook network", e);
      }
    },
    stop() {
      unhookFetch2();
      ...
      ResponseStore: {
        ...
        handler() {
          try {
            ...
            logger32.debug("store ingest failed", e);
          }
        }
      },
      MessageStore: {
    ...
    _renderTimestamp: ErrorBoundary.wrap(({ response, isUser }) => {
      ...
      if (settings27.store.hideOwnMessages && human)
        return null;
      ...
      }, formatTimestamp(ms, settings27.store.showDate));
    }),
    patches: [
      {
...
`);
  ...
  var logger33 = new Logger("BetterNavigator");
  var cl27 = classNameFactory("void-bn-");
  ...
  function liveAssistantEl() {
    ...
    if (!last)
      return null;
    if (USER_INTERRUPT5.test(last.textContent ?? ""))
      return null;
    const live2 = storeLive();
    if (live2)
      return last;
    if (live2 == null && (stopVisible() || last.querySelector(THINK_SEL)))
      return last;
    return null;
  }
  function collect() {
    const root = chatPane() ?? document;
    const showAsst = settings28.store.showAssistant;
    ...
    for (const el of root.querySelectorAll(MSG_SEL)) {
      ...
      if (!text)
        continue;
      out.push({ el, role, text, live: live2 });
    }
    return out;
  }
  ...
  function metaLabel(index) {
    ...
    if (!settings28.store.showAssistant || !n)
      return pos;
  ...
  function flash(el) {
    clearFlash();
    if (settings28.store.jumpEffect !== "border")
      return;
    flashing = el;
  ...
  function applyActive(index) {
    ...
    if (!overMenu) {
      const row = host2?.querySelector(`.void-bn-item[data-void-bn-i="${index}"]`);
  ...
  function alignMenu(index) {
    ...
    const selfTick = host2.querySelectorAll(".void-bn-tick")[index];
  ...
  function menuEl(nav) {
    ...
    meta.className = cl27("meta");
    meta.textContent = metaLabel(0);
    const ul = document.createElement("ul");
    ul.className = cl27("list");
    nav.forEach((item, i) => {
      const li = document.createElement("li");
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = cl27("item");
      btn.dataset.voidBnI = String(i);
      const emoji = document.createElement("span");
      emoji.className = cl27("emoji");
      emoji.textContent = item.role === "user" ? "❓" : "\uD83E\uDD16";
      const label = document.createElement("span");
      label.className = cl27("label");
  ...
  function tickRail(nav) {
    ...
    wrap.className = cl27("ticks", { dense: nav.length > DENSE_N });
    nav.forEach((item, i) => {
      const tick2 = document.createElement("button");
      tick2.type = "button";
      tick2.className = cl27("tick", item.role === "user" ? "tick-user" : "tick-asst", { "tick-live": item.live });
  ...
  function syncHideTip() {
    document.documentElement.classList.toggle(HIDE_CLASS, !!settings28.store.hideNativeHover);
  }
  function setOpen(on) {
  ...
  function paint2() {
    ...
    if (nextKey === paintedKey && host2?.isConnected && sameEls(nav)) {
      lastNav = nav;
      patchLive(nav);
      setActive(nav);
      return;
...
`);
  ...
  function maxCount() {
    const n = Number(settings30.store.maxRecent);
    return Number.isFinite(n) && n > 0 ? n : 5;
  }
  function capVisits(ids) {
    const allowHome = settings30.store.includeHome;
    const current = currentVisit();
    const dirtyGlobalWs = asWorkspaceId2(settings30.plain.workspaceByConv?.[HOME_KEY]);
    const seen = new Set;
    const out = [];
  ...
  function assignRecord(key, next) {
    if (sameRecord(settings30.plain[key], next))
      return false;
    settings30.store[key] = next;
    return true;
  }
  ...
  function initEffect() {
    ...
    if (disk) {
      ...
      const fromSettings = {
        visits: asStringList(settings30.plain.visits),
        deniedIds: asStringList(settings30.plain.deniedIds),
        deniedAt: asStringRecord(settings30.plain.deniedAt)
      };
      effect = {
        v: 1,
  ...
  function commitVisits(next) {
    ...
    const rawWs = pruneRecord(settings30.plain.workspaceByConv, visits);
    ...
    const pages = pruneRecord(settings30.plain.pages, visits);
    ...
    for (const [id, name] of Object.entries(settings30.plain.projectNames ?? {})) {
    ...
    for (const [id, snap] of Object.entries(settings30.plain.projectIcons ?? {})) {
    ...
    const titles = {};
    for (const [id, name] of Object.entries(pruneRecord(settings30.plain.titles, visits))) {
  ...
  function rememberTitle(id, title) {
    ...
    if (id === chatIdFromUrl() && isAccessDeniedPage())
      return;
    const prev = settings30.plain.titles ?? {};
    if (prev[id] === t)
      return;
    settings30.store.titles = { ...prev, [id]: t };
  }
  ...
  function titleOf2(id) {
    if (!id || isHomeId(id))
      return "New chat";
    if (isDenied(id))
      return usableTitle(settings30.plain.titles?.[id]) || "Untitled";
    const conv = lookup(id);
    return usableTitle(conv?.title) || usableTitle(settings30.plain.titles?.[id]) || titleFromPage(id) || "Untitled";
  }
...
`) : "";
  ...
  function rememberProjectIcon(ws) {
    ...
    const prev = settings30.plain.projectIcons ?? {};
    if (snap) {
      wsIcons[ws] = snap;
      if (prev[ws] !== snap)
        settings30.store.projectIcons = { ...prev, [ws]: snap };
      return;
    }
    if (prev[ws] && isChromeSnap(prev[ws])) {
      ...
      delete next[ws];
      delete wsIcons[ws];
      settings30.store.projectIcons = next;
    }
  }
  ...
  function dropWorkspace(id) {
    const prev = settings30.plain.workspaceByConv ?? {};
    if (!prev[id])
      return;
    const next = { ...prev };
    delete next[id];
    settings30.store.workspaceByConv = next;
  }
  function workspaceOf2(id) {
    ...
    if (fromSidebar)
      return fromSidebar;
    const cached = asWorkspaceId2(settings30.plain.workspaceByConv?.[id]);
    if (cached)
      return cached;
  ...
  function projectNameOf(id) {
    ...
    if (!ws)
      return "";
    const idx = sidebarIndex();
    const named = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || wsNames[ws] || settings30.plain.projectNames?.[ws] || "");
    if (!named)
      return "";
    const live2 = liveWorkspaceId();
  ...
  function rememberProject(id) {
    ...
    if (!ws)
      return;
    const prevWs = settings30.plain.workspaceByConv ?? {};
    if (prevWs[id] !== ws)
      settings30.store.workspaceByConv = { ...prevWs, [id]: ws };
    const idx = sidebarIndex();
    const sidebarName = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || "");
    const liveName = ws === liveWorkspaceId() ? readOpenProjectName() : "";
    const cached = usableName(wsNames[ws] || settings30.plain.projectNames?.[ws] || "");
    ...
    if (!name)
      return;
    wsNames[ws] = name;
    const prevNames = settings30.plain.projectNames ?? {};
    if (prevNames[ws] !== name)
      settings30.store.projectNames = { ...prevNames, [ws]: name };
  }
  function reconcileSidebarCache() {
    const idx = sidebarIndex();
    const prevWs = { ...settings30.plain.workspaceByConv };
    const prevNames = { ...settings30.plain.projectNames };
    const prevIcons = { ...settings30.plain.projectIcons };
    let wsChanged = false;
    ...
    if (wsChanged)
      settings30.store.workspaceByConv = prevWs;
    if (namesChanged)
      settings30.store.projectNames = prevNames;
    if (iconsChanged)
      settings30.store.projectIcons = prevIcons;
  }
  function requestWorkspace(id) {
    ...
    try {
      ...
      fetchConv(id).then((conv) => {
        ...
        const prev = settings30.plain.workspaceByConv ?? {};
        if (prev[id] !== ws)
          settings30.store.workspaceByConv = { ...prev, [id]: ws };
        const live2 = liveWorkspaceId();
        const liveName = usableName(readOpenProjectName());
        const names = settings30.plain.projectNames ?? {};
        if (live2 && ws !== live2 && liveName && names[ws] === liveName) {
          const next = { ...names };
          delete next[ws];
          settings30.store.projectNames = next;
          delete wsNames[ws];
        }
        maybePaint();
  ...
  function snapOf(id) {
    if (!id || isHomeId(id))
      return null;
    const snap = thumbs.get(id) ?? parseSnap(settings30.plain.pages?.[id]);
    if (!snap)
      return null;
  ...
  function rememberPage(id, snap) {
    const json = JSON.stringify(snap);
    const prev = settings30.plain.pages ?? {};
    if (prev[id] === json)
      return;
    settings30.store.pages = { ...prev, [id]: json };
  }
  ...
  function captureId(id) {
    ...
    if (!lines.length)
      return;
    const prev = thumbs.get(id) ?? parseSnap(settings30.plain.pages?.[id]);
  ...
  function bump(id) {
    if (!id)
      return;
    if (isHomeId(id) && !settings30.store.includeHome)
      return;
  ...
  function folderIcon() {
    const svg2 = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg2.setAttribute("class", cl28("folder"));
    svg2.setAttribute("viewBox", "0 0 24 24");
    svg2.setAttribute("fill", "none");
    svg2.setAttribute("stroke", "currentColor");
    svg2.setAttribute("stroke-width", "2");
...
`)) {
  ...
  function projectIconOf(ws) {
    const raw = ws ? wsIcons[ws] || settings30.plain.projectIcons?.[ws] || liveIconSnap(ws) || "" : "";
  ...
  function apply6() {
    const pct = clamp(settings31.store.opacity, 0, 100);
    const blur = clamp(settings31.store.blur, 0, 40);
    const alpha = pct / 100;
  ...
  completeToast_default.updatedAt = 0;
...
`);
  ...
  // src/VoidPP.ts
  var logger35 = new Logger("TurbopackPatcher", "#e78284");
  ...
  function waitForModulesStable() {
    const fire = onlyOnce(() => {
      ...
      logger35.info(`${getModuleCache().size} modules loaded, ready`);
