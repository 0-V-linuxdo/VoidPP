...
Neon rain in a quiet city`
  ...
  var logger31 = new Logger("QuoteSticky");
  var KEEP = 40;
  var saved = new Map;
  var lastKey = "";
  var applying3 = false;
  ...
  function restore(key) {
    ...
    try {
      ...
      if (snap?.text) {
        ...
        logger31.info("restored", key);
        return;
      }
      if (chat.quotedText || chat.quotePopupData)
        applyQuote(undefined, undefined);
    } catch (e) {
...
`)) {
  ...
  var logger33 = new Logger("ResponseNotification");
  ...
  function getCtx() {
    ...
    try {
      ...
      logger33.debug("AudioContext unavailable:", e);
      audioCtx = null;
      return null;
    }
  }
  ...
  function playUrl(ctx, url) {
    loadBuffer(ctx, url).then((buf) => playBuffer(ctx, buf), (err) => {
      logger33.info("sample play failed:", err);
      if (url !== DEFAULT_CHIME)
        loadBuffer(ctx, DEFAULT_CHIME).then((buf) => playBuffer(ctx, buf), (e) => logger33.info("default chime failed:", e));
    });
  }
  function playSound() {
    if (!userGestured) {
      logger33.info("sound skipped, no user gesture yet");
      return;
    }
    const ctx = getCtx();
    if (!ctx)
    ...
    if (ctx.state === "suspended")
      ctx.resume().then(() => playUrl(ctx, url), () => logger33.info("AudioContext resume failed"));
    else
      playUrl(ctx, url);
  }
  ...
  function notify(responseId, state) {
    logger33.info("notify", responseId, state ?? "unset", "permission", Notification.permission);
  ...
  function onStreamEnd7({ responseId }) {
    logger33.info("streamEnd", responseId);
    if (retryTimer2)
    ...
    const attempt = (retried) => {
      ...
      try {
        ...
        logger33.info("ResponseStore unavailable:", e);
      }
      ...
      if (isErrorResponse3(response)) {
        logger33.info("skip error", responseId);
        return;
      }
      if (!retried) {
      ...
      logger33.info("skip", responseId, response?.state ?? "unset");
    };
    attempt(false);
  }
...
`));
  ...
  quoteSticky_default.updatedAt = 0;
...
`);
  ...
  // src/VoidPP.ts
  var logger36 = new Logger("TurbopackPatcher", "#e78284");
  ...
  function waitForModulesStable() {
    const fire = onlyOnce(() => {
      ...
      logger36.info(`${getModuleCache().size} modules loaded, ready`);
