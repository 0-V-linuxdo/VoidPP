...
`);
  ...
  function nodeLive(node) {
    if (LIVE_NODE.has(node.status))
      return true;
    if (node.content?.partial && !isDeadResponse(node.content))
      return true;
  ...
  function itemFromNode(cid, node, liveId, liveEl) {
    ...
    if (!settings6.store.showAssistant && role === "assistant")
      return null;
  ...
  function tickRail(nav) {
    ...
    wrap.className = cl17("ticks", { dense: nav.length > DENSE_N });
    nav.forEach((item, i) => {
      const tick = document.createElement("button");
      tick.type = "button";
      tick.className = cl17("tick", item.role === "user" ? "tick-user" : "tick-asst", { "tick-live": item.live });
...
`);
  ...
  function isLiveCid(cid) {
    ...
    if (node && LIVE_NODE2.has(node.status))
      return true;
    if (isLiveResponse(node?.content))
      return true;
    try {
  ...
  function isBadFinish(cid, rid) {
    try {
      ...
      if (isUserInterrupt2(response) || isErrorResponse(response))
        return true;
    } catch {}
    const node = lastAssistantNode(gatewayOf2(cid));
    if (node && (DEAD_ERR.has(node.status) || isUserInterrupt2(node.content) || isErrorResponse(node.content)))
      return true;
    if (node && USER_INTERRUPT2.test(String(node.content?.message ?? "")))
      return true;
    return false;
  }
  ...
  function previewOf(cid, rid) {
    if (!settings8.store.showPreview)
      return "";
    try {
  ...
  function shouldPersist() {
    return !!settings8.store.keepUntilDismissed || clamp(settings8.store.duration, 0, DURATION_MAX) <= 0;
  }
  function armTimer() {
    clearTimer();
    if (shouldPersist())
      return;
    const ms = clamp(settings8.store.duration, 0, DURATION_MAX) * 1000;
    hideAt = Date.now() + ms;
    hideTimer = setTimeout(hide, ms);
  }
  function migratePersist() {
    if (settings8.store.duration !== 0 || settings8.store.keepUntilDismissed)
      return;
  ...
  function show(cid, rid, kind = "chat", previewText = "") {
    ...
    card.className = cl18("card");
    const main = document.createElement("button");
    main.type = "button";
    main.className = cl18("main");
    const icon = document.createElement("span");
    icon.className = cl18("icon");
    ...
    try {
      ...
      for (const id of Object.keys(byConversationId ?? {})) {
        ...
        if (!last?.responseId || toasted.has(last.responseId))
          continue;
        if (isLiveResponse(last) || isLiveCid(id))
          continue;
...
`));
  ...
  function FilterButtons() {
    ...
    return /* @__PURE__ */ React.createElement(Fragment, null, /* @__PURE__ */ React.createElement(Select, {
      ...
      className: sortActive ? cl20("sort-select", "sort-active") : cl20("sort-select")
      ...
      size: "sm",
      shape: "pill",
      className: currentFilter !== f ? cl20("chip") : undefined,
...
`);
  ...
  function isUserInterrupt3(response) {
    ...
    if (state === "interrupted" || state === "stopped" || state === "aborted" || state === "cancelled" || state === "canceled")
      return true;
    return USER_INTERRUPT3.test(interruptBlob(response.error)) || USER_INTERRUPT3.test(String(response.message ?? ""));
  }
  function shouldRetry(response) {
    if (isUserInterrupt3(response))
      return false;
  ...
  function apply3() {
    ...
    if (!settings14.store.italic)
      rules.push(`${SEL}{font-style:inherit!important}`);
    if (!settings14.store.quotes) {
...
`);
  ...
  var logger23 = new Logger("QuoteJump");
  ...
  function storeNeedle(needle) {
    ...
    try {
      ...
      logger23.debug("store search failed", e);
    }
    return null;
  }
  ...
  async function hydrate(cid) {
    ...
    try {
      ...
      logger23.debug("loadResponses failed", e);
    }
    try {
      await ResponseStore.useResponseStore.getState().loadMoreResponses?.(cid);
    } catch (e) {
      logger23.debug("loadMoreResponses failed", e);
    }
  }
  ...
  async function jump2(origin) {
    ...
    if (mine !== gen)
      return;
    if (!el) {
      logger23.debug("no source message");
      return;
    }
  ...
  var logger24 = new Logger("QuoteSticky");
  ...
  function dismiss() {
    ...
    try {
      ...
      logger24.debug("dismiss failed", e);
    } finally {
      applying = false;
    }
    removeFallback();
  ...
  function onChat() {
    if (!armed || applying || onImaginePage2())
      return;
    const now = readText();
    const dest = destKey();
    const key = snapKey();
  ...
  var logger25 = new Logger("BetterQuotes");
  ...
  function migrateLegacy2() {
    ...
    SettingsStore3.markAsChanged();
    logger25.info("Migrated QuoteJump / QuoteSticky into BetterQuotes");
  }
  var pluginName = Object.getOwnPropertyDescriptor(settings15, "pluginName");
    ...
    Object.defineProperty(settings15, "pluginName", {
      ...
      set(name) {
        if (name === "BetterQuotes")
          migrateLegacy2();
        pluginName.set.call(settings15, name);
      }
    });
  }
  function apply4() {
    if (settings15.store.jumpToPassage)
      startJump();
    else
      stopJump();
    if (settings15.store.persistAcrossChats)
      startSticky();
    else
      stopSticky();
  }
  ...
  function paintMenu() {
    ...
    for (const menu of document.querySelectorAll(MENU)) {
      ...
      if (!settings18.store.applyToMenu) {
        dropNames(menu);
        unhide(menu);
        for (const img of menu.querySelectorAll(`img[${MARK}]`))
          restoreImg(img);
        continue;
      }
  ...
  function onMut(muts) {
    ...
    for (const m of muts) {
      ...
      if (!(el instanceof HTMLImageElement))
        continue;
      if (el.closest(FOOTER2) || settings18.store.applyToMenu && el.closest(MENU)) {
        schedule();
        return;
      }
    }
...
`);
  ...
  function maxCount() {
    const n = Number(settings19.store.maxRecent);
    return Number.isFinite(n) && n > 0 ? n : 5;
  }
  function capVisits(ids) {
    const allowHome = settings19.store.includeHome;
    const current = currentVisit();
    const dirtyGlobalWs = asWorkspaceId2(settings19.plain.workspaceByConv?.[HOME_KEY]);
    const seen = new Set;
    const out = [];
  ...
  function assignRecord(key, next) {
    if (sameRecord(settings19.plain[key], next))
      return false;
    settings19.store[key] = next;
    return true;
  }
  ...
  function initEffect() {
    ...
    if (disk) {
      ...
      const fromSettings = {
        visits: asStringList(settings19.plain.visits),
        deniedIds: asStringList(settings19.plain.deniedIds),
        deniedAt: asStringRecord(settings19.plain.deniedAt)
      };
      effect = {
        v: 1,
  ...
  function commitVisits(next) {
    ...
    const rawWs = pruneRecord(settings19.plain.workspaceByConv, visits);
    ...
    const pages = pruneRecord(settings19.plain.pages, visits);
    ...
    for (const [id, name] of Object.entries(settings19.plain.projectNames ?? {})) {
    ...
    for (const [id, snap] of Object.entries(settings19.plain.projectIcons ?? {})) {
    ...
    const titles = {};
    for (const [id, name] of Object.entries(pruneRecord(settings19.plain.titles, visits))) {
  ...
  function rememberTitle(id, title) {
    ...
    if (id === chatIdFromUrl() && isAccessDeniedPage())
      return;
    const prev = settings19.plain.titles ?? {};
    if (prev[id] === t)
      return;
    settings19.store.titles = { ...prev, [id]: t };
  }
  ...
  function titleOf2(id) {
    if (!id || isHomeId(id))
      return "New chat";
    if (isDenied(id))
      return usableTitle(settings19.plain.titles?.[id]) || "Untitled";
    const conv = lookup(id);
    return usableTitle(conv?.title) || usableTitle(settings19.plain.titles?.[id]) || titleFromPage(id) || "Untitled";
  }
...
`) : "";
  ...
  function rememberProjectIcon(ws) {
    ...
    const prev = settings19.plain.projectIcons ?? {};
    if (snap) {
      wsIcons[ws] = snap;
      if (prev[ws] !== snap)
        settings19.store.projectIcons = { ...prev, [ws]: snap };
      return;
    }
    if (prev[ws] && isChromeSnap(prev[ws])) {
      ...
      delete next[ws];
      delete wsIcons[ws];
      settings19.store.projectIcons = next;
    }
  }
  ...
  function dropWorkspace(id) {
    const prev = settings19.plain.workspaceByConv ?? {};
    if (!prev[id])
      return;
    const next = { ...prev };
    delete next[id];
    settings19.store.workspaceByConv = next;
  }
  function workspaceOf2(id) {
    ...
    if (fromSidebar)
      return fromSidebar;
    const cached = asWorkspaceId2(settings19.plain.workspaceByConv?.[id]);
    if (cached)
      return cached;
  ...
  function projectNameOf(id) {
    ...
    if (!ws)
      return "";
    const idx = sidebarIndex();
    const named = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || wsNames[ws] || settings19.plain.projectNames?.[ws] || "");
    if (!named)
      return "";
    const live = liveWorkspaceId();
  ...
  function rememberProject(id) {
    ...
    if (!ws)
      return;
    const prevWs = settings19.plain.workspaceByConv ?? {};
    if (prevWs[id] !== ws)
      settings19.store.workspaceByConv = { ...prevWs, [id]: ws };
    const idx = sidebarIndex();
    const sidebarName = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || "");
    const liveName = ws === liveWorkspaceId() ? readOpenProjectName() : "";
    const cached = usableName(wsNames[ws] || settings19.plain.projectNames?.[ws] || "");
    ...
    if (!name)
      return;
    wsNames[ws] = name;
    const prevNames = settings19.plain.projectNames ?? {};
    if (prevNames[ws] !== name)
      settings19.store.projectNames = { ...prevNames, [ws]: name };
  }
  function reconcileSidebarCache() {
    const idx = sidebarIndex();
    const prevWs = { ...settings19.plain.workspaceByConv };
    const prevNames = { ...settings19.plain.projectNames };
    const prevIcons = { ...settings19.plain.projectIcons };
    let wsChanged = false;
    ...
    if (wsChanged)
      settings19.store.workspaceByConv = prevWs;
    if (namesChanged)
      settings19.store.projectNames = prevNames;
    if (iconsChanged)
      settings19.store.projectIcons = prevIcons;
  }
  function requestWorkspace(id) {
    ...
    try {
      ...
      fetchConv(id).then((conv) => {
        ...
        const prev = settings19.plain.workspaceByConv ?? {};
        if (prev[id] !== ws)
          settings19.store.workspaceByConv = { ...prev, [id]: ws };
        const live = liveWorkspaceId();
        const liveName = usableName(readOpenProjectName());
        const names = settings19.plain.projectNames ?? {};
        if (live && ws !== live && liveName && names[ws] === liveName) {
          const next = { ...names };
          delete next[ws];
          settings19.store.projectNames = next;
          delete wsNames[ws];
        }
        maybePaint();
  ...
  function snapOf(id) {
    if (!id || isHomeId(id))
      return null;
    const snap = thumbs.get(id) ?? parseSnap(settings19.plain.pages?.[id]);
    if (!snap)
      return null;
  ...
  function rememberPage(id, snap) {
    const json = JSON.stringify(snap);
    const prev = settings19.plain.pages ?? {};
    if (prev[id] === json)
      return;
    settings19.store.pages = { ...prev, [id]: json };
  }
  ...
  function captureId(id) {
    ...
    if (!lines.length)
      return;
    const prev = thumbs.get(id) ?? parseSnap(settings19.plain.pages?.[id]);
  ...
  function bump(id) {
    if (!id)
      return;
    if (isHomeId(id) && !settings19.store.includeHome)
      return;
...
`)) {
  ...
  function projectIconOf(ws) {
    const raw = ws ? wsIcons[ws] || settings19.plain.projectIcons?.[ws] || liveIconSnap(ws) || "" : "";
...
`);
  ...
  var logger30 = new Logger("CustomGreeting");
...
`, "\\A ");
  ...
  function migrateLegacy3() {
    ...
    SettingsStore3.markAsChanged();
    logger30.info("Migrated Placeholder into CustomGreeting");
  }
  var pluginName2 = Object.getOwnPropertyDescriptor(settings20, "pluginName");
  if (pluginName2?.set && pluginName2.get) {
    Object.defineProperty(settings20, "pluginName", {
      ...
      set(name) {
        if (name === NEW_NAME)
          migrateLegacy3();
        pluginName2.set.call(settings20, name);
      }
    });
  }
  ...
  function PhraseListsEditor() {
    ...
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      onChange: (e) => {
        settings20.store.imaginePhrases = e.target.value;
      },
...
Neon rain in a quiet city`
  ...
  function replaceChatInput() {
    if (isProjectChat())
      return true;
    return settings20.store.heroOnlyOutsideProject === false;
  }
  ...
  function imaginePhrases() {
    try {
      const lines = parsePhrases(settings20.store.imaginePhrases);
      return lines.length ? lines : null;
    } catch {
      return null;
    }
  ...
  function rotateMode() {
    const value = String(settings20.store.mode ?? "refresh");
    if (value === "interval" || value === "manual")
      return value;
    return "refresh";
  }
  function rotateOrder() {
    return settings20.store.order === "random" ? "random" : "sequential";
  }
  function intervalMs() {
    return clamp(Number(settings20.store.intervalSec ?? 10), 1, 3600) * 1000;
  }
  ...
  function pickNextIndex(listLen, advance) {
    if (listLen <= 0)
      return 0;
    const current = Number(settings20.store.greetIndex ?? -1);
    const last = Number(settings20.store.lastRandom ?? -1);
    if (listLen === 1) {
      if (current !== 0)
        settings20.store.greetIndex = 0;
      if (last !== 0)
        settings20.store.lastRandom = 0;
      return 0;
    }
    if (!advance)
    ...
    if (rotateOrder() === "random") {
      ...
      settings20.store.greetIndex = next;
      settings20.store.lastRandom = next;
      return next;
    }
  ...
  var logger31 = new Logger("BetterAvatarPlugins");
  ...
  function migrateLegacy4() {
    ...
    SettingsStore3.markAsChanged();
    logger31.info("Migrated NoSidebarPlugins into BetterAvatarPlugins");
  }
  var pluginName3 = Object.getOwnPropertyDescriptor(settings21, "pluginName");
  if (pluginName3?.set && pluginName3.get) {
    Object.defineProperty(settings21, "pluginName", {
      configurable: true,
      enumerable: true,
      get: pluginName3.get,
      set(name) {
        if (name === NEW_NAME2)
          migrateLegacy4();
        pluginName3.set.call(settings21, name);
      }
    });
  }
...
`);
  ...
  function getPresets() {
    return settings23.plain.presets ?? [];
  }
  function setPresets(presets) {
    settings23.store.presets = presets;
  }
  function getAssignments() {
    return settings23.plain.assignments ?? {};
  }
  ...
  function PresetsEditor() {
    const presets = settings23.use(["presets"]).presets ?? [];
    ...
    const deletePreset = useCallback((id) => {
      ...
      settings23.store.assignments = a;
  ...
  function InstructionsMenu({ conversationId }) {
    const presets = settings23.use(["presets"]).presets ?? [];
    const assignments = settings23.use(["assignments"]).assignments ?? {};
  ...
  var logger32 = new Logger("ResponseNotification");
  ...
  function getCtx() {
    ...
    try {
      ...
      logger32.debug("AudioContext unavailable:", e);
      audioCtx = null;
      return null;
    }
  }
  ...
  function playUrl(ctx, url) {
    loadBuffer(ctx, url).then((buf) => playBuffer(ctx, buf), (err) => {
      logger32.info("sample play failed:", err);
      if (url !== DEFAULT_CHIME)
        loadBuffer(ctx, DEFAULT_CHIME).then((buf) => playBuffer(ctx, buf), (e) => logger32.info("default chime failed:", e));
    });
  }
  function playSound() {
    if (!userGestured) {
      logger32.info("sound skipped, no user gesture yet");
      return;
    }
    const ctx = getCtx();
    if (!ctx)
    ...
    if (ctx.state === "suspended")
      ctx.resume().then(() => playUrl(ctx, url), () => logger32.info("AudioContext resume failed"));
    else
      playUrl(ctx, url);
  }
  ...
  function isLiveResponse2(response) {
  ...
  function notify(responseId, state) {
    logger32.info("notify", responseId, state ?? "unset", "permission", Notification.permission);
  ...
  function onResponses(current, prev) {
    ...
    for (const id of Object.keys(cur)) {
      if (isLiveResponse2(old[id]) && shouldNotify(cur[id]))
        notifyOnce(id, cur[id]?.state);
    }
  }
  function onStreamEnd4({ responseId }) {
    logger32.info("streamEnd", responseId);
    if (retryTimer2)
    ...
    const attempt = (retried) => {
      ...
      try {
        ...
        logger32.info("ResponseStore unavailable:", e);
      }
      ...
      if (isErrorResponse2(response)) {
        logger32.info("skip error", responseId);
        return;
      }
      if (!retried) {
      ...
      logger32.info("skip", responseId, response?.state ?? "unset");
    };
    attempt(false);
  }
  ...
  function syncImagine2(current, prev) {
    if (!settings24.store.imagineGeneration || !prev)
      return;
    if (onImaginePage3())
      return;
    const now = new Set(current ? current.split(",") : []);
  ...
  function apply6() {
    const pct = clamp(settings25.store.opacity, 0, 100);
    const blur = clamp(settings25.store.blur, 0, 40);
    const alpha = pct / 100;
...
`);
  ...
  var logger33 = new Logger("ExportChat");
...
`);
  ...
  function ExportMenu({ conversationId }) {
    ...
    return /* @__PURE__ */ React.createElement(MenuSub, null, /* @__PURE__ */ React.createElement(MenuSubTrigger, {
      ...
      key: fmt,
      onSelect: () => exportChat(conversationId, fmt).catch((e) => logger33.error("Failed to export chat", e))
    }, label))));
  }
...
`);
  ...
  function storeGet(key) {
    ...
    try {
      ...
      logger34.debug("Failed to read usage stats", error);
      return memory.get(key) ?? null;
    }
  }
  function storeSet(key, value) {
    ...
    try {
      ...
      logger34.debug("Failed to persist usage stats", error);
      memory.set(key, value);
    }
  }
  function storeRemove(key) {
    ...
    try {
      ...
      logger34.debug("Failed to clear usage stats", error);
      memory.delete(key);
    }
  }
  ...
  async function fetchBotUsage() {
    try {
      ...
      logger35.warn("Failed to fetch Grok Bot usage", error);
      return null;
    }
  }
    ...
    refreshPromise = (async () => {
      try {
        ...
        const remote = await fetchOfficialUsage().then((usage) => ({ ok: true, usage })).catch((error) => {
          logger35.warn("Failed to fetch official usage", error);
          return { ok: false };
        });
        if (currentPoolId() !== poolId)
          return false;
  ...
  var usageDisplay_default = definePlugin({
    ...
    start() {
      ...
      try {
        ...
        logger35.warn("RoutingStore subscribe failed", error);
      }
    },
    stop() {
      unsubRoute?.();
...
`);
  ...
  var logger36 = new Logger("QueuePersist");
  ...
  function withIntent(intent, fn) {
    ...
    try {
      ...
      logger36.debug("intent apply failed", e);
    }
    try {
      if (intent?.modeId)
      ...
      try {
        ...
        logger36.debug("intent restore failed", e);
      }
    }
  }
  function qid(item) {
  ...
  async function persist3() {
    ...
    try {
      ...
      logger36.debug("persist failed", e);
    }
  }
  async function load() {
    try {
  ...
  async function replay(cid, saved) {
    ...
    for (const item of ordered) {
      ...
      try {
        ...
        logger36.debug("replay failed", e);
      } finally {
        suppress = false;
      }
  ...
  async function restore2(cid) {
    ...
    try {
      ...
      logger36.debug("restore failed", e);
      retry = true;
    } finally {
      replaying = false;
  ...
  var logger37 = new Logger("ModeSync");
  ...
  function syncRestoreFlag() {
    if (!settings28.store.stickyOnNavigate) {
  ...
  function rememberMode(modeId) {
    ...
    awaitingMenu = false;
    applyIntent(intent);
    logger37.info("intent", intent.modeId);
  }
  ...
  function fightHydrate() {
    if (sendOverride || !settings28.store.stickyOnNavigate || applying3 || userPicking || awaitingMenu || !intent.modeId)
      return;
    if (!loadPending())
      return;
    const cur = snapshot();
    ...
    if (modeSlug(cur.modeId) === slug && (!cid || sessionAdjusted(cid) === slug) && (!intent.modelMode || modeSlug(cur.modelMode) === slug) && (!intent.activeModelId || cur.activeModelId === intent.activeModelId))
      return;
    logger37.info("hydrate fought", cur.modeId, "->", intent.modeId);
    applyIntent(intent);
  }
  function navKey() {
    try {
  ...
  function onNavigate() {
    ...
    closeMenu();
    schedulePaint();
    if (!settings28.store.stickyOnNavigate || !intent.modeId)
      return;
    setRestoreFlag(true);
    applyIntent(intent);
    syncRestoreFlag();
  ...
  function patchPayload(raw, live) {
    if (onImaginePage4() || !raw || typeof raw !== "object" || Array.isArray(raw) || !live.modeId)
      return false;
    const rec = raw;
    if (!isChatSend(rec))
      return false;
  ...
  function pruneIntents() {
    ...
    const live = new Set;
    for (const list of held2.values())
      for (const h of list)
        live.add(h.id);
  ...
  function holdQueueEvent(cid, event) {
    ...
    if (type === QUEUE_ADD) {
      ...
      schedulePaint();
      if (diverting) {
        mapGetOrCreate(held2, cid, () => []).push({ id, args: diverting, intent: { ...saved } });
        diverting = null;
        logger37.info("held", id, "for", saved.modeId);
        return true;
      }
      return false;
    }
    if (QUEUE_SILENT.has(String(type)))
      return held2.get(cid)?.some((h) => h.id === id) ?? false;
  ...
  function flushTurn(cid) {
    ...
    forgetItem(turn.id);
    logger37.info("flushed", turn.id, "as", item.modeId, "session", ackedModel.get(cid) ?? "?", busy.has(cid) ? "busy" : "idle");
  }
  ...
  function patchGwEvent(event, live) {
    if (onImaginePage4() || !event || typeof event !== "object" || Array.isArray(event) || !live.modeId)
      return;
    const rec = event;
  ...
  function intentForText(cid, text) {
    ...
    if (!body || !cid)
      return;
    for (const turn of held2.get(cid) ?? []) {
  ...
  function wrapGatewaySend() {
    try {
      ...
      logger37.debug("gateway wrap failed", e);
    }
  }
  function unwrapGatewaySend() {
    ...
    try {
      ...
      logger37.debug("gateway unwrap failed", e);
    }
    origGwSend = null;
    wrappedGwSend = null;
    gwHost = null;
  ...
  function rewriteIfChatPost(url, method, text) {
    ...
    if (!next || next === text)
      return null;
    applyIntent(live);
    logger37.info("rewrite", live.modeId, url.replace(/^https?:\/\/[^/]+/, ""));
    return next;
  }
  ...
  function hookFetch2() {
    ...
    pageWindow.fetch = function voidModeSyncFetch(input, init) {
      try {
        ...
        logger37.debug("fetch patch failed", e);
      }
  ...
  function setItemMode(id, modeId) {
    ...
    logger37.info("queue item", id, "->", next.modeId);
    schedulePaint();
  }
  ...
  function paint3() {
    paintRaf = 0;
    if (!settings28.store.showQueueMode || onImaginePage4()) {
      unpaint();
      return;
    }
  ...
  function startMode() {
    ...
    try {
      ...
      logger37.warn("Failed to hook send path", e);
    }
    if (intent.modeId)
      applyIntent(intent);
  }
  ...
  var logger38 = new Logger("BetterQueue");
  ...
  function migrateLegacy5() {
    ...
    SettingsStore3.markAsChanged();
    logger38.info("Migrated ModeSync / QueuePersist into BetterQueue");
  }
  ...
  function applyPersist() {
    if (settings28.store.persistAcrossRefresh)
      startPersist();
    else
      stopPersist();
  }
...
`);
  ...
  var logger39 = new Logger("BetterModeSelect");
  ...
  var OLD_NAME3 = "CompactModeSelect";
  var NEW_NAME3 = "BetterModeSelect";
  ...
  function migrateLegacy6() {
    ...
    SettingsStore3.markAsChanged();
    logger39.info("Migrated CompactModeSelect into BetterModeSelect");
  }
  ...
  function setOrder(ids) {
    settings29.store.pinOrder = ids.join(",");
  }
  function setPinned(pin, on) {
    settings29.store[pin] = on;
  }
  ...
  async function harvestIcons() {
    ...
    try {
      ...
      logger39.warn("Failed to harvest mode icons:", e);
    } finally {
      setPicking(false);
      harvesting = false;
    }
  ...
  async function selectMode(id) {
    ...
    try {
      ...
      if (!menu) {
        const trigger = nativeTrigger();
        if (!trigger) {
          logger39.warn("Native mode selector not found");
          return;
        }
        clickEl(trigger);
        menu = await waitForMenu();
      ...
      if (!menu) {
        logger39.warn("Native mode item not found:", id);
        return;
      }
      cloak(menu);
      stashGlyphs(menu.items);
      ...
      logger39.warn("Failed to select mode:", e);
    } finally {
      setPicking(false);
    }
  }
  ...
  function PinOrderEditor() {
    const cfg = settings29.use(["pinAuto", "pinFast", "pinExpert", "pinHeavy", "pinBuild", "pinOrder"]);
  ...
  function PinnedModes() {
    const cfg = settings29.use([...SETTING_KEYS]);
...
`);
  ...
  var cl31 = classNameFactory("void-sf-");
  ...
  function VoidPPSection({ tabs }) {
    ...
    return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement(Text2, {
      size: "xs",
      color: "secondary",
      className: cl31("group")
    }, "Void++"), tabItems(tabs));
  }
  function SettingsMenu({ onOpen }) {
    const cfg = settings30.use([
      "showOpenSettings",
      "voidppPosition",
      "plugins",
      "themes",
  ...
  var EDITOR_SEL3 = '.tiptap.ProseMirror[contenteditable="true"]';
  var STOP_SELECTORS = [
    'button[aria-label="Stop model response"]',
  ...
  function getActiveEditor() {
    const list = Array.from(document.querySelectorAll(EDITOR_SEL3));
  ...
  var logger40 = new Logger("ChatStateFavicons");
  ...
  function currentStyle() {
    const value = settings31.store.style;
  ...
  function storeStreaming() {
    try {
      ...
      logger40.debug("stream stores unavailable:", e);
      return false;
    }
  }
  function officialInterruptedDom() {
    try {
      ...
      logger40.debug("interrupt DOM unavailable:", e);
      return false;
    }
  }
  function currentChatInterrupted() {
    try {
      ...
      logger40.debug("interrupt lookup failed:", e);
    }
  ...
  function currentConversationId() {
    try {
      ...
      logger40.debug("RoutingStore unavailable:", e);
    }
    try {
      ...
      logger40.debug("ChatPageStore unavailable:", e);
    }
  ...
  function hasError() {
    ...
    try {
      ...
      logger40.debug("ResponseStore unavailable:", e);
      return false;
    }
  }
  ...
  function attachStores() {
    ...
    try {
      ...
      logger40.debug("RoutingStore subscribe failed:", e);
      try {
        unsubRoute2 = RoutingStore.useRoutingStore.subscribe(() => scheduleEvaluate());
      } catch (err) {
        logger40.debug("RoutingStore full subscribe failed:", err);
      }
    }
    try {
      ...
      logger40.debug("ChatPageStore subscribe failed:", e);
    }
    try {
      ...
      logger40.debug("ResponseStore subscribe failed:", e);
    }
  }
...
`;
  ...
  function apply7() {
    ...
    if (settings32.store.hideDictationRefinement)
...
`);
  ...
  var logger41 = new Logger("BetterFiles");
  ...
  async function deleteAssets(ids) {
    ...
    for (const id of ids) {
      ...
      try {
        ...
        logger41.error("Failed to delete asset", id, e);
      }
  ...
  function apply8() {
    const rules = [];
    if (settings33.store.hideShareProject) {
      rules.push('button[aria-label="Share Project"]{display:none!important}');
    }
    if (settings33.store.hideCreateShareLink) {
...
`);
  ...
  var logger42 = new Logger("ChatListStatus");
  var MARK2 = "void-cls";
  ...
  function currentIds2() {
    ...
    try {
      ...
      logger42.debug("page ids unavailable:", e);
    }
    try {
      const { route } = RoutingStore.useRoutingStore.getState();
      add(route.conversationId);
      ...
      logger42.debug("route ids unavailable:", e);
    }
    try {
      const url = new URL(location.href);
      add(url.searchParams.get("chat"));
      ...
      logger42.debug("url ids unavailable:", e);
    }
    return ids;
  }
  ...
  function officialInterruptedDom2() {
    try {
      const root = document.querySelector("main") ?? document.body;
      return USER_INTERRUPT5.test(root.textContent ?? "");
    } catch {
      return false;
    }
  }
  function currentChatInterrupted2() {
    try {
      const page = ChatPageStore.useChatPageStore.getState();
      const { byId, byConversationId } = ResponseStore.useResponseStore.getState();
      if (isUserInterrupt5(byId[page.streamedMessageId ?? ""]) || isUserInterrupt5(byId[page.lastMessageId ?? ""]))
        return true;
      for (const cid of currentIds2()) {
        if (isUserInterrupt5(lastAssistant2(cid, byConversationId)))
          return true;
      }
    } catch {}
  ...
  function liveIds() {
    ...
    try {
      ...
      logger42.debug("stream stores unavailable:", e);
    }
    try {
      const { byId, byIdWithWorkspaces, list } = ConversationStore.useConversationStore.getState();
      ...
      logger42.debug("conversation store unavailable:", e);
    }
    extraLiveIds(ids);
  ...
  function errorOf(id) {
    try {
      ...
      logger42.debug("error lookup failed:", e);
    }
    return false;
  }
  function interruptOf(id) {
    try {
      ...
      logger42.debug("interrupt lookup failed:", e);
    }
    return false;
  }
  ...
  function convOfResponse(responseId) {
    try {
      ...
      logger42.debug("conv lookup failed:", e);
      return "";
    }
  }
  function onStreamEnd8({ responseId }) {
    ...
    try {
      ...
      logger42.debug("streamEnd lookup failed:", e);
    }
    if (liveIds().has(cid)) {
      schedule2();
      return;
    }
    if (currentIds2().includes(cid) || isUserInterrupt5(response)) {
      marks.delete(cid);
      schedule2();
      return;
    }
  ...
  function paint4() {
    ...
    if (live.length && !rowById.size)
      logger42.info("live ids with no rows", live);
  }
  function schedule2() {
    if (!started6 || raf5)
      return;
  ...
  function ColorRow2() {
    const { starColor } = settings34.use(["starColor"]);
    return /* @__PURE__ */ React.createElement(ColorSettingRow, {
      value: starColor,
      onChange: (v) => {
        settings34.store.starColor = v;
      },
      title: "Star color",
...
`;
  ...
  var __plugins_default = { [noTelemetry_default.name]: noTelemetry_default, [settings_default.name]: settings_default, [fixChrome_default.name]: fixChrome_default, [chatBarButtons_default.name]: chatBarButtons_default, [contextMenu_default.name]: contextMenu_default, [betterNavigator_default.name]: betterNavigator_default, [noSidebarIdentity_default.name]: noSidebarIdentity_default, [completeToast_default.name]: completeToast_default, [cleaner_default.name]: cleaner_default, [betterSidebar_default.name]: betterSidebar_default, [betterImagine_default.name]: betterImagine_default, [messageTimestamps_default.name]: messageTimestamps_default, [autoRetry_default.name]: autoRetry_default, [userQuotes_default.name]: userQuotes_default, [betterQuotes_default.name]: betterQuotes_default, [cloneChats_default.name]: cloneChats_default, [streamerMode_default.name]: streamerMode_default, [inputHistory_default.name]: inputHistory_default, [customSidebarIdentity_default.name]: customSidebarIdentity_default, [downloadTTS_default.name]: downloadTTS_default, [recentTopics_default.name]: recentTopics_default, [customGreeting_default.name]: customGreeting_default, [betterAvatarPlugins_default.name]: betterAvatarPlugins_default, [betterLinks_default.name]: betterLinks_default, [experiments_default.name]: experiments_default, [customInstructions_default.name]: customInstructions_default, [noBuildStarters_default.name]: noBuildStarters_default, [responseNotification_default.name]: responseNotification_default, [incognito_default.name]: incognito_default, [betterCanvas_default.name]: betterCanvas_default, [composerOpacity_default.name]: composerOpacity_default, [exportChat_default.name]: exportChat_default, [autoCollapse_default.name]: autoCollapse_default, [usageDisplay_default.name]: usageDisplay_default, [widerChat_default.name]: widerChat_default, [betterQueue_default.name]: betterQueue_default, [betterModeSelect_default.name]: betterModeSelect_default, [settingsFlyout_default.name]: settingsFlyout_default, [chatStateFavicons_default.name]: chatStateFavicons_default, [noDictation_default.name]: noDictation_default, [betterFiles_default.name]: betterFiles_default, [noShareLink_default.name]: noShareLink_default, [chatListStatus_default.name]: chatListStatus_default, [stableComposer_default.name]: stableComposer_default, [consoleJanitor_default.name]: consoleJanitor_default, [oneko_default.name]: oneko_default, [starry_default.name]: starry_default, [pluginsFlyout_default.name]: pluginsFlyout_default, [noGrokBot_default.name]: noGrokBot_default };
