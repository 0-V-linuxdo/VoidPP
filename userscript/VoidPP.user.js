...
/**
 * Void++ [20260920.5] v1.0.0 — A modification for grok.com
...
`);
  ...
  function VersionInfo() {
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      as: "span",
      color: "secondary"
    }, "[20260920.5] v1.0.0"), /* @__PURE__ */ React.createElement(Dot, null), /* @__PURE__ */ React.createElement(VersionLink, {
      href: `${"https://github.com/0-V-linuxdo/VoidPP"}/commit/${"f7dff03"}`
    }, `(${"f7dff03"})`)), /* @__PURE__ */ React.createElement(Flex, {
      alignItems: "center",
      gap: "0.25rem"
    }, /* @__PURE__ */ React.createElement(Text2, {
      as: "span",
...
`);
  ...
  function apply2() {
    const pct = clamp(settings14.store.opacity, 0, 100);
    const blur = clamp(settings14.store.blur, 0, 40);
    const alpha = pct / 100;
...
`);
  ...
  function getEntries() {
    const raw = settings16.plain.entries;
    return Array.isArray(raw) ? raw.filter((x) => typeof x === "string") : [];
  }
  function cap(entries) {
    const max = clamp(settings16.store.maxEntries ?? MAX_DEFAULT, MAX_MIN, MAX_MAX);
    return entries.length > max ? entries.slice(entries.length - max) : entries;
  }
  function setEntries(entries) {
    settings16.store.entries = entries;
  }
...
`) : el.innerText ?? el.textContent ?? "";
  ...
  function HistoryPanel() {
    const { entries } = settings16.use(["entries"]);
    const list = entries ?? [];
...
`);
  ...
  function syncRestoreFlag() {
    if (!settings18.store.stickyOnNavigate) {
  ...
  function onNavigate() {
    ...
    if (!intent.modeId)
      intent = snapshot();
    if (!settings18.store.stickyOnNavigate || !intent.modeId)
      return;
    setRestoreFlag(true);
    applyIntent(intent);
    syncRestoreFlag();
...
`) : "";
  ...
  function buildPageShot(snap) {
    const page = node("span", cl25("page"));
    page.dataset.theme = snap.theme;
    for (const line of lastRound(snap.lines)) {
      const el = node("span", cl25("page-line", line.role === "user" && "page-line-user"), line.text);
  ...
  function fillShot(box, id) {
    ...
    if (!snap) {
      const fallback = node("span", cl25("fallback"));
      fallback.append(faviconImg(cl25("favicon")));
      box.append(fallback);
      return;
    }
    const shot = node("span", cl25("shot"));
  ...
  function folderIcon() {
    const svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg.setAttribute("class", cl25("folder"));
    svg.setAttribute("viewBox", "0 0 24 24");
    svg.setAttribute("fill", "none");
    svg.setAttribute("stroke", "currentColor");
...
`)) {
  ...
  function renderList(items) {
    if (!host2)
      return;
    const panel = host2.querySelector(`.${cl25("panel")}`);
    if (!panel)
      return;
    let list = panel.querySelector(`.${cl25("list")}`);
    if (!list) {
      panel.replaceChildren();
      list = node("div", cl25("list"));
      panel.append(list);
    }
    list.replaceChildren();
    items.forEach((topic, i) => {
      const btn = node("button", cl25("card"));
      btn.type = "button";
      btn.tabIndex = -1;
      ...
      const shot = node("span", cl25("thumb"));
      shot.setAttribute("aria-hidden", "true");
      fillShot(shot, topic.id);
      const meta = node("span", cl25("meta"));
      meta.append(node("span", cl25("name"), topic.title));
      if (topic.project) {
        const proj = node("span", cl25("host"));
        proj.append(projectIconOf(topic.ws), node("span", cl25("host-name"), topic.project));
        meta.append(proj);
      }
  ...
  function patchList(items) {
    if (!host2)
      return;
    const cards = [...host2.querySelectorAll(`.${cl25("card")}`)];
    ...
    items.forEach((topic, i) => {
      ...
      const name = card.querySelector(`.${cl25("name")}`);
      if (name)
        name.textContent = topic.title;
      const meta = card.querySelector(`.${cl25("meta")}`);
      if (!meta)
        return;
      let row = meta.querySelector(`.${cl25("host")}`);
      if (!topic.project) {
        row?.remove();
        return;
      }
      if (!row) {
        row = node("span", cl25("host"));
        row.append(projectIconOf(topic.ws), node("span", cl25("host-name"), topic.project));
        meta.append(row);
        return;
      }
      const label = row.querySelector(`.${cl25("host-name")}`);
      if (label)
  ...
  function paint3() {
    ...
    if (!items.length) {
      if (paintedIds !== "__empty__") {
        panel.replaceChildren(node("div", cl25("empty"), "Open a few chats, then hold Ctrl+` to switch."));
    ...
    if (paintedIds === "__empty__" || !panel.querySelector(`.${cl25("list")}`)) {
      panel.replaceChildren(node("div", cl25("list")));
      paintedIds = "";
      paintedMeta = "";
    }
    if (paintedIds !== ids) {
