...
`);
  ...
  function paint2() {
    paintRaf = 0;
    if (!settings12.store.showQueueMode || onImaginePage2()) {
      unpaint();
      return;
    }
    const card = trayCard();
...
`);
  ...
  function unwrapSendFns2() {
    ...
    unwrapOne(() => ChatPageStore.useChatPageStore.getState(), (p) => ChatPageStore.useChatPageStore.setState(p), "sendResponse", "chat.sendResponse");
    origFns2.clear();
    wrappedFns2.clear();
  }
  ...
  function wrapFetch() {
    ...
    window.fetch = function voidQuoteStickyFetch(input, init) {
      ...
      if ((method === "POST" || method === "PUT") && CHAT_POST2.test(url)) {
        const key = liveKey();
        const clip = (saved.get(key)?.text || readText().text).replaceAll(/\s+/g, " ").trim().slice(0, 16);
        if (clip && requestBody(input, init).includes(clip))
          markConsumed(key);
      }
...
`);
  ...
  var logger29 = new Logger("ExportChat");
...
`);
  ...
  function ExportMenu({ conversationId }) {
    ...
    return /* @__PURE__ */ React.createElement(MenuSub, null, /* @__PURE__ */ React.createElement(MenuSubTrigger, {
      ...
      key: fmt,
      onSelect: () => exportChat(conversationId, fmt).catch((e) => logger29.error("Failed to export chat", e))
    }, label))));
  }
...
`);
  ...
  var logger32 = new Logger("ChatStateFavicons");
  ...
  function storeStreaming() {
    try {
      ...
      logger32.debug("stream stores unavailable:", e);
      return false;
    }
  }
  function officialInterruptedDom() {
    try {
      ...
      logger32.debug("interrupt DOM unavailable:", e);
      return false;
    }
  }
  function currentChatInterrupted() {
    try {
      ...
      logger32.debug("interrupt lookup failed:", e);
    }
  ...
  function currentConversationId() {
    ...
    try {
      ...
      logger32.debug("ChatPageStore unavailable:", e);
    }
  ...
  function hasError() {
    ...
    try {
      ...
      logger32.debug("ResponseStore unavailable:", e);
      return false;
    }
  }
  ...
  function attachStores() {
    ...
    try {
      ...
      logger32.debug("RoutingStore subscribe failed:", e);
      try {
        unsubRoute2 = RoutingStore.useRoutingStore.subscribe(() => scheduleEvaluate());
      } catch (err) {
        logger32.debug("RoutingStore full subscribe failed:", err);
      }
    }
    try {
      ...
      logger32.debug("ChatPageStore subscribe failed:", e);
    }
    try {
      ...
      logger32.debug("ResponseStore subscribe failed:", e);
    }
  }
...
`);
  ...
  var logger33 = new Logger("BetterFiles");
  ...
  async function deleteAssets(ids) {
    ...
    for (const id of ids) {
      ...
      try {
        ...
        logger33.error("Failed to delete asset", id, e);
      }
...
`);
  ...
  var logger34 = new Logger("ChatListStatus");
  var MARK2 = "void-cls";
  ...
  function currentIds2() {
    ...
    try {
      ...
      logger34.debug("page ids unavailable:", e);
    }
    try {
      const { route } = RoutingStore.useRoutingStore.getState();
      add(route.conversationId);
      ...
      logger34.debug("route ids unavailable:", e);
    }
    try {
      const url = new URL(location.href);
      add(url.searchParams.get("chat"));
      ...
      logger34.debug("url ids unavailable:", e);
    }
    return ids;
  }
  ...
  function liveIds() {
    ...
    try {
      ...
      logger34.debug("conversation store unavailable:", e);
    }
    extraLiveIds(ids);
  ...
  function errorOf(id) {
    try {
      ...
      logger34.debug("error lookup failed:", e);
    }
    return false;
  }
  ...
  function convOfResponse(responseId) {
    try {
      ...
      logger34.debug("conv lookup failed:", e);
      return "";
    }
  }
  function onStreamEnd8({ responseId }) {
    ...
    try {
      ...
      logger34.debug("streamEnd lookup failed:", e);
    }
    if (liveIds().has(cid)) {
      schedule2();
      return;
  ...
  function paint4() {
    ...
    if (live.length && !rowById.size)
      logger34.info("live ids with no rows", live);
  }
  function schedule2() {
    if (!started5 || raf4)
      return;
  ...
  function observe() {
    obs2?.disconnect();
    obs2 = new MutationObserver((list) => {
      if (ownMutation(list))
        return;
      schedule2();
    });
    const node = document.querySelector(SIDEBAR) ?? document.body;
    obs2.observe(node, node === document.body ? { childList: true, subtree: true } : { childList: true, subtree: true, attributes: true, attributeFilter: ["href"] });
  }
  function pageKey3(s) {
...
`);
  ...
  var KEEP2 = /submit|send|attach|dictat|mode|file/i;
  var FLASH_MS2 = 1800;
  var WAIT_MS = 50;
  var WAIT_N = 24;
  var ALIGNED_PX = 8;
  ...
  function storeNeedle(needle) {
    ...
    try {
      ...
      logger35.debug("store search failed", e);
    }
    return null;
  }
  ...
  async function hydrate2(cid) {
    ...
    try {
      ...
      logger35.debug("loadResponses failed", e);
    }
    try {
      await ResponseStore.useResponseStore.getState().loadMoreResponses?.(cid);
    } catch (e) {
      logger35.debug("loadMoreResponses failed", e);
    }
  }
  ...
  async function jump2(origin) {
    ...
    if (mine !== gen)
      return;
    if (!el) {
      logger35.debug("no source message");
      return;
    }
...
`);
  ...
  async function harvestIcons() {
    ...
    try {
      ...
      logger36.warn("Failed to harvest mode icons:", e);
    } finally {
      setPicking(false);
      harvesting = false;
    }
  ...
  async function selectMode(id) {
    ...
    try {
      ...
      if (!menu) {
        const trigger = nativeTrigger();
        if (!trigger) {
          logger36.warn("Native mode selector not found");
          return;
        }
        clickEl(trigger);
        menu = await waitForMenu();
      ...
      if (!menu) {
        logger36.warn("Native mode item not found:", id);
        return;
      }
      cloak(menu);
      stashGlyphs(menu.items);
      ...
      logger36.warn("Failed to select mode:", e);
    } finally {
      setPicking(false);
    }
  }
...
Neon rain in a quiet city`
  ...
  // virtual:~plugins
  noTelemetry_default.updatedAt = 1789881199000;
  settings_default.updatedAt = 1789897669000;
  fixChrome_default.updatedAt = 1789881199000;
  fixChrome_default.chrome = true;
  fixChrome_default.hidden = !window.chrome;
  chatBarButtons_default.updatedAt = 1790097681000;
  contextMenu_default.updatedAt = 1789881199000;
  betterNavigator_default.updatedAt = 1789908648000;
  noSidebarIdentity_default.updatedAt = 1789881199000;
  ...
  cloneChats_default.updatedAt = 1789881199000;
  streamerMode_default.updatedAt = 1789881199000;
  inputHistory_default.updatedAt = 1790093417000;
  customSidebarIdentity_default.updatedAt = 1789918488000;
  downloadTTS_default.updatedAt = 1789881199000;
  recentTopics_default.updatedAt = 1789881199000;
  ...
  noSidebarPlugins_default.updatedAt = 1789881199000;
  composerOpacity_default.updatedAt = 1790097681000;
  exportChat_default.updatedAt = 1789881199000;
  autoCollapse_default.updatedAt = 1789881199000;
  usageDisplay_default.updatedAt = 1789881199000;
  widerChat_default.updatedAt = 1789881199000;
  settingsFlyout_default.updatedAt = 1789881199000;
  chatStateFavicons_default.updatedAt = 1789921507000;
  noDictation_default.updatedAt = 1789881199000;
  ...
  oneko_default.updatedAt = 1789881199000;
  starry_default.updatedAt = 1789881199000;
  pluginsFlyout_default.updatedAt = 1789881199000;
  noGrokBot_default.updatedAt = 1789881199000;
