...
${sourceUrl}`;
  ...
  var MessageCircleIcon = (props = {}) => svg(props, /* @__PURE__ */ React.createElement("path", {
...
`);
  ...
  function snapshotToday() {
    if (!settings6.store.usageStats)
      return;
    syncAccount();
    if (!state.userId)
      return;
    recordSnapshot(state.userId, state.usage?.weekly.usedPercent ?? null, state.usage?.weekly.resetAt ?? null, retainDaysOf(settings6.store.retainDays));
  }
  ...
  function ButtonIcon() {
    ...
    const { showPercent } = settings6.use(["showPercent"]);
  ...
  function UsagePanel() {
    useExternalStore(store2);
    const { usageStats, hoverStatsDelay } = settings6.use(["usageStats", "hoverStatsDelay"]);
  ...
  function StatsToggle() {
    const { usageStats } = settings6.use(["usageStats"]);
    return /* @__PURE__ */ React.createElement(Flex, {
      alignItems: "center",
      justifyContent: "space-between",
      gap: "0.75rem",
      ...
      onCheckedChange: (value) => {
        settings6.store.usageStats = value;
        store2.notify();
        if (value)
          refresh("manual");
      }
  ...
  function RepairReset({ rec, prev, userId }) {
    ...
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      onClick: () => {
        if (pre == null)
          return;
        writeDay(userId, repairWipedReset(rec, dayStart, pre, Date.now()), retainDaysOf(settings6.store.retainDays));
        store2.notify();
      }
    }, "Repair")));
  }
...
`);
  ...
  var logger20 = new Logger("QueuePersist");
  ...
  function withIntent(intent, fn) {
    ...
    try {
      ...
      logger20.debug("intent apply failed", e);
    }
    try {
      if (intent?.modeId)
      ...
      try {
        ...
        logger20.debug("intent restore failed", e);
      }
    }
  }
  function qid(item) {
  ...
  function syncOne(cid, hydrated) {
    ...
    const bound = bindPending(freshSnaps(memory2.get(cid) ?? [], now), official, pending.get(cid) ?? [], now);
    if (bound.pending.length)
      pending.set(cid, bound.pending);
    else
      pending.delete(cid);
    let next = projectQueue(bound.saved, official, bound.pending, hydrated, now);
    if (hydrated && !replaying && cid === currentCid())
  ...
  async function persist() {
    ...
    try {
      ...
      logger20.debug("persist failed", e);
    }
  }
  async function load() {
    try {
  ...
  async function replay(cid, saved) {
    ...
    for (const item of ordered) {
      ...
      try {
        ...
        logger20.debug("replay failed", e);
      } finally {
        suppress = false;
      }
  ...
  async function restore(cid) {
    ...
    const saved = freshSnaps(memory2.get(cid) ?? [], Date.now());
    ...
    try {
      ...
      logger20.debug("restore failed", e);
      retry = true;
    } finally {
      replaying = false;
  ...
  function syncRestoreFlag() {
    if (!settings7.store.stickyOnNavigate) {
  ...
  function fightHydrate() {
    if (sendOverride || !settings7.store.stickyOnNavigate || applying || userPicking || awaitingMenu || !intent.modeId)
      return;
    if (!loadPending())
      return;
    const cur = snapshot();
  ...
  function onNavigate() {
    ...
    closeMenu();
    schedulePaint();
    if (!settings7.store.stickyOnNavigate || !intent.modeId)
      return;
    setRestoreFlag(true);
    applyIntent(intent);
    syncRestoreFlag();
  ...
  function flushTurn(cid) {
    ...
    forgetItem(turn.id);
    logger21.info("flushed", turn.id, "as", item.modeId, "session", ackedModel.get(cid) ?? "?", busy.has(cid) ? "busy" : "idle");
  }
  ...
  function paint() {
    paintRaf = 0;
    if (!settings7.store.showQueueMode || onImaginePage()) {
      unpaint();
      return;
    }
    stripToggleChips();
  ...
  var logger22 = new Logger("BetterQueue");
  ...
  function migrateLegacy2() {
    ...
    SettingsStore3.markAsChanged();
    logger22.info("Migrated ModeSync / QueuePersist into BetterQueue");
  }
  ...
  function applyPersist() {
    if (settings7.store.persistAcrossRefresh)
      startPersist();
    else
      stopPersist();
  }
...
`);
  ...
  function apply2() {
    ...
    if (!settings9.store.italic)
      rules.push(`${SEL}{font-style:inherit!important}`);
    if (!settings9.store.quotes) {
...
`));
  ...
  function officialInterruptedDom() {
    try {
      ...
      for (const node of document.querySelectorAll("[data-testid='assistant-message']")) {
        if (USER_INTERRUPT.test(node.textContent ?? ""))
          return true;
      }
      return false;
    } catch (e) {
  ...
  function currentChatInterrupted() {
    try {
      const page = ChatPageStore.useChatPageStore.getState();
      const { byId } = ResponseStore.useResponseStore.getState();
      if (isUserInterrupt(byId[page.streamedMessageId ?? ""]) || isUserInterrupt(byId[page.lastMessageId ?? ""]))
        return true;
    } catch (e) {
  ...
  function hasError() {
    ...
    try {
      ...
      if (!id)
        return false;
      const response = byId[id];
      if (!response || isUserInterrupt(response))
        return false;
...
`);
  ...
  var cl19 = classNameFactory("void-rt-");
  var HOME_KEY = "home";
  var HOME_SEP = "home:";
  ...
  function maxCount() {
    const n = Number(settings11.store.maxRecent);
    return Number.isFinite(n) && n > 0 ? n : 5;
  }
  function capVisits(ids) {
    const allowHome = settings11.store.includeHome;
    const current = currentVisit();
    const dirtyGlobalWs = asWorkspaceId(settings11.plain.workspaceByConv?.[HOME_KEY]);
    const seen = new Set;
    const out = [];
  ...
  function assignRecord(key, next) {
    if (sameRecord(settings11.plain[key], next))
      return false;
    settings11.store[key] = next;
    return true;
  }
  ...
  function initEffect() {
    ...
    if (disk) {
      ...
      const fromSettings = {
        visits: asStringList(settings11.plain.visits),
        deniedIds: asStringList(settings11.plain.deniedIds),
        deniedAt: asStringRecord(settings11.plain.deniedAt)
      };
      effect = {
        v: 1,
  ...
  function commitVisits(next) {
    ...
    const rawWs = pruneRecord(settings11.plain.workspaceByConv, visits);
    ...
    const pages = pruneRecord(settings11.plain.pages, visits);
    ...
    for (const [id, name] of Object.entries(settings11.plain.projectNames ?? {})) {
    ...
    for (const [id, snap] of Object.entries(settings11.plain.projectIcons ?? {})) {
    ...
    const titles = {};
    for (const [id, name] of Object.entries(pruneRecord(settings11.plain.titles, visits))) {
  ...
  function rememberTitle(id, title) {
    ...
    if (id === chatIdFromUrl() && isAccessDeniedPage())
      return;
    const prev = settings11.plain.titles ?? {};
    if (prev[id] === t)
      return;
    settings11.store.titles = { ...prev, [id]: t };
  }
  ...
  function titleOf(id) {
    if (!id || isHomeId(id))
      return "New chat";
    if (isDenied(id))
      return usableTitle(settings11.plain.titles?.[id]) || "Untitled";
    const conv = lookup(id);
    return usableTitle(conv?.title) || usableTitle(settings11.plain.titles?.[id]) || titleFromPage(id) || "Untitled";
  }
...
`) : "";
  ...
  function rememberProjectIcon(ws) {
    ...
    const prev = settings11.plain.projectIcons ?? {};
    if (snap) {
      wsIcons[ws] = snap;
      if (prev[ws] !== snap)
        settings11.store.projectIcons = { ...prev, [ws]: snap };
      return;
    }
    if (prev[ws] && isChromeSnap(prev[ws])) {
      ...
      delete next[ws];
      delete wsIcons[ws];
      settings11.store.projectIcons = next;
    }
  }
  ...
  function dropWorkspace(id) {
    const prev = settings11.plain.workspaceByConv ?? {};
    if (!prev[id])
      return;
    const next = { ...prev };
    delete next[id];
    settings11.store.workspaceByConv = next;
  }
  function workspaceOf(id) {
    ...
    if (fromSidebar)
      return fromSidebar;
    const cached = asWorkspaceId(settings11.plain.workspaceByConv?.[id]);
    if (cached)
      return cached;
  ...
  function projectNameOf(id) {
    ...
    if (!ws)
      return "";
    const idx = sidebarIndex();
    const named = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || wsNames[ws] || settings11.plain.projectNames?.[ws] || "");
    if (!named)
      return "";
    const live = liveWorkspaceId();
  ...
  function rememberProject(id) {
    ...
    if (!ws)
      return;
    const prevWs = settings11.plain.workspaceByConv ?? {};
    if (prevWs[id] !== ws)
      settings11.store.workspaceByConv = { ...prevWs, [id]: ws };
    const idx = sidebarIndex();
    const sidebarName = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || "");
    const liveName = ws === liveWorkspaceId() ? readOpenProjectName() : "";
    const cached = usableName(wsNames[ws] || settings11.plain.projectNames?.[ws] || "");
    ...
    if (!name)
      return;
    wsNames[ws] = name;
    const prevNames = settings11.plain.projectNames ?? {};
    if (prevNames[ws] !== name)
      settings11.store.projectNames = { ...prevNames, [ws]: name };
  }
  function reconcileSidebarCache() {
    const idx = sidebarIndex();
    const prevWs = { ...settings11.plain.workspaceByConv };
    const prevNames = { ...settings11.plain.projectNames };
    const prevIcons = { ...settings11.plain.projectIcons };
    let wsChanged = false;
    ...
    if (wsChanged)
      settings11.store.workspaceByConv = prevWs;
    if (namesChanged)
      settings11.store.projectNames = prevNames;
    if (iconsChanged)
      settings11.store.projectIcons = prevIcons;
  }
  function requestWorkspace(id) {
    ...
    try {
      ...
      fetchConv(id).then((conv) => {
        ...
        const prev = settings11.plain.workspaceByConv ?? {};
        if (prev[id] !== ws)
          settings11.store.workspaceByConv = { ...prev, [id]: ws };
        const live = liveWorkspaceId();
        const liveName = usableName(readOpenProjectName());
        const names = settings11.plain.projectNames ?? {};
        if (live && ws !== live && liveName && names[ws] === liveName) {
          const next = { ...names };
          delete next[ws];
          settings11.store.projectNames = next;
          delete wsNames[ws];
        }
        maybePaint();
  ...
  function snapOf(id) {
    if (!id || isHomeId(id))
      return null;
    const snap = thumbs.get(id) ?? parseSnap(settings11.plain.pages?.[id]);
    if (!snap)
      return null;
  ...
  function rememberPage(id, snap) {
    const json = JSON.stringify(snap);
    const prev = settings11.plain.pages ?? {};
    if (prev[id] === json)
      return;
    settings11.store.pages = { ...prev, [id]: json };
  }
  ...
  function buildPageShot(snap) {
    const page = node("span", cl19("page"));
    page.dataset.theme = snap.theme;
    for (const line of lastRound(snap.lines)) {
      const el = node("span", cl19("page-line", line.role === "user" && "page-line-user"), line.text);
  ...
  function captureId(id) {
    ...
    if (!lines.length)
      return;
    const prev = thumbs.get(id) ?? parseSnap(settings11.plain.pages?.[id]);
  ...
  function bump(id) {
    if (!id)
      return;
    if (isHomeId(id) && !settings11.store.includeHome)
      return;
  ...
  function fillShot(box, id) {
    ...
    if (!snap) {
      const fallback = node("span", cl19("fallback"));
      fallback.append(faviconImg(cl19("favicon")));
      box.append(fallback);
      return;
    }
    const shot = node("span", cl19("shot"));
  ...
  function folderIcon() {
    ...
    svg.setAttribute("class", cl19("folder"));
...
`)) {
  ...
  function projectIconOf(ws) {
    const raw = ws ? wsIcons[ws] || settings11.plain.projectIcons?.[ws] || liveIconSnap(ws) || "" : "";
  ...
  function renderList(items) {
    ...
    if (!list) {
      ...
      list = node("div", cl19("list"));
      panel.append(list);
    }
    list.replaceChildren();
    items.forEach((topic, i) => {
      const btn = node("button", cl19("card"));
      btn.type = "button";
      btn.tabIndex = -1;
      ...
      const shot = node("span", cl19("thumb"));
      shot.setAttribute("aria-hidden", "true");
      fillShot(shot, topic.id);
      const meta = node("span", cl19("meta"));
      meta.append(node("span", cl19("name"), topic.title));
      if (topic.project) {
        const proj = node("span", cl19("host"));
        proj.append(projectIconOf(topic.ws), node("span", cl19("host-name"), topic.project));
        meta.append(proj);
      }
  ...
  function patchList(items) {
    ...
    items.forEach((topic, i) => {
      ...
      const name = card.querySelector(`.${cl19("name")}`);
      if (name)
        name.textContent = topic.title;
      const meta = card.querySelector(`.${cl19("meta")}`);
      if (!meta)
        return;
      let row = meta.querySelector(`.${cl19("host")}`);
      if (!topic.project) {
        row?.remove();
        return;
      }
      if (!row) {
        row = node("span", cl19("host"));
        row.append(projectIconOf(topic.ws), node("span", cl19("host-name"), topic.project));
        meta.append(row);
        return;
      }
      const label = row.querySelector(`.${cl19("host-name")}`);
      if (label)
  ...
  function paint2() {
    ...
    if (!items.length) {
      if (paintedIds !== "__empty__") {
        panel.replaceChildren(node("div", cl19("empty"), "Open a few chats, then hold Ctrl+` to switch."));
    ...
    if (paintedIds === "__empty__" || !panel.querySelector(`.${cl19("list")}`)) {
      panel.replaceChildren(node("div", cl19("list")));
      paintedIds = "";
      paintedMeta = "";
    }
    if (paintedIds !== ids) {
  ...
  var recentTopics_default = definePlugin({
    ...
    start() {
      ...
      if (!keys) {
        ...
        const { signal } = keys;
        window.addEventListener("keydown", onKeyDown2, { capture: true, signal });
...
`);
  ...
  var logger25 = new Logger("BetterNavigator");
  ...
  function nodeTerminal(node) {
    ...
    if (node.status === "complete" || node.status === "stream-error" || node.status === "send-error")
      return true;
    return isDeadResponse2(node.content);
  }
    ...
    try {
      ...
      if (streamedId) {
        ...
        if (genNode && nodeTerminal(genNode) && sameGen)
          return false;
        if (streamed && !isDeadResponse2(streamed)) {
  ...
  function currentCid3() {
    try {
      ...
      logger25.debug("chat page unavailable:", e);
      return "";
    }
  }
  function gatewayOf(cid) {
    ...
    try {
      ...
      logger25.debug("message store unavailable:", e);
      return;
    }
  }
  ...
  function contentOf(cid, node) {
    try {
      ...
      logger25.debug("nodeToResponse failed:", e);
      return node.content;
    }
  }
  function nodeLive(node) {
    if (LIVE_NODE.has(node.status))
      return true;
    if (node.content?.partial && !isDeadResponse2(node.content))
      return true;
    const state = (node.content?.state ?? "").trim().toLowerCase();
    return LIVE.has(state) && !isDeadResponse2(node.content);
  }
  function collectDom() {
    const root = chatPane2() ?? document;
  ...
  function messageKey(s) {
    try {
      ...
      logger25.debug("message key failed:", e);
      return "";
    }
  }
  function onStreamEnd4(_data) {
    paint3();
  }
  function start() {
    if (ac)
  ...
  var logger26 = new Logger("ResponseNotification");
  ...
  function getCtx() {
    ...
    try {
      ...
      logger26.debug("AudioContext unavailable:", e);
      audioCtx = null;
      return null;
    }
  }
  function onUserGesture() {
    userGestured = true;
    if (settings13.store.browserNotification && Notification.permission === "default")
  ...
  function playUrl(ctx, url) {
    loadBuffer(ctx, url).then((buf) => playBuffer(ctx, buf), (err) => {
      logger26.info("sample play failed:", err);
      if (url !== DEFAULT_CHIME)
        loadBuffer(ctx, DEFAULT_CHIME).then((buf) => playBuffer(ctx, buf), (e) => logger26.info("default chime failed:", e));
    });
  }
  function playSound() {
    if (!userGestured) {
      logger26.info("sound skipped, no user gesture yet");
      return;
    }
    const ctx = getCtx();
    if (!ctx)
      return;
    const url = settings13.store.soundUrl?.trim() || DEFAULT_CHIME;
    if (ctx.state === "suspended")
      ctx.resume().then(() => playUrl(ctx, url), () => logger26.info("AudioContext resume failed"));
    else
      playUrl(ctx, url);
  }
  ...
  function notify(responseId, state) {
    logger26.info("notify", responseId, state ?? "unset", "permission", Notification.permission);
  ...
  function onStreamEnd5({ responseId }) {
    logger26.info("streamEnd", responseId);
    if (retryTimer)
      clearTimeout(retryTimer);
    const attempt = (retried) => {
      ...
      try {
        ...
        logger26.info("ResponseStore unavailable:", e);
      }
      ...
      if (isErrorResponse(response)) {
        logger26.info("skip error", responseId);
        return;
      }
      if (!retried) {
      ...
      logger26.info("skip", responseId, response?.state ?? "unset");
    };
    attempt(false);
  }
  ...
  function syncImagine(current, prev) {
    if (!settings13.store.imagineGeneration || !prev)
      return;
    if (onImaginePage2())
      return;
...
`);
  ...
  var logger27 = new Logger("BetterModeSelect");
  var cl21 = classNameFactory("void-cms-");
  var MODES = [
  ...
  function migrateLegacy3() {
    ...
    SettingsStore3.markAsChanged();
    logger27.info("Migrated CompactModeSelect into BetterModeSelect");
  }
  var pluginName2 = Object.getOwnPropertyDescriptor(settings15, "pluginName");
  if (pluginName2?.set && pluginName2.get) {
    Object.defineProperty(settings15, "pluginName", {
      configurable: true,
      enumerable: true,
      get: pluginName2.get,
      set(name) {
        if (name === NEW_NAME)
          migrateLegacy3();
        pluginName2.set.call(settings15, name);
      }
    });
  }
  var picking = false;
  ...
  function uncloak() {
    for (const host of ghosts) {
      host.classList.remove(cl21("ghost"));
  ...
  async function harvestIcons() {
    ...
    try {
      ...
      logger27.warn("Failed to harvest mode icons:", e);
    } finally {
      setPicking(false);
      harvesting = false;
    }
  ...
  async function selectMode(id) {
    ...
    try {
      ...
      if (!menu) {
        const trigger = nativeTrigger();
        if (!trigger) {
          logger27.warn("Native mode selector not found");
          return;
        }
        clickEl(trigger);
        menu = await waitForMenu();
      ...
      if (!menu) {
        logger27.warn("Native mode item not found:", id);
        return;
      }
      cloak(menu);
      stashGlyphs(menu.items);
      ...
      logger27.warn("Failed to select mode:", e);
    } finally {
      setPicking(false);
    }
  }
  ...
  function PinOrderEditor() {
    ...
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      return /* @__PURE__ */ React.createElement("div", {
        key: m.id,
        role: "listitem",
        className: classes(cl21("order-row"), dragId === m.id && cl21("dragging")),
        onDragOver: preventDragOver,
        onDrop: onDrop(m.id)
      }, /* @__PURE__ */ React.createElement(Flex, {
        alignItems: "center",
        gap: "0.5rem",
        className: cl21("order-main")
      }, /* @__PURE__ */ React.createElement("span", {
        className: cl21("grip"),
        draggable: true,
  ...
  function PinnedModes() {
    ...
    return /* @__PURE__ */ React.createElement("div", {
      className: classes(cl21("pins"), hideNative && cl21("hide-native"))
    }, items.map((m) => /* @__PURE__ */ React.createElement("span", {
      key: m.id,
      className: cl21("pin-host"),
      ...
      tooltip: m.label,
      onClick: onPin(m.id),
      className: classes(cl21("pin"), selectedModeId === m.id && cl21("on"), showLabels && cl21("labeled"), "hover:bg-button-ghost-hover"),
      "aria-label": m.label
    }))));
  }
...
`);
  ...
  function stamps() {
    if (cache)
      return cache;
    cache = new Map;
    const raw = settings17.plain.stamps;
    if (raw && typeof raw === "object") {
  ...
  var messageTimestamps_default = definePlugin({
    ...
    _renderTimestamp: ErrorBoundary.wrap(({ response, isUser }) => {
      ...
      if (settings17.store.hideOwnMessages && human)
        return null;
      ...
      }, formatTimestamp(ms, settings17.store.showDate));
    }),
    patches: [
      {
        find: "response-family:handleEditSave",
...
`);
  ...
  function syncClasses() {
    ...
    for (const [key, cls] of Object.entries(CSS_CLASSES)) {
      classList.toggle(cls, !!settings18.store[key]);
    }
  }
...
`);
  ...
  var logger29 = new Logger("QuoteJump");
  var cl22 = classNameFactory("void-qj-");
  var HL = "void-qj";
  ...
  function clearHighlight() {
    ...
    flashTimer2 = 0;
    flashing2?.classList.remove(cl22("hit"));
    flashing2 = null;
  ...
  var logger30 = new Logger("QuoteSticky");
  var cl23 = classNameFactory("void-qs-");
  var KEEP2 = 40;
  ...
  function officialVisible(text) {
    ...
    if (!clip)
      return false;
    for (const n of bar.querySelectorAll("div, span, button")) {
      if (!(n instanceof HTMLElement) || n.closest(`.${cl23("chip")}`))
        continue;
  ...
  function removeFallback() {
    for (const n of document.querySelectorAll(`.${cl23("chip")}`))
      n.remove();
  }
  ...
  function makeChip() {
    ...
    el.className = cl23("chip");
    el.dataset.voidQs = "";
    const mark = document.createElement("span");
    mark.className = cl23("mark");
    mark.setAttribute("aria-hidden", "true");
    mountQuoteMark(mark);
    const text = document.createElement("span");
    text.className = cl23("text");
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = cl23("x");
  ...
  function paintFallback(key, snap) {
    ...
    let el = document.querySelector(`.${cl23("chip")}`);
    ...
    el.dataset.voidQsKey = key;
    const label = el.querySelector(`.${cl23("text")}`);
  ...
  function dismiss() {
    ...
    try {
      ...
      logger30.debug("dismiss failed", e);
    } finally {
      applying2 = false;
    }
    removeFallback();
  ...
  function isQuoteDismiss(el) {
    if (el.closest(`.${cl23("x")}`))
      return true;
    const btn = barButton(el);
    if (!btn || btn.closest(`.${cl23("chip")}`))
      return false;
    const label = `${btn.getAttribute("aria-label") || ""} ${btn.getAttribute("title") || ""}`;
    if (KEEP.test(label))
      return false;
  ...
  var logger31 = new Logger("BetterQuotes");
  ...
  function migrateLegacy4() {
    ...
    if (jump?.enabled === false && !("jumpToPassage" in target))
      target.jumpToPassage = false;
    if (sticky?.enabled === false && !("persistAcrossChats" in target))
      target.persistAcrossChats = false;
    if (jump?.enabled === false && sticky?.enabled === false && typeof target.enabled !== "boolean")
      target.enabled = false;
    ...
    if (meta) {
      ...
      const known = meta.knownPlugins;
      if (known && typeof known === "object") {
        delete known.QuoteJump;
        delete known.QuoteSticky;
      }
    }
    SettingsStore3.markAsChanged();
    logger31.info("Migrated QuoteJump / QuoteSticky into BetterQuotes");
  }
  var pluginName3 = Object.getOwnPropertyDescriptor(settings20, "pluginName");
  if (pluginName3?.set && pluginName3.get) {
    Object.defineProperty(settings20, "pluginName", {
      configurable: true,
      enumerable: true,
      get: pluginName3.get,
      set(name) {
        if (name === "BetterQuotes")
          migrateLegacy4();
        pluginName3.set.call(settings20, name);
      }
    });
  }
  function apply5() {
    if (settings20.store.jumpToPassage)
      startJump();
    else
      stopJump();
    if (settings20.store.persistAcrossChats)
      startSticky();
    else
      stopSticky();
  }
...
`);
  ...
  function apply6() {
    const pct = clamp(settings21.store.opacity, 0, 100);
    const blur = clamp(settings21.store.blur, 0, 40);
    const alpha = pct / 100;
...
`);
  ...
  var logger33 = new Logger("ChatListStatus");
  var MARK2 = "void-cls";
  ...
  function liveIds() {
    ...
    try {
      ...
      logger33.debug("conversation store unavailable:", e);
    }
    extraLiveIds(ids);
  ...
  function errorOf(id) {
    try {
      ...
      logger33.debug("error lookup failed:", e);
    }
    return false;
  }
  ...
  function convOfResponse(responseId) {
    try {
      ...
      logger33.debug("conv lookup failed:", e);
      return "";
    }
  }
  function onStreamEnd6({ responseId }) {
    ...
    try {
      ...
      logger33.debug("streamEnd lookup failed:", e);
    }
    if (liveIds().has(cid)) {
      schedule2();
      return;
  ...
  function paint5() {
    ...
    if (live.length && !rowById.size)
      logger33.info("live ids with no rows", live);
  }
  function schedule2() {
    if (!started3 || raf4)
      return;
  ...
  function getColor(key, fallback) {
    const val = settings22.store[key];
  ...
  function applyColors() {
    ...
    if (settings22.store.enableVisitedColor) {
  ...
  function ColorRow2({ settingKey, title, description, fallback }) {
    settings22.use([settingKey]);
    return /* @__PURE__ */ React.createElement(ColorSettingRow, {
      value: getColor(settingKey, fallback),
      onChange: (v) => {
        settings22.store[settingKey] = v;
        applyColors();
      },
      title,
      description
  ...
  var betterLinks_default = definePlugin({
    ...
    start() {
      settings22.store.linkColor ??= DEFAULT_LINK;
      settings22.store.visitedColor ??= DEFAULT_VISITED;
      applyColors();
...
`);
  ...
  var logger35 = new Logger("CompleteToast");
  var cl25 = classNameFactory("void-ct-");
  ...
  function isUserInterrupt4(r) {
    ...
    if (state === "interrupted" || state === "stopped")
      return true;
    return USER_INTERRUPT4.test(errorBlob4(r.error)) || USER_INTERRUPT4.test(String(r.message ?? ""));
  }
  function isLiveResponse3(r) {
    if (!r || isUserInterrupt4(r))
      return false;
    if (r.partial)
      return true;
    const state = (r.state ?? "").trim().toLowerCase();
  ...
  function isErrorResponse3(r) {
    return !!r && !isUserInterrupt4(r) && (r.state === "error" || r.error != null);
  }
  ...
  function gatewayOf2(cid) {
    try {
      ...
      logger35.debug("MessageStore unavailable:", e);
      return;
    }
  }
  function isLiveCid(cid) {
    ...
    if (node && LIVE_NODE2.has(node.status))
      return true;
    if (isLiveResponse3(node?.content))
      return true;
    try {
      const { byConversationId } = ResponseStore.useResponseStore.getState();
      if (isLiveResponse3(lastAssistant2(cid, byConversationId)))
        return true;
    } catch (e) {
      logger35.debug("ResponseStore live lookup failed:", e);
    }
    return false;
  }
  function isBadFinish(cid, rid) {
    try {
      ...
      if (isUserInterrupt4(response) || isErrorResponse3(response))
        return true;
    } catch {}
    const node = lastAssistantNode(gatewayOf2(cid));
    if (node && (DEAD_ERR.has(node.status) || isUserInterrupt4(node.content) || isErrorResponse3(node.content)))
      return true;
    if (node && USER_INTERRUPT4.test(String(node.content?.message ?? "")))
      return true;
    return false;
  }
  function cidOf(responseId) {
    ...
    try {
      ...
      logger35.debug("gateway cid lookup failed:", e);
    }
    try {
      const { byConversationId } = ResponseStore.useResponseStore.getState();
      for (const [id, list] of Object.entries(byConversationId ?? {})) {
      ...
      logger35.debug("response cid lookup failed:", e);
    }
    return "";
  }
  function convOf(cid) {
    try {
      ...
      logger35.debug("conversation lookup failed:", e);
      return;
    }
  }
  function workspaceOf2(cid) {
    try {
      const { byId, byIdWithWorkspaces } = ConversationStore.useConversationStore.getState();
      ...
      logger35.debug("workspace lookup failed:", e);
      return "";
    }
  }
  function titleOf2(cid) {
  ...
  function applyChatPage2(id, ws) {
    try {
      ...
      logger35.debug("ChatPageStore update failed:", e);
    }
  }
  function navigateTo2(id) {
    ...
    try {
      ...
      routing.push(dest);
      applyChatPage2(cid, ws);
    } catch (e) {
      logger35.error("Failed to navigate:", e);
      try {
        location.assign(hrefFor2(cid, ws));
      } catch (navErr) {
        logger35.error("Fallback navigation failed:", navErr);
      }
    }
  }
  ...
  function shouldPersist() {
    return !!settings24.store.keepUntilDismissed || clamp(settings24.store.duration, 0, DURATION_MAX) <= 0;
  }
  function armTimer() {
    clearTimer();
    if (shouldPersist())
      return;
    const ms = clamp(settings24.store.duration, 0, DURATION_MAX) * 1000;
    hideAt = Date.now() + ms;
    hideTimer = setTimeout(hide, ms);
  }
  function migratePersist() {
    if (settings24.store.duration !== 0 || settings24.store.keepUntilDismissed)
      return;
  ...
  function show(cid, rid, kind = "chat", previewText = "") {
    ...
    if (kind === "chat" && onBotPage2())
      return;
    if (kind === "imagine" && onImaginePage4())
      return;
    toast = { cid, rid, kind };
    const root = document.createElement("div");
    root.id = HOST2;
    root.className = cl25("host");
    root.setAttribute("role", "status");
    root.setAttribute("aria-live", "polite");
    const card = document.createElement("div");
    card.className = cl25("card");
    const main = document.createElement("button");
    main.type = "button";
    main.className = cl25("main");
    const icon = document.createElement("span");
    icon.className = cl25("icon");
    icon.append(svgIcon("check"));
    const body = document.createElement("span");
    body.className = cl25("body");
    const title = document.createElement("span");
    title.className = cl25("title");
    title.textContent = kind === "imagine" ? "Imagine ready" : titleOf2(cid);
    body.append(title);
    const preview = kind === "imagine" ? previewText : previewOf(cid, rid);
    const sub = document.createElement("span");
    sub.className = cl25("preview");
  ...
  function navigateToImagine(id) {
    try {
      ...
      logger35.error("Failed to navigate to Imagine:", e);
      try {
        location.assign(id ? `/imagine/post/${encodeURIComponent(id)}` : "/imagine");
      } catch (navErr) {
        logger35.error("Fallback Imagine navigation failed:", navErr);
      }
    }
  }
  ...
  function maybeFinishImagine(id) {
    if (!started4 || !settings24.store.imagineGeneration || !id)
      return;
    if (onImaginePage4())
      return;
    ...
    try {
      ...
      logger35.debug("Imagine item unavailable:", e);
      return;
    }
    if (!item)
      return;
    ...
    show(id, "", "imagine", settings24.store.showPreview ? prompt.slice(0, PREVIEW_MAX) : "");
  }
  function syncImagine2(current, prev) {
    if (!started4 || !settings24.store.imagineGeneration)
      return;
    if (!prev)
      return;
    const now = new Set(current ? current.split(",") : []);
  ...
  function liveCids() {
    ...
    try {
      ...
      logger35.debug("gateway live scan failed:", e);
    }
    try {
      const { byConversationId } = ResponseStore.useResponseStore.getState();
      for (const [id, list] of Object.entries(byConversationId ?? {})) {
      ...
      logger35.debug("response live scan failed:", e);
    }
    return ids;
  }
  function seedToasted() {
    try {
      ...
      logger35.debug("seed responses failed:", e);
    }
    try {
      ...
      logger35.debug("seed gateway failed:", e);
    }
  }
    ...
    try {
      ...
      for (const id of Object.keys(byConversationId ?? {})) {
        ...
        if (!last?.responseId || toasted.has(last.responseId))
          continue;
        if (isLiveResponse3(last) || isLiveCid(id))
          continue;
        maybeFinish(id, last.responseId);
      }
    } catch (e) {
      logger35.debug("closed scan failed:", e);
    }
  }
  function syncLive() {
    if (!started4)
...
`);
  ...
  var logger36 = new Logger("BetterImagine");
  var cl26 = classNameFactory("void-imagine-");
  ...
  function buildFilename(post, isVideo) {
    if (!settings25.store.smartFilenames || !post)
      return null;
  ...
  function persist3() {
    if (!settings25.store.persistFilters)
      return;
    try {
...
${p.originalPrompt ?? ""}`.toLowerCase();
  ...
  function filterItems(items) {
    const { hideModerated } = settings25.store;
  ...
  var pending2 = new WeakMap;
  function pauseVideo(video) {
    const promise = pending2.get(video);
    pending2.delete(video);
    if (promise) {
      promise.then(() => {
        if (pending2.has(video))
          return;
        video.pause();
        video.currentTime = 0;
      }).catch((e) => logger36.warn("Failed to pause video:", e));
    } else {
      video.pause();
      video.currentTime = 0;
    }
  ...
  var onMouseEnter = (e) => {
    ...
    if (video)
      pending2.set(video, video.play().catch((e) => logger36.error("Failed to play video", e)));
  };
  var onMouseLeave = (e) => {
    const video = e.currentTarget.querySelector("video");
    if (video)
...
`));
      ...
      logger36.error(`Failed to copy ${label}s`, e);
    ...
    for (const id of ids) {
      ...
      for (const video of videos) {
        ...
        try {
          ...
          logger36.error("Failed to upscale video:", id, video.id, e);
        }
      }
    }
    if (upscaled)
  ...
  function FilterButtons() {
    ...
    return /* @__PURE__ */ React.createElement(Fragment, null, /* @__PURE__ */ React.createElement(Select, {
      ...
      className: sortActive ? cl26("sort-select", "sort-active") : cl26("sort-select")
  ...
  function onVisibilityChange() {
    if (!settings25.store.pauseWhenHidden)
      return;
  ...
  var betterImagine_default = definePlugin({
    ...
    tags: ["ui"],
    settings: settings25,
    _hideDefault: () => settings25.store.hideDefaultPreviews,
    _NullGrid: () => null,
    _autoPlay: () => !settings25.store.noAutoplay,
    _bypassPaywall: () => settings25.store.bypassPaywall,
    _ctrlClickSelect: () => settings25.store.ctrlClickSelect,
    _hoverProps: () => settings25.store.playOnHover ? { onMouseEnter, onMouseLeave } : {},
  ...
  function applyImagineUpgrade() {
    if (settings26.store.hideImagineUpgrade)
  ...
  var logger37 = new Logger("BetterAvatarPlugins");
  var PluginsDialogStore = findByPropsLazy("usePluginsDialogStore");
  var settings28 = definePluginSettings({});
  var OLD_NAME2 = "NoSidebarPlugins";
  var NEW_NAME2 = "BetterAvatarPlugins";
  ...
  function migrateLegacy5() {
    ...
    SettingsStore3.markAsChanged();
    logger37.info("Migrated NoSidebarPlugins into BetterAvatarPlugins");
  }
  var pluginName4 = Object.getOwnPropertyDescriptor(settings28, "pluginName");
  if (pluginName4?.set && pluginName4.get) {
    Object.defineProperty(settings28, "pluginName", {
      configurable: true,
      enumerable: true,
      get: pluginName4.get,
      set(name) {
        if (name === NEW_NAME2)
          migrateLegacy5();
        pluginName4.set.call(settings28, name);
      }
    });
  }
...
`);
  ...
  // src/plugins/customGreeting/clamp.ts
  var ELLIPSIS = "…";
  ...
  var logger38 = new Logger("CustomGreeting");
  ...
  var EDITOR_SEL2 = ".query-bar .tiptap";
  var EMPTY_SEL = `${EDITOR_SEL2} p.is-editor-empty, ${EDITOR_SEL2} p.is-empty:only-child`;
  var ROOT_BEFORE = `${EDITOR_SEL2}::before`;
  var EMPTY_BEFORE = `${EDITOR_SEL2} p.is-editor-empty:first-child::before,${EDITOR_SEL2} p.is-empty:only-child::before`;
...
`, "\\A ");
  ...
  function migrateLegacy6() {
    ...
    SettingsStore3.markAsChanged();
    logger38.info("Migrated Placeholder into CustomGreeting");
  }
  var pluginName5 = Object.getOwnPropertyDescriptor(settings29, "pluginName");
  if (pluginName5?.set && pluginName5.get) {
    Object.defineProperty(settings29, "pluginName", {
      configurable: true,
      enumerable: true,
      get: pluginName5.get,
      set(name) {
        if (name === NEW_NAME3)
          migrateLegacy6();
        pluginName5.set.call(settings29, name);
      }
    });
  }
  ...
  function PhraseListsEditor() {
    ...
    const { phrases, imaginePhrases } = settings29.use(["phrases", "imaginePhrases"]);
    ...
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      onChange: (e) => {
        settings29.store.imaginePhrases = e.target.value;
      },
...
Neon rain in a quiet city`
  ...
  function replaceChatInput() {
    if (isProjectChat())
      return true;
    return settings29.store.heroOnlyOutsideProject === false;
  }
  ...
  function imaginePhrases() {
    try {
      const lines = parsePhrases(settings29.store.imaginePhrases);
      return lines.length ? lines : null;
    } catch {
      return null;
    }
  ...
  function rotateMode() {
    const value = String(settings29.store.mode ?? "refresh");
    if (value === "interval" || value === "manual")
      return value;
    return "refresh";
  }
  function rotateOrder() {
    return settings29.store.order === "random" ? "random" : "sequential";
  }
  function intervalMs() {
    return clamp(Number(settings29.store.intervalSec ?? 10), 1, 3600) * 1000;
  }
  ...
  function pickNextIndex(listLen, advance) {
    if (listLen <= 0)
      return 0;
    const current = Number(settings29.store.greetIndex ?? -1);
    const last = Number(settings29.store.lastRandom ?? -1);
    if (listLen === 1) {
      if (current !== 0)
        settings29.store.greetIndex = 0;
      if (last !== 0)
        settings29.store.lastRandom = 0;
      return 0;
    }
    if (!advance)
    ...
    if (rotateOrder() === "random") {
      ...
      settings29.store.greetIndex = next;
      settings29.store.lastRandom = next;
      return next;
    }
  ...
  function bindSize(p) {
    const editor = p?.closest(EDITOR_SEL2) ?? p;
    if (editor === observed)
      return;
    sizeObs?.disconnect();
    observed = editor;
  ...
  function paintInput() {
    ...
    const css = `${ROOT_BEFORE}{content:none!important}${EMPTY_BEFORE}{content:"${escapeForCssContent(shown)}"!important}`;
    if (css === lastInputCss)
      return;
    lastInputCss = css;
  ...
  var customGreeting_default = definePlugin({
    name: "CustomGreeting",
...
`);
  ...
  var logger39 = new Logger("ExportChat");
...
`);
  ...
  function ExportMenu({ conversationId }) {
    ...
    return /* @__PURE__ */ React.createElement(MenuSub, null, /* @__PURE__ */ React.createElement(MenuSubTrigger, {
      ...
      key: fmt,
      onSelect: () => exportChat(conversationId, fmt).catch((e) => logger39.error("Failed to export chat", e))
    }, label))));
  }
...
`);
  ...
  var cl28 = classNameFactory("void-ci-");
  ...
  function getPresets() {
    return settings30.plain.presets ?? [];
  }
  function setPresets(presets) {
    settings30.store.presets = presets;
  }
  function getAssignments() {
    return settings30.plain.assignments ?? {};
  }
  ...
  function PresetEditor({ preset, onUpdate, onClose }) {
    ...
    return /* @__PURE__ */ React.createElement("div", {
      ...
      className: cl28("textarea-wrap", { "textarea-wrap-error": overLimit })
    }, /* @__PURE__ */ React.createElement(Textarea, {
      className: cl28("textarea"),
      placeholder: "How should Grok behave?",
      value: preset.prompt,
      onChange: (e) => onUpdate({ ...preset, prompt: e.target.value })
    })), /* @__PURE__ */ React.createElement("div", {
      className: cl28("editor-footer")
    }, /* @__PURE__ */ React.createElement(Text2, {
      size: "xs",
      color: overLimit ? undefined : "muted",
      className: overLimit ? cl28("error-text") : undefined
  ...
  function PresetsEditor() {
    const presets = settings30.use(["presets"]).presets ?? [];
    ...
    const deletePreset = useCallback((id) => {
      ...
      settings30.store.assignments = a;
  ...
  function InstructionsMenu({ conversationId }) {
    const presets = settings30.use(["presets"]).presets ?? [];
    const assignments = settings30.use(["assignments"]).assignments ?? {};
...
`);
  ...
  var logger40 = new Logger("BetterSidebar");
  var cl29 = classNameFactory("void-sidebar-");
  ...
  function applyHeaderHover() {
    if (settings31.store.titleRowHover)
  ...
  function startBotsCollapse() {
    stopBotsCollapse();
    if (!settings31.store.botsDefaultCollapsed)
      return;
    let done = false;
    const tick = () => {
      if (done)
  ...
  function resetChatsCollapsedStorage() {
    if (!settings31.store.chatsDefaultExpanded)
      return;
    try {
  ...
  function resetProjectsCollapsedStorage() {
    if (!settings31.store.projectsDefaultCollapsed)
      return;
    try {
  ...
  var ChatsPlus = ErrorBoundary.wrap(function ChatsPlusButton() {
    if (!settings31.use(["chatsPlus"]).chatsPlus)
      return null;
    return /* @__PURE__ */ React.createElement("button", {
      type: "button",
      className: BTN_CLASS,
  ...
  async function deleteConversations(ids) {
    ...
    await Promise.allSettled(ids.map((id) => fetchSoftDeleteConversation(id).catch((e) => logger40.error("Failed to delete", id, e))));
  }
  function SelectCheckbox({ id, route }) {
    const enabled = settings31.use(["batchSelect"]).batchSelect;
  ...
  var betterSidebar_default = definePlugin({
    ...
    _wrapSidebarClick(onClick, id, route) {
      return (e) => {
        if (id && settings31.store.batchSelect && isConversationRoute(route) && (e.ctrlKey || e.metaKey)) {
          e.preventDefault();
          e.stopPropagation();
    ...
    _defaultOpen() {
      return !settings31.store.defaultCollapsed;
    },
    _botsDefaultCollapsed() {
      return settings31.store.botsDefaultCollapsed;
    },
    _chatsCollapsedInit() {
      resetChatsCollapsedStorage();
      return false;
    ...
    _projectsAutoExpand() {
      return !settings31.store.projectsDefaultCollapsed;
    },
    _onSidebarClick() {
      if (!settings31.store.clickToToggle)
        return;
      return (e) => {
  ...
  var logger41 = new Logger("AutoRetry");
  ...
  function isUserInterrupt5(response) {
    ...
    if (state === "interrupted" || state === "stopped" || state === "aborted" || state === "cancelled" || state === "canceled")
      return true;
    return USER_INTERRUPT5.test(interruptBlob(response.error)) || USER_INTERRUPT5.test(String(response.message ?? ""));
  }
  function shouldRetry(response) {
    if (isUserInterrupt5(response))
      return false;
    if (isModeration(response))
      return settings32.store.retryModeration;
    return settings32.store.retryNetwork;
  }
  function retry(responseId, conversationId, response) {
    const count = (retryCounts.get(conversationId) ?? 0) + 1;
    const max = settings32.store.maxRetries;
    if (count > max) {
    ...
    const delaySec = settings32.store.delay;
    showToast(`Retrying... (${count}/${max})`, 0 /* MESSAGE */);
    logger41.info(`Retry ${count}/${max} for ${conversationId} in ${delaySec}s`);
    clearPending();
  ...
  function onStreamEnd8({ responseId }) {
    const response = ResponseStore.useResponseStore.getState().byId[responseId];
...
`);
  ...
  var cl30 = classNameFactory("void-csi-");
  ...
  function resetCrop() {
    settings33.store.cropX = 0.5;
    settings33.store.cropY = 0.5;
    settings33.store.cropZoom = 1;
  }
  function clearAvatar() {
    settings33.store.avatarUrl = "";
    settings33.store.avatarSource = "";
    resetCrop();
  }
  var adoptGen = 0;
  async function adoptSource(src) {
    const gen = ++adoptGen;
    resetCrop();
    settings33.store.avatarSource = src;
    const baked = await bake(src, 0.5, 0.5, 1);
    if (gen !== adoptGen)
      return false;
    if (baked)
      settings33.store.avatarUrl = baked;
    return !!baked;
  }
  ...
  function CropStage({ src }) {
    const { cropX, cropY, cropZoom } = settings33.use(["cropX", "cropY", "cropZoom"]);
    ...
    useEffect(() => {
      ...
      img.src = src;
      setX(num(settings33.store.cropX, 0.5));
      setY(num(settings33.store.cropY, 0.5));
      setZoom(num(settings33.store.cropZoom, 1));
      if (!settings33.store.avatarSource)
        settings33.store.avatarSource = src;
      return () => {
        dead = true;
      };
    ...
    function commit(nx, ny, nz, immediate = false) {
      ...
      const run = () => {
        settings33.store.cropX = next.x;
        settings33.store.cropY = next.y;
        settings33.store.cropZoom = next.z;
        bake(src, next.x, next.y, next.z).then((url) => {
          if (url)
            settings33.store.avatarUrl = url;
        });
      };
      if (bakeTimer.current)
        clearTimeout(bakeTimer.current);
  ...
  function AvatarUrlField() {
    const { avatarUrl, avatarSource } = settings33.use(["avatarUrl", "avatarSource"]);
    ...
    function onUrlChange(value) {
      settings33.store.avatarUrl = value;
  ...
  function avatarSrc() {
    const raw = String(settings33.store.avatarUrl ?? "").trim();
  ...
  function paintMenu() {
    ...
    for (const menu of document.querySelectorAll(MENU)) {
      if (!isAccountMenu(menu))
        continue;
      if (!settings33.store.applyToMenu) {
        dropNames(menu);
        unhide(menu);
  ...
  function applySize() {
    const n = clamp(Math.round(num(settings33.store.avatarSize, SIZE_DEFAULT)), SIZE_MIN, SIZE_MAX);
  ...
  function onMut(muts) {
    ...
    for (const m of muts) {
      ...
      if (!(el instanceof HTMLImageElement))
        continue;
      if (el.closest(FOOTER2) || settings33.store.applyToMenu && el.closest(MENU)) {
        schedule3();
        return;
      }
    }
  ...
  function bind() {
    treeObs2?.disconnect();
    treeObs2 = new MutationObserver(onMut);
...
`);
  ...
  var logger42 = new Logger("InputHistory");
  var cl31 = classNameFactory("void-ih-");
  ...
  function useImagineBucket() {
    return !!settings34.store.separateImagine && isImaginePage3();
  }
  function listOf(raw) {
    return Array.isArray(raw) ? raw.filter((x) => typeof x === "string") : [];
  }
  function getEntries() {
    return listOf(useImagineBucket() ? settings34.plain.imagineEntries : settings34.plain.entries);
  }
  function cap(entries) {
    const max = clamp(settings34.store.maxEntries ?? MAX_DEFAULT, MAX_MIN, MAX_MAX);
    return entries.length > max ? entries.slice(entries.length - max) : entries;
  }
  function setEntries(entries) {
    if (useImagineBucket())
      settings34.store.imagineEntries = entries;
    else
  ...
  function chatEditor(t) {
    if (t instanceof Text)
      return t.parentElement?.closest(EDITOR_SEL3) ?? null;
    if (t instanceof Element)
...
`) : el.innerText ?? el.textContent ?? "";
  ...
  function placeCaret(el, atStart) {
    ...
    try {
      ...
      logger42.debug("placeCaret pm failed:", err);
    }
  ...
  function onSubmit(e) {
    ...
    if (!(form instanceof HTMLFormElement))
      return;
    const editor = form.querySelector(EDITOR_SEL3);
  ...
  function onClick2(e) {
    ...
    if (!submit && !label.includes("send") && !label.includes("submit"))
      return;
    const editor = bar.querySelector(EDITOR_SEL3);
    if (editor instanceof HTMLElement)
      pushEntry(editorText(editor));
  }
  function removeEntry(index, imagine) {
    const list = listOf(imagine ? settings34.plain.imagineEntries : settings34.plain.entries);
    if (index < 0 || index >= list.length)
      return;
    const next = list.filter((_, i) => i !== index);
    if (imagine)
      settings34.store.imagineEntries = next;
    else
      settings34.store.entries = next;
  ...
  function HistoryPanel() {
    ...
    return /* @__PURE__ */ React.createElement(Flex, {
      flexDirection: "column",
      gap: "0.5rem",
      className: cl31("panel")
    }, /* @__PURE__ */ React.createElement(Flex, {
      flexDirection: "column",
      gap: "0.35rem",
      className: cl31("toolbar")
      ...
      className: cl31("search")
    }), /* @__PURE__ */ React.createElement(Flex, {
      className: cl31("meta"),
      alignItems: "center",
      gap: "0.5rem"
    }, /* @__PURE__ */ React.createElement(Paragraph, {
      className: cl31("count")
      ...
      return /* @__PURE__ */ React.createElement("div", {
        ...
        "aria-label": "Copy",
        onClick: () => {
          copyToClipboard(row.text).catch((err) => logger42.error("copy failed:", err));
        }
      }, /* @__PURE__ */ React.createElement(CopyIcon, {
        size: 16
      })), /* @__PURE__ */ React.createElement(ButtonWithTooltip, {
      ...
      onConfirm: () => {
        if (imagine)
          settings34.store.imagineEntries = [];
        else
          settings34.store.entries = [];
        if (imagine === useImagineBucket())
          resetBrowse(0);
        setOpenId(null);
        setQuery("");
  ...
  var inputHistory_default = definePlugin({
    ...
    onSettingsChange() {
      ...
      if (imagineNext.length !== imagine.length)
        settings34.store.imagineEntries = imagineNext;
    },
    zustand: {
      RoutingStore: {
...
`;
  var noGrokBot_default = definePlugin({
    ...
    enabledByDefault: true,
    start() {
      registerStyle(STYLE_NAME9, CSS2);
    },
    stop() {
