...
(() => {
  ...
  function proxyLazy(factory, maxRetries = MAX_RETRIES) {
...
${sourceUrl}`;
  ...
  function makeFinder(name, filterFactory) {
    ...
    const lazy = (...args) => {
      ...
      let scanned = -1;
      return proxyLazy(() => {
        if (getModuleCache().size === scanned)
          return null;
        const found = resolve();
...
`);
  ...
  function placeHost() {
    ...
    host2.style.top = `${span2.top}px`;
    host2.style.height = `${span2.height}px`;
  ...
  function clampMenu() {
    ...
    menu2.style.overflowY = natural > cap2 + 1 ? "auto" : "hidden";
    menu2.style.top = "";
    menu2.style.transform = "";
    const originRect = host2.getBoundingClientRect();
    const mh = menu2.offsetHeight || natural;
