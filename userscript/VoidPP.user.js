...
/**
 * Void++ [20260920.32] v1.0.0 — A modification for grok.com
...
`);
  ...
  function VersionInfo() {
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      as: "span",
      color: "secondary"
    }, "[20260920.32] v1.0.0"), /* @__PURE__ */ React.createElement(Dot, null), /* @__PURE__ */ React.createElement(VersionLink, {
      href: `${"https://github.com/0-V-linuxdo/VoidPP"}/commit/${"3520c7a"}`
    }, `(${"3520c7a"})`)), /* @__PURE__ */ React.createElement(Flex, {
      alignItems: "center",
      gap: "0.25rem"
    }, /* @__PURE__ */ React.createElement(Text2, {
      as: "span",
...
`));
  ...
  function fightHydrate() {
    if (!settings12.store.stickyOnNavigate || applying || userPicking || awaitingMenu || !intent.modeId)
      return;
    const cur = snapshot();
  ...
  function holdQueueEvent(cid, event) {
    ...
    if (type === QUEUE_ADD && diverting) {
      ...
      diverting = null;
      logger20.info("held", id, "for", liveIntent().modeId);
      return true;
    }
  ...
  function flushTurn(cid, turn, parentId) {
    ...
    logger20.info("flushed", turn.id, "as", liveIntent().modeId);
  }
...
`);
  ...
  function stamps() {
    if (cache)
      return cache;
    cache = new Map;
    const raw = settings13.plain.stamps;
  ...
  var messageTimestamps_default = definePlugin({
    ...
    _renderTimestamp: ErrorBoundary.wrap(({ response, isUser }) => {
      ...
      if (settings13.store.hideOwnMessages && human)
        return null;
      ...
      }, formatTimestamp(ms, settings13.store.showDate));
    }),
    patches: [
      {
  ...
  function shouldRetry(response) {
    ...
    if (isModeration(response))
      return settings14.store.retryModeration;
    return settings14.store.retryNetwork;
  }
  function retry(responseId, conversationId, response) {
    const count = (retryCounts.get(conversationId) ?? 0) + 1;
    const max = settings14.store.maxRetries;
    if (count > max) {
  ...
  function apply3() {
    ...
    if (!settings15.store.italic)
      rules.push(`${SEL}{font-style:inherit!important}`);
    if (!settings15.store.quotes) {
...
`);
  ...
  var cl22 = classNameFactory("void-csi-");
...
`) : "";
  ...
  function buildPageShot(snap) {
    const page = node("span", cl24("page"));
    page.dataset.theme = snap.theme;
    for (const line of lastRound(snap.lines)) {
      const el = node("span", cl24("page-line", line.role === "user" && "page-line-user"), line.text);
  ...
  function fillShot(box, id) {
    ...
    if (!snap) {
      const fallback = node("span", cl24("fallback"));
      fallback.append(faviconImg(cl24("favicon")));
      box.append(fallback);
      return;
    }
    const shot = node("span", cl24("shot"));
  ...
  function folderIcon() {
    ...
    svg.setAttribute("class", cl24("folder"));
...
`)) {
  ...
  function renderList(items) {
    if (!host3)
      return;
    const panel = host3.querySelector(`.${cl24("panel")}`);
    if (!panel)
      return;
    let list = panel.querySelector(`.${cl24("list")}`);
    if (!list) {
      panel.replaceChildren();
      list = node("div", cl24("list"));
      panel.append(list);
    }
    list.replaceChildren();
    items.forEach((topic, i) => {
      const btn = node("button", cl24("card"));
      btn.type = "button";
      btn.tabIndex = -1;
      ...
      const shot = node("span", cl24("thumb"));
      shot.setAttribute("aria-hidden", "true");
      fillShot(shot, topic.id);
      const meta = node("span", cl24("meta"));
      meta.append(node("span", cl24("name"), topic.title));
      if (topic.project) {
        const proj = node("span", cl24("host"));
        proj.append(projectIconOf(topic.ws), node("span", cl24("host-name"), topic.project));
        meta.append(proj);
      }
  ...
  function patchList(items) {
    if (!host3)
      return;
    const cards = [...host3.querySelectorAll(`.${cl24("card")}`)];
    ...
    items.forEach((topic, i) => {
      ...
      const name = card.querySelector(`.${cl24("name")}`);
      if (name)
        name.textContent = topic.title;
      const meta = card.querySelector(`.${cl24("meta")}`);
      if (!meta)
        return;
      let row = meta.querySelector(`.${cl24("host")}`);
      if (!topic.project) {
        row?.remove();
        return;
      }
      if (!row) {
        row = node("span", cl24("host"));
        row.append(projectIconOf(topic.ws), node("span", cl24("host-name"), topic.project));
        meta.append(row);
        return;
      }
      const label = row.querySelector(`.${cl24("host-name")}`);
      if (label)
  ...
  function paint2() {
    ...
    if (!items.length) {
      if (paintedIds !== "__empty__") {
        panel.replaceChildren(node("div", cl24("empty"), "Open a few chats, then hold Ctrl+` to switch."));
    ...
    if (paintedIds === "__empty__" || !panel.querySelector(`.${cl24("list")}`)) {
      panel.replaceChildren(node("div", cl24("list")));
      paintedIds = "";
      paintedMeta = "";
    }
    if (paintedIds !== ids) {
  ...
  function getColor(key, fallback) {
    const val = settings20.store[key];
  ...
  function applyColors() {
    ...
    if (settings20.store.enableVisitedColor) {
  ...
  var betterLinks_default = definePlugin({
    ...
    start() {
      settings20.store.linkColor ??= DEFAULT_LINK;
      settings20.store.visitedColor ??= DEFAULT_VISITED;
      applyColors();
...
`);
  ...
  var logger27 = new Logger("ResponseNotification");
  ...
  function getCtx() {
    ...
    try {
      ...
      logger27.debug("AudioContext unavailable:", e);
      audioCtx = null;
      return null;
    }
  }
  ...
  function playUrl(ctx, url) {
    loadBuffer(ctx, url).then((buf) => playBuffer(ctx, buf), (err) => {
      logger27.info("sample play failed:", err);
      if (url !== DEFAULT_CHIME)
        loadBuffer(ctx, DEFAULT_CHIME).then((buf) => playBuffer(ctx, buf), (e) => logger27.info("default chime failed:", e));
    });
  }
  function playSound() {
    if (!userGestured) {
      logger27.info("sound skipped, no user gesture yet");
      return;
    }
    const ctx = getCtx();
    if (!ctx)
    ...
    if (ctx.state === "suspended")
      ctx.resume().then(() => playUrl(ctx, url), () => logger27.info("AudioContext resume failed"));
    else
      playUrl(ctx, url);
  }
  ...
  function notify(responseId, state) {
    logger27.info("notify", responseId, state ?? "unset", "permission", Notification.permission);
  ...
  function onStreamEnd5({ responseId }) {
    logger27.info("streamEnd", responseId);
    if (retryTimer2)
    ...
    const attempt = (retried) => {
      ...
      try {
        ...
        logger27.info("ResponseStore unavailable:", e);
      }
      ...
      if (isErrorResponse2(response)) {
        logger27.info("skip error", responseId);
        return;
      }
      if (!retried) {
      ...
      logger27.info("skip", responseId, response?.state ?? "unset");
    };
    attempt(false);
  }
  ...
  function apply5() {
    const pct = clamp(settings23.store.opacity, 0, 100);
...
`);
  ...
  var logger28 = new Logger("ExportChat");
...
`);
  ...
  function ExportMenu({ conversationId }) {
    ...
    return /* @__PURE__ */ React.createElement(MenuSub, null, /* @__PURE__ */ React.createElement(MenuSubTrigger, {
      ...
      key: fmt,
      onSelect: () => exportChat(conversationId, fmt).catch((e) => logger28.error("Failed to export chat", e))
    }, label))));
  }
...
`);
  ...
  var logger29 = new Logger("UsageDisplay");
  ...
  function storeGet(key) {
    ...
    try {
      ...
      logger29.debug("Failed to read usage stats", error);
      return memory.get(key) ?? null;
    }
  }
  function storeSet(key, value) {
    ...
    try {
      ...
      logger29.debug("Failed to persist usage stats", error);
      memory.set(key, value);
    }
  }
  function storeRemove(key) {
    ...
    try {
      ...
      logger29.debug("Failed to clear usage stats", error);
      memory.delete(key);
    }
  }
  ...
  async function fetchBotUsage() {
    try {
      ...
      logger30.warn("Failed to fetch Grok Bot usage", error);
      return null;
    }
  }
  ...
  function snapshotToday() {
    if (!settings24.store.usageStats)
      return;
    syncAccount();
    if (!state.userId)
      return;
    recordSnapshot(state.userId, state.usage?.weekly.usedPercent ?? null, state.usage?.weekly.resetAt ?? null, retainDaysOf(settings24.store.retainDays));
  }
    ...
    refreshPromise = (async () => {
      try {
        ...
        const remote = await fetchOfficialUsage().then((usage) => ({ ok: true, usage })).catch((error) => {
          logger30.warn("Failed to fetch official usage", error);
          return { ok: false };
        });
        if (currentPoolId() !== poolId)
          return false;
  ...
  function ButtonIcon() {
    ...
    const { showPercent } = settings24.use(["showPercent"]);
  ...
  function UsagePanel() {
    useExternalStore(store3);
    const { usageStats, hoverStatsDelay } = settings24.use(["usageStats", "hoverStatsDelay"]);
  ...
  function StatsToggle() {
    const { usageStats } = settings24.use(["usageStats"]);
    return /* @__PURE__ */ React.createElement(Flex, {
      alignItems: "center",
      justifyContent: "space-between",
      gap: "0.75rem",
      ...
      onCheckedChange: (value) => {
        settings24.store.usageStats = value;
        store3.notify();
        if (value)
          refresh("manual");
      }
  ...
  function RepairReset({ rec, prev, userId }) {
    ...
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      onClick: () => {
        if (pre == null)
          return;
        writeDay(userId, repairWipedReset(rec, dayStart, pre, Date.now()), retainDaysOf(settings24.store.retainDays));
        store3.notify();
      }
    }, "Repair")));
  }
  ...
  var usageDisplay_default = definePlugin({
    ...
    start() {
      ...
      try {
        ...
        logger30.warn("RoutingStore subscribe failed", error);
      }
    },
    stop() {
      unsubRoute?.();
...
`);
  ...
  function currentStyle() {
    const value = settings27.store.style;
...
`;
  ...
  function apply6() {
    ...
    if (settings28.store.hideDictationRefinement)
...
`);
  ...
  var logger32 = new Logger("BetterFiles");
  ...
  async function deleteAssets(ids) {
    ...
    for (const id of ids) {
      ...
      try {
        ...
        logger32.error("Failed to delete asset", id, e);
      }
  ...
  function apply7() {
    const rules = [];
    if (settings29.store.hideShareProject) {
      rules.push('button[aria-label="Share Project"]{display:none!important}');
    }
    if (settings29.store.hideCreateShareLink) {
...
`);
  ...
  var logger34 = new Logger("CompactModeSelect");
  var cl28 = classNameFactory("void-cms-");
  var MODES = [
  ...
  function uncloak() {
    for (const host of ghosts) {
      host.classList.remove(cl28("ghost"));
  ...
  function setOrder(ids) {
    settings30.store.pinOrder = ids.join(",");
  }
  function setPinned(pin, on) {
    settings30.store[pin] = on;
  }
  ...
  async function harvestIcons() {
    ...
    try {
      ...
      logger34.warn("Failed to harvest mode icons:", e);
    } finally {
      setPicking(false);
      harvesting = false;
    }
  ...
  async function selectMode(id) {
    ...
    try {
      ...
      if (!menu) {
        const trigger = nativeTrigger();
        if (!trigger) {
          logger34.warn("Native mode selector not found");
          return;
        }
        clickEl(trigger);
        menu = await waitForMenu();
      ...
      if (!menu) {
        logger34.warn("Native mode item not found:", id);
        return;
      }
      cloak(menu);
      stashGlyphs(menu.items);
      ...
      logger34.warn("Failed to select mode:", e);
    } finally {
      setPicking(false);
    }
  }
  ...
  function PinOrderEditor() {
    const cfg = settings30.use(["pinAuto", "pinFast", "pinExpert", "pinHeavy", "pinBuild", "pinOrder"]);
    ...
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      return /* @__PURE__ */ React.createElement("div", {
        key: m.id,
        role: "listitem",
        className: classes(cl28("order-row"), dragId === m.id && cl28("dragging")),
        onDragOver: preventDragOver,
        onDrop: onDrop(m.id)
      }, /* @__PURE__ */ React.createElement(Flex, {
        alignItems: "center",
        gap: "0.5rem",
        className: cl28("order-main")
      }, /* @__PURE__ */ React.createElement("span", {
        className: cl28("grip"),
        draggable: true,
  ...
  function PinnedModes() {
    const cfg = settings30.use([...SETTING_KEYS]);
    ...
    return /* @__PURE__ */ React.createElement("div", {
      className: classes(cl28("pins"), hideNative && cl28("hide-native"))
    }, items.map((m) => /* @__PURE__ */ React.createElement("span", {
      key: m.id,
      className: cl28("pin-host"),
      ...
      tooltip: m.label,
      onClick: onPin(m.id),
      className: classes(cl28("pin"), selectedModeId === m.id && cl28("on"), showLabels && cl28("labeled"), "hover:bg-button-ghost-hover"),
      "aria-label": m.label
    }))));
  }
  ...
  function ColorRow2() {
    const { starColor } = settings31.use(["starColor"]);
    return /* @__PURE__ */ React.createElement(ColorSettingRow, {
      value: starColor,
      onChange: (v) => {
        settings31.store.starColor = v;
      },
      title: "Star color",
...
`, "\\A ");
  ...
  function PhrasesEditor() {
    const { phrases } = settings32.use(["phrases"]);
    return /* @__PURE__ */ React.createElement(Flex, {
      flexDirection: "column",
      gap: "0.5rem",
      className: cl29("root")
  ...
  function rotateMode() {
    const value = String(settings32.store.mode ?? "refresh");
    if (value === "interval" || value === "manual")
      return value;
    return "refresh";
  }
  function rotateOrder() {
    return settings32.store.order === "random" ? "random" : "sequential";
  }
  function intervalMs() {
    return clamp(Number(settings32.store.intervalSec ?? 10), 1, 3600) * 1000;
  }
  ...
  function pickNextIndex(listLen, advance) {
    if (listLen <= 0)
      return 0;
    const current = Number(settings32.store.greetIndex ?? -1);
    const last = Number(settings32.store.lastRandom ?? -1);
    if (listLen === 1) {
      if (current !== 0)
        settings32.store.greetIndex = 0;
      if (last !== 0)
        settings32.store.lastRandom = 0;
      return 0;
    }
    if (!advance)
    ...
    if (rotateOrder() === "random") {
      ...
      settings32.store.greetIndex = next;
      settings32.store.lastRandom = next;
      return next;
    }
  ...
  // virtual:~plugins
  noTelemetry_default.updatedAt = 1789905284000;
  settings_default.updatedAt = 1789905284000;
  fixChrome_default.updatedAt = 1789905284000;
  fixChrome_default.chrome = true;
  fixChrome_default.hidden = !window.chrome;
  chatBarButtons_default.updatedAt = 1789921507000;
  contextMenu_default.updatedAt = 1789905284000;
  betterNavigator_default.updatedAt = 1789908648000;
  noSidebarIdentity_default.updatedAt = 1789905284000;
  completeToast_default.updatedAt = 1789916728000;
  cleaner_default.updatedAt = 1789905284000;
  betterSidebar_default.updatedAt = 1789905284000;
  betterImagine_default.updatedAt = 1789905284000;
  modeSync_default.updatedAt = 1789905284000;
  messageTimestamps_default.updatedAt = 1789905284000;
  autoRetry_default.updatedAt = 1789906500000;
  userQuotes_default.updatedAt = 1789905284000;
  cloneChats_default.updatedAt = 1789905284000;
  streamerMode_default.updatedAt = 1789905284000;
  inputHistory_default.updatedAt = 1789905284000;
  customSidebarIdentity_default.updatedAt = 1789918488000;
  downloadTTS_default.updatedAt = 1789905284000;
  recentTopics_default.updatedAt = 1789905284000;
  betterLinks_default.updatedAt = 1789905284000;
  experiments_default.updatedAt = 1789905284000;
  customInstructions_default.updatedAt = 1789905284000;
  noBuildStarters_default.updatedAt = 1789905284000;
  responseNotification_default.updatedAt = 1789905284000;
  incognito_default.updatedAt = 1789905284000;
  betterCanvas_default.updatedAt = 1789905284000;
  noSidebarPlugins_default.updatedAt = 1789905284000;
  composerOpacity_default.updatedAt = 1789921507000;
  exportChat_default.updatedAt = 1789905284000;
  autoCollapse_default.updatedAt = 1789905284000;
  usageDisplay_default.updatedAt = 1789905284000;
  widerChat_default.updatedAt = 1789905284000;
  settingsFlyout_default.updatedAt = 1789905284000;
  chatStateFavicons_default.updatedAt = 1789921507000;
  noDictation_default.updatedAt = 1789905284000;
  betterFiles_default.updatedAt = 1789905284000;
  noShareLink_default.updatedAt = 1789905284000;
  chatListStatus_default.updatedAt = 1789906500000;
  stableComposer_default.updatedAt = 1789905284000;
  compactModeSelect_default.updatedAt = 1789905284000;
  consoleJanitor_default.updatedAt = 1789905284000;
  oneko_default.updatedAt = 1789905284000;
  starry_default.updatedAt = 1789905284000;
  pluginsFlyout_default.updatedAt = 1789905284000;
  noGrokBot_default.updatedAt = 1789905284000;
