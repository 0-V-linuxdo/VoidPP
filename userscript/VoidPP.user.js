...
/**
 * Void++ [20260923.1] v1.0.0 — A modification for grok.com
...
`;
  ...
  var WorkspaceStore = findByPropsLazy("useWorkspaceStore");
  var domObs = null;
  var themeObs = null;
    ...
    try {
      ...
      if (plugin.zustand) {
        for (const [storeName, sub] of Object.entries(plugin.zustand)) {
          ...
          const attach = (store2) => {
            if (!sub.selector) {
              unsubs.push(store2.subscribe(wrappedHandler));
              return;
            }
            try {
              const unsub = store2.subscribe(sub.selector, wrappedHandler);
              ...
              if (typeof getState === "function") {
                try {
                  prev = select(getState.call(store2));
                } catch {
                  prev = undefined;
                }
              }
              unsubs.push(store2.subscribe((state) => {
                let cur;
                try {
                  cur = select(state);
                } catch (err) {
...
`);
  ...
  function VersionInfo() {
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      as: "span",
      color: "secondary"
    }, "[20260923.1] v1.0.0"), /* @__PURE__ */ React.createElement(Dot, null), /* @__PURE__ */ React.createElement(VersionLink, {
      href: `${"https://github.com/0-V-linuxdo/VoidPP"}/commit/${"7f646ba"}`
    }, `(${"7f646ba"})`)), /* @__PURE__ */ React.createElement(Flex, {
      alignItems: "center",
      gap: "0.25rem"
    }, /* @__PURE__ */ React.createElement(Text2, {
      as: "span",
...
`;
  var noGrokBot_default = definePlugin({
    ...
    enabledByDefault: true,
    start() {
      registerStyle(STYLE_NAME3, CSS3);
    },
    stop() {
  ...
  var logger17 = new Logger("ResponseNotification");
  ...
  function getCtx() {
    ...
    try {
      ...
      logger17.debug("AudioContext unavailable:", e);
      audioCtx = null;
      return null;
    }
  }
  ...
  function playUrl(ctx, url) {
    loadBuffer(ctx, url).then((buf) => playBuffer(ctx, buf), (err) => {
      logger17.info("sample play failed:", err);
      if (url !== DEFAULT_CHIME)
        loadBuffer(ctx, DEFAULT_CHIME).then((buf) => playBuffer(ctx, buf), (e) => logger17.info("default chime failed:", e));
    });
  }
  function playSound() {
    if (!userGestured) {
      logger17.info("sound skipped, no user gesture yet");
      return;
    }
    const ctx = getCtx();
    if (!ctx)
    ...
    if (ctx.state === "suspended")
      ctx.resume().then(() => playUrl(ctx, url), () => logger17.info("AudioContext resume failed"));
    else
      playUrl(ctx, url);
  }
  ...
  function notify(responseId, state) {
    logger17.info("notify", responseId, state ?? "unset", "permission", Notification.permission);
  ...
  function onStreamEnd({ responseId }) {
    logger17.info("streamEnd", responseId);
    if (retryTimer)
      clearTimeout(retryTimer);
    const attempt = (retried) => {
      ...
      try {
        ...
        logger17.info("ResponseStore unavailable:", e);
      }
      ...
      if (isErrorResponse(response)) {
        logger17.info("skip error", responseId);
        return;
      }
      if (!retried) {
      ...
      logger17.info("skip", responseId, response?.state ?? "unset");
    };
    attempt(false);
  }
  ...
  function syncImagine(current, prev) {
    if (!settings8.store.imagineGeneration || !prev)
      return;
    if (onImaginePage())
      return;
...
`);
  ...
  function storeGet(key) {
    ...
    try {
      ...
      logger18.debug("Failed to read usage stats", error);
      return memory.get(key) ?? null;
    }
  }
  function storeSet(key, value) {
    ...
    try {
      ...
      logger18.debug("Failed to persist usage stats", error);
      memory.set(key, value);
    }
  }
  function storeRemove(key) {
    ...
    try {
      ...
      logger18.debug("Failed to clear usage stats", error);
      memory.delete(key);
    }
  }
  ...
  var logger19 = new Logger("UsageDisplay");
  ...
  async function fetchBotUsage() {
    try {
      ...
      logger19.warn("Failed to fetch Grok Bot usage", error);
      return null;
    }
  }
    ...
    refreshPromise = (async () => {
      try {
        ...
        const remote = await fetchOfficialUsage().then((usage) => ({ ok: true, usage })).catch((error) => {
          logger19.warn("Failed to fetch official usage", error);
          return { ok: false };
        });
        if (currentPoolId() !== poolId)
          return false;
  ...
  var usageDisplay_default = definePlugin({
    ...
    start() {
      ...
      try {
        ...
        logger19.warn("RoutingStore subscribe failed", error);
      }
    },
    stop() {
      unsubRoute?.();
...
`);
  ...
  // voidpp-css:/Users/zhutaiyu/Downloads/Cursor Workspace/Void Fork/src/plugins/completeToast/styles.css
...
`);
  ...
  var logger20 = new Logger("CompleteToast");
  var cl19 = classNameFactory("void-ct-");
  ...
  function isUserInterrupt(r) {
    if (!r)
      return false;
    const state2 = (r.state ?? "").trim().toLowerCase();
    if (state2 === "interrupted" || state2 === "stopped")
      return true;
    return USER_INTERRUPT.test(errorBlob(r.error)) || USER_INTERRUPT.test(String(r.message ?? ""));
  }
  function isLiveResponse2(r) {
    if (!r || isUserInterrupt(r))
      return false;
    if (r.partial)
      return true;
    const state2 = (r.state ?? "").trim().toLowerCase();
    return !!state2 && LIVE_RESP.has(state2);
  }
  ...
  function gatewayOf(cid) {
    try {
      ...
      logger20.debug("MessageStore unavailable:", e);
      return;
    }
  }
  function isLiveCid(cid) {
    ...
    try {
      ...
      if (isLiveResponse2(lastAssistant(cid, byConversationId)))
        return true;
    } catch (e) {
      logger20.debug("ResponseStore live lookup failed:", e);
    }
    return false;
  }
  function isBadFinish(cid, rid) {
    try {
      ...
      if (isUserInterrupt(response) || isErrorResponse2(response))
        return true;
    } catch {}
    const node = lastAssistantNode(gatewayOf(cid));
    if (node && (DEAD_ERR.has(node.status) || isUserInterrupt(node.content) || isErrorResponse2(node.content)))
      return true;
    if (node && USER_INTERRUPT.test(String(node.content?.message ?? "")))
      return true;
    return false;
  }
  function cidOf(responseId) {
    ...
    try {
      ...
      logger20.debug("gateway cid lookup failed:", e);
    }
    try {
      const { byConversationId } = ResponseStore.useResponseStore.getState();
      ...
      logger20.debug("response cid lookup failed:", e);
    }
    return "";
  }
  function convOf(cid) {
    try {
      ...
      logger20.debug("conversation lookup failed:", e);
      return;
    }
  }
  function workspaceOf(cid) {
    try {
      ...
      logger20.debug("workspace lookup failed:", e);
      return "";
    }
  }
  ...
  function previewOf(cid, rid) {
    if (!settings12.store.showPreview)
      return "";
    try {
  ...
  function applyChatPage(id, ws) {
    try {
      ...
      logger20.debug("ChatPageStore update failed:", e);
    }
  }
  function navigateTo(id) {
    ...
    try {
      ...
      logger20.error("Failed to navigate:", e);
      try {
        location.assign(hrefFor(cid, ws));
      } catch (navErr) {
        logger20.error("Fallback navigation failed:", navErr);
      }
    }
  }
  function svgIcon(kind) {
    const svg2 = document.createElementNS(NS, "svg");
    svg2.setAttribute("viewBox", kind === "check" ? "0 0 24 24" : "0 0 15 15");
    svg2.setAttribute("width", kind === "check" ? "16" : "14");
    svg2.setAttribute("height", kind === "check" ? "16" : "14");
  ...
  function shouldPersist() {
    return !!settings12.store.keepUntilDismissed || clamp(settings12.store.duration, 0, DURATION_MAX) <= 0;
  }
  function armTimer() {
    clearTimer();
    if (shouldPersist())
      return;
    const ms = clamp(settings12.store.duration, 0, DURATION_MAX) * 1000;
    hideAt = Date.now() + ms;
    hideTimer = setTimeout(hide, ms);
  }
  function migratePersist() {
    if (settings12.store.duration !== 0 || settings12.store.keepUntilDismissed)
      return;
  ...
  function navigateToImagine(id) {
    try {
      ...
      logger20.error("Failed to navigate to Imagine:", e);
      try {
        location.assign(id ? `/imagine/post/${encodeURIComponent(id)}` : "/imagine");
      } catch (navErr) {
        logger20.error("Fallback Imagine navigation failed:", navErr);
      }
    }
  }
  ...
  function maybeFinishImagine(id) {
    if (!started2 || !settings12.store.imagineGeneration || !id)
      return;
    if (onImaginePage2())
      return;
    ...
    try {
      ...
      logger20.debug("Imagine item unavailable:", e);
      return;
    }
    if (!item)
      return;
    ...
    show(id, "", "imagine", settings12.store.showPreview ? prompt.slice(0, PREVIEW_MAX) : "");
  }
  function syncImagine2(current, prev) {
    if (!started2 || !settings12.store.imagineGeneration)
      return;
    if (!prev)
      return;
    const now = new Set(current ? current.split(",") : []);
  ...
  function maybeFinish(cid, responseId = "") {
    if (!started2 || !isConvId(cid) || onBotPage())
      return;
  ...
  function liveCids() {
    ...
    try {
      ...
      logger20.debug("gateway live scan failed:", e);
    }
    try {
      const { byConversationId } = ResponseStore.useResponseStore.getState();
      for (const [id, list] of Object.entries(byConversationId ?? {})) {
      ...
      logger20.debug("response live scan failed:", e);
    }
    return ids;
  }
  function seedToasted() {
    try {
      ...
      logger20.debug("seed responses failed:", e);
    }
    try {
      ...
      logger20.debug("seed gateway failed:", e);
    }
  }
  function finishClosed() {
    try {
      ...
      logger20.debug("closed scan failed:", e);
    }
  }
  function syncLive() {
    if (!started2)
  ...
  // voidpp-css:/Users/zhutaiyu/Downloads/Cursor Workspace/Void Fork/src/plugins/quoteSticky/styles.css
...
`);
  ...
  var logger21 = new Logger("QuoteSticky");
  ...
  function restore(key) {
    ...
    try {
      const chat = ChatPageStore.useChatPageStore.getState();
      const live2 = String(chat.quotedText || "");
      const same = live2 === snap.text && popupSig(chat.quotePopupData) === popupSig(popupFor(snap));
      const now = performance.now();
      if (!same && now - lastRestoreAt >= RESTORE_GAP_MS) {
        lastRestoreAt = now;
        applyQuote(key, snap.text, popupFor(snap));
        logger21.info("restored", key);
      }
  ...
  function dismiss() {
    ...
    try {
      ...
      logger21.debug("dismiss failed", e);
    } finally {
      applying = false;
    }
    removeFallback();
  ...
  function makePopupWrapper(orig) {
    return function voidQuoteStickyPopup(...args) {
      ...
      const dest = destKey();
      const popup = args[0];
      const live2 = String(ChatPageStore.useChatPageStore.getState().quotedText || "");
      if (popup != null && live2 && !(dest && lastKey && dest !== lastKey)) {
        const key = dest || pathCid() || routeCid() || lastKey;
        if (key) {
          remember(key, live2, popup);
          lastPopup = popup;
          lastText = live2;
          lastKey = key;
        }
  ...
  function unwrapOne(getState, setState, key, label) {
    ...
    try {
      ...
      if (state2[key] === wrappedFns.get(label))
        setState({ [key]: orig });
    } catch {}
  }
  function unwrapAll() {
  ...
  function apply3() {
    ...
    if (!settings13.store.italic)
      rules.push(`${SEL}{font-style:inherit!important}`);
    if (!settings13.store.quotes) {
...
`);
  ...
  function isUserInterrupt2(r) {
    ...
    if (state2 === "interrupted" || state2 === "stopped")
      return true;
    return USER_INTERRUPT2.test(errorBlob2(r.error)) || USER_INTERRUPT2.test(String(r.message ?? ""));
  }
  function isDeadResponse(r) {
    ...
    if (isUserInterrupt2(r))
      return true;
    const state2 = (r.state ?? "").trim().toLowerCase();
    return DEAD_RESPONSE.has(state2) || r.error != null && !LIVE_RESPONSE.has(state2);
  }
  function liveResponse(id, byId) {
    if (!id)
      return false;
    const response = byId[id];
    if (!response || isDeadResponse(response))
      return false;
    if (response.partial)
      return true;
  ...
  function officialInterruptedDom() {
    try {
      ...
      for (const node of document.querySelectorAll("[data-testid='assistant-message']")) {
        if (USER_INTERRUPT2.test(node.textContent ?? ""))
          return true;
      }
      return false;
    } catch (e) {
      logger23.debug("interrupt DOM unavailable:", e);
      return false;
    }
  }
  function currentChatInterrupted() {
    try {
      ...
      logger23.debug("interrupt lookup failed:", e);
    }
  ...
  function hasError() {
    ...
    try {
      ...
      if (!id)
        return false;
      const response = byId[id];
      if (!response || isUserInterrupt2(response))
        return false;
      return response.state === "error" || response.error != null;
    } catch (e) {
  ...
  function onStreamEnd4({ responseId }) {
    try {
      ...
      lastWasError = !!response && !isUserInterrupt2(response) && (response.state === "error" || response.error != null);
    } catch (e) {
      logger23.debug("ResponseStore unavailable:", e);
    }
    scheduleEvaluate();
  ...
  function attachStores() {
    ...
    try {
      ...
      if (typeof routeStore?.subscribe === "function") {
        unsubRoute2 = routeStore.subscribe((s) => s.route.conversationId, (id, prev) => {
    ...
    try {
      ...
      logger23.debug("ResponseStore subscribe failed:", e);
    }
  }
...
`);
  ...
  var logger24 = new Logger("BetterImagine");
  var cl21 = classNameFactory("void-imagine-");
  ...
  function buildFilename(post, isVideo) {
    if (!settings15.store.smartFilenames || !post)
      return null;
  ...
  function persist() {
    if (!settings15.store.persistFilters)
      return;
    try {
...
${p.originalPrompt ?? ""}`.toLowerCase();
  ...
  function filterItems(items) {
    const { hideModerated } = settings15.store;
  ...
  function pauseVideo(video) {
    ...
    if (promise) {
      promise.then(() => {
        ...
        video.pause();
        video.currentTime = 0;
      }).catch((e) => logger24.warn("Failed to pause video:", e));
    } else {
      video.pause();
      video.currentTime = 0;
    }
  ...
  var onMouseEnter = (e) => {
    ...
    if (video)
      pending.set(video, video.play().catch((e2) => logger24.error("Failed to play video", e2)));
  };
  var onMouseLeave = (e) => {
    const video = e.currentTarget.querySelector("video");
    if (video)
...
`));
      ...
      logger24.error(`Failed to copy ${label}s`, e);
    ...
    for (const id of ids) {
      ...
      for (const video of videos) {
        ...
        try {
          ...
          logger24.error("Failed to upscale video:", id, video.id, e);
        }
      }
    }
    if (upscaled)
  ...
  function FilterButtons() {
    ...
    return /* @__PURE__ */ React.createElement(Fragment, null, /* @__PURE__ */ React.createElement(Select, {
      ...
      className: sortActive ? cl21("sort-select", "sort-active") : cl21("sort-select")
      ...
      size: "sm",
      shape: "pill",
      className: currentFilter !== f ? cl21("chip") : undefined,
  ...
  function onVisibilityChange() {
    if (!settings15.store.pauseWhenHidden)
      return;
  ...
  var betterImagine_default = definePlugin({
    ...
    tags: ["ui"],
    settings: settings15,
    _hideDefault: () => settings15.store.hideDefaultPreviews,
    _NullGrid: () => null,
    _autoPlay: () => !settings15.store.noAutoplay,
    _bypassPaywall: () => settings15.store.bypassPaywall,
    _ctrlClickSelect: () => settings15.store.ctrlClickSelect,
    _hoverProps: () => settings15.store.playOnHover ? { onMouseEnter, onMouseLeave } : {},
...
`);
  ...
  var logger25 = new Logger("BetterFiles");
  ...
  async function deleteAssets(ids) {
    ...
    for (const id of ids) {
      ...
      try {
        ...
        logger25.error("Failed to delete asset", id, e);
      }
...
`);
  ...
  var logger26 = new Logger("CloneChats");
  ...
  function CloneItem({ conversationId }) {
    const streaming = useIsStreaming(conversationId);
    return /* @__PURE__ */ React.createElement(MenuItem, {
      onSelect: () => cloneChat(conversationId).catch((e) => logger26.error("Failed to clone chat:", e)),
      disabled: streaming
    }, /* @__PURE__ */ React.createElement(CopyIcon, {
      size: 16,
...
`);
  ...
  var HOST2 = '[data-sidebar="menu-button"], [data-sidebar="menu-sub-button"]';
  ...
  function isUserInterrupt3(r) {
    ...
    if (state2 === "interrupted" || state2 === "stopped")
      return true;
    return USER_INTERRUPT3.test(errorBlob3(r.error)) || USER_INTERRUPT3.test(String(r.message ?? ""));
  }
  function isDeadResponse2(r) {
    ...
    if (isUserInterrupt3(r))
      return true;
    const state2 = (r.state ?? "").trim().toLowerCase();
    return DEAD.has(state2) || r.error != null && !LIVE.has(state2);
  }
  function isLiveResponse3(r) {
    ...
    if (!state2)
      return false;
    return LIVE.has(state2);
  }
  function isErrorResponse3(r) {
  ...
  function collectConvIds(value, out, depth = 0) {
    ...
    for (const [key, child] of Object.entries(rec)) {
      ...
      if (SKIP_KEY.test(key))
        continue;
      if (isConvId2(key) && isLiveBag(child))
        out.add(key);
  ...
  function looksExtraStore(name, state2) {
    ...
    if (keys2.some((key) => EXTRA_HINT.test(key)))
      return true;
    let n = 0;
  ...
  function pageLooksLive(page, byId) {
    if (isLiveResponse3(byId[page.streamedMessageId ?? ""]) || isLiveResponse3(byId[page.lastMessageId ?? ""]) || isLiveResponse3(byId[page.sidePanelResponseId ?? ""]))
      return true;
    if (!page.showStreamingIndicator)
      return false;
  ...
  function currentChatInterrupted2() {
    try {
      ...
      if (isUserInterrupt3(byId[page.streamedMessageId ?? ""]) || isUserInterrupt3(byId[page.lastMessageId ?? ""]))
        return true;
      for (const cid of currentIds2()) {
        if (isUserInterrupt3(lastAssistant2(cid, byConversationId)))
          return true;
      }
    } catch {}
  ...
  function liveIds() {
    ...
    try {
      ...
      for (const id of Object.keys(inflightPromisesByConversationId ?? {})) {
        ...
        if (last && !isLiveResponse3(last))
          continue;
        ids.add(id);
      }
      for (const [id, list] of Object.entries(byConversationId ?? {})) {
        if (list?.some(isLiveResponse3))
          ids.add(id);
      }
    } catch (e) {
  ...
  function interruptOf(id) {
    try {
      ...
      logger27.debug("interrupt lookup failed:", e);
    }
    return false;
  }
  function refreshMarks() {
    const live2 = liveIds();
    const opened = new Set(currentIds2());
    for (const id of live2)
      marks.set(id, "streaming");
    for (const [id, kind2] of marks) {
      let next = kind2;
      if (kind2 === "streaming" && !live2.has(id)) {
  ...
  function onStreamEnd5({ responseId }) {
    ...
    try {
      ...
      logger27.debug("streamEnd lookup failed:", e);
    }
    if (liveIds().has(cid)) {
      schedule();
      return;
    }
    if (currentIds2().includes(cid) || isUserInterrupt3(response)) {
      marks.delete(cid);
      schedule();
      return;
    }
    marks.set(cid, isErrorResponse3(response) ? "error" : "done");
    schedule();
  }
  function hrefOf(el) {
  ...
  function rowHost(el, root) {
    ...
    if (!hrefId(el))
      return null;
    const wrapped = el.closest(HOST2);
  ...
  function sweepPrimaryMarks() {
    for (const mark of document.querySelectorAll(`.${MARK}`)) {
      const host2 = mark.parentElement;
      if (host2 && isPrimaryNav(host2))
        clearMark(host2);
    }
  }
  function paint() {
    ...
    const live2 = [...marks].filter(([, kind2]) => kind2 === "streaming").map(([id]) => id);
    if (live2.length && !rowById.size)
      logger27.info("live ids with no rows", live2);
  }
  function schedule() {
    if (!started4 || raf2)
      return;
  ...
  var logger28 = new Logger("AutoRetry");
  ...
  function isUserInterrupt4(response) {
    const state2 = String(response.state ?? "").trim().toLowerCase();
    if (state2 === "interrupted" || state2 === "stopped" || state2 === "aborted" || state2 === "cancelled" || state2 === "canceled")
      return true;
    return USER_INTERRUPT4.test(interruptBlob(response.error)) || USER_INTERRUPT4.test(String(response.message ?? ""));
  }
  function shouldRetry(response) {
    if (isUserInterrupt4(response))
      return false;
    if (isModeration(response))
      return settings18.store.retryModeration;
    return settings18.store.retryNetwork;
  }
  function retry(responseId, conversationId, response) {
    const count = (retryCounts.get(conversationId) ?? 0) + 1;
    const max = settings18.store.maxRetries;
    if (count > max) {
    ...
    const delaySec = settings18.store.delay;
    showToast(`Retrying... (${count}/${max})`, 0 /* MESSAGE */);
    logger28.info(`Retry ${count}/${max} for ${conversationId} in ${delaySec}s`);
    clearPending();
...
`);
  ...
  var cloakWatch = null;
  function uncloak() {
    for (const host2 of ghosts) {
      host2.classList.remove(cl22("ghost"));
      host2.style.removeProperty("opacity");
      host2.style.removeProperty("visibility");
      host2.style.removeProperty("pointer-events");
    }
    ghosts.clear();
  }
  ...
  function setOrder(ids) {
    settings19.store.pinOrder = ids.join(",");
  }
  function setPinned(pin, on) {
    settings19.store[pin] = on;
  }
  ...
  function cloak(menu) {
    const host2 = ghostHost(menu.root);
    if (ghosts.has(host2))
      return;
    host2.classList.add(cl22("ghost"));
    host2.style.setProperty("opacity", GHOST_STYLE.opacity, "important");
    host2.style.setProperty("visibility", GHOST_STYLE.visibility, "important");
    ghosts.add(host2);
  }
  function lockGhosts() {
    document.documentElement.classList.add("void-cms-picked");
    for (const host2 of ghosts)
      host2.style.setProperty("pointer-events", "none", "important");
  }
  function modeMenu() {
  ...
  function PinOrderEditor() {
    const cfg = settings19.use(["pinAuto", "pinFast", "pinExpert", "pinHeavy", "pinBuild", "pinOrder"]);
  ...
  function PinnedModes() {
    const cfg = settings19.use([...SETTING_KEYS]);
...
`);
  ...
  var logger30 = new Logger("InputHistory");
...
`) : el.innerText ?? el.textContent ?? "";
  ...
  function placeCaret(el, atStart) {
    ...
    try {
      ...
      logger30.debug("placeCaret pm failed:", err);
    }
  ...
  function setEditorText(el, text, atStart) {
    ...
    try {
      ...
      logger30.debug("insertText failed:", err);
    }
    placeCaret(el, atStart);
    scheduleApplyEnd(gen);
  }
  function hudEl() {
    let el = document.querySelector(`.${cl23("hud")}`);
    if (el)
      return el;
    el = document.createElement("div");
    el.className = cl23("hud");
    el.setAttribute("aria-live", "polite");
    document.body.appendChild(el);
    return el;
  }
  function hideHud() {
    document.querySelector(`.${cl23("hud")}`)?.classList.remove(cl23("hud-on"));
  }
  ...
  function HistoryPanel() {
    ...
    return /* @__PURE__ */ React.createElement(Flex, {
      ...
      className: cl23("search")
    }), /* @__PURE__ */ React.createElement(Flex, {
      className: cl23("meta"),
      alignItems: "center",
      gap: "0.5rem"
    }, /* @__PURE__ */ React.createElement(Paragraph, {
      className: cl23("count")
      ...
      return /* @__PURE__ */ React.createElement("div", {
        key: row.index,
        className: cl23("item", expanded && "item-on")
      }, /* @__PURE__ */ React.createElement("span", {
        className: cl23("index")
        ...
        className: cl23("body", !expanded && "clamp")
      }, row.text)), /* @__PURE__ */ React.createElement("div", {
        className: cl23("actions")
      }, /* @__PURE__ */ React.createElement(ButtonWithTooltip, {
        variant: "tertiary",
        size: "sm",
        shape: "square",
        ...
        "aria-label": "Copy",
        onClick: () => {
          copyToClipboard(row.text).catch((err) => logger30.error("copy failed:", err));
        }
      }, /* @__PURE__ */ React.createElement(CopyIcon, {
        size: 16
      })), /* @__PURE__ */ React.createElement(ButtonWithTooltip, {
  ...
  // voidpp-css:/Users/zhutaiyu/Downloads/Cursor Workspace/Void Fork/src/plugins/customSidebarIdentity/styles.css
...
`);
  ...
  var cl24 = classNameFactory("void-csi-");
  ...
  function CropStage({ src }) {
    ...
    function applyPos(nx, ny, nz) {
      ...
      const r2 = cropRect(nat.w, nat.h, nz, nx * nat.w, ny * nat.h);
      const cx = r2.x / nat.w;
      const cy = r2.y / nat.h;
      setX(cx);
      setY(cy);
      setZoom(r2.z);
      pos.current = { x: cx, y: cy, zoom: r2.z };
      return { x: cx, y: cy, z: r2.z };
    }
  ...
  function ensureName(host2, text) {
    let el = host2.querySelector(`:scope > .${NAME_CLASS}`);
    if (!el) {
  ...
  function unhide(scope) {
    for (const el of scope.querySelectorAll(`.${HIDE_CLASS}`))
      el.classList.remove(HIDE_CLASS);
  }
  function hideOfficial(host2, keep) {
    if (host2.getAttribute("role") === "menu")
      return;
    for (const node of host2.querySelectorAll("span, p")) {
  ...
  function syncName(host2, text, scope) {
    if (!host2 || !text) {
      dropNames(scope);
      unhide(scope);
      return;
    }
    const el = ensureName(host2, text);
  ...
  function paintMenu() {
    ...
    for (const menu of document.querySelectorAll(MENU)) {
      ...
      if (!settings21.store.applyToMenu) {
        dropNames(menu);
        unhide(menu);
        for (const img2 of menu.querySelectorAll(`img[${MARK2}]`))
          restoreImg(img2);
        continue;
      }
      for (const img2 of pfps(menu, menu))
        paintImg(img2, url);
      if (!name) {
        dropNames(menu);
        unhide(menu);
      ...
      if (!img)
        continue;
      const host2 = menuNameHost(img, menu);
      const el = ensureName(host2, name);
      hideOfficial(host2, el);
  ...
  function onMut(muts) {
    ...
    for (const m of muts) {
      ...
      if (!(el instanceof HTMLImageElement))
        continue;
      if (el.closest(FOOTER2) || settings21.store.applyToMenu && el.closest(MENU)) {
        schedule2();
        return;
      }
    }
...
`);
  ...
  var cl25 = classNameFactory("void-download-tts-");
  var logger31 = new Logger("DownloadTTS");
  ...
  function DownloadButton() {
    const [loading, onClick2] = useAsyncAction(async () => {
      try {
        ...
        logger31.error("Failed to download TTS audio:", e);
      }
    });
    return /* @__PURE__ */ React.createElement(Button, {
...
`);
  ...
  function syncClasses() {
    ...
    for (const [key, cls] of Object.entries(CSS_CLASSES)) {
      classList.toggle(cls, !!settings22.store[key]);
    }
  }
  ...
  // voidpp-css:/Users/zhutaiyu/Downloads/Cursor Workspace/Void Fork/src/plugins/modeSync/styles.css
...
`);
  ...
  var logger32 = new Logger("ModeSync");
  ...
  function coerceModelMode(existing, live2) {
    const raw = live2.modeId || live2.modelMode;
  ...
  function setIntent(next) {
    ...
    const host2 = pageWindow;
    if (intent.modeId)
      host2[REMEMBERED] = intent;
    else
      delete host2[REMEMBERED];
  }
  ...
  function syncRestoreFlag() {
    if (!settings23.store.stickyOnNavigate) {
  ...
  function applyIntent(next) {
    if (!next.modeId || applying3 || onImaginePage4())
      return;
  ...
  function rememberMode(modeId) {
    ...
    awaitingMenu = false;
    applyIntent(intent);
    logger32.info("intent", intent.modeId);
  }
  ...
  function fightHydrate() {
    if (sendOverride || !settings23.store.stickyOnNavigate || applying3 || userPicking || awaitingMenu || !intent.modeId)
      return;
    if (!loadPending())
      return;
    const cur = snapshot();
    ...
    if (modeSlug(cur.modeId) === slug && (!cid || sessionAdjusted(cid) === slug) && (!intent.modelMode || modeSlug(cur.modelMode) === slug) && (!intent.activeModelId || cur.activeModelId === intent.activeModelId))
      return;
    logger32.info("hydrate fought", cur.modeId, "->", intent.modeId);
    applyIntent(intent);
  }
  function navKey() {
    try {
      ...
      let routeCid2 = "";
      try {
        routeCid2 = String(RoutingStore.useRoutingStore.getState().route.conversationId ?? "");
      } catch {
        routeCid2 = "";
      }
      return `${chat.conversationId ?? ""}|${chat.optimisticConversationId ?? ""}|${chat.projectId ?? ""}|${routeCid2}`;
    } catch {
      return "";
    }
  }
  function onNavigate() {
    ...
    closeMenu();
    schedulePaint();
    if (!settings23.store.stickyOnNavigate || !intent.modeId)
      return;
    setRestoreFlag(true);
    applyIntent(intent);
    syncRestoreFlag();
  ...
  function patchPayload(raw, live2) {
    if (onImaginePage4() || !raw || typeof raw !== "object" || Array.isArray(raw) || !live2.modeId)
      return false;
    const rec = raw;
    if (!isChatSend(rec))
      return false;
    ...
    rec.modeId = slug;
    rec.modelMode = coerceModelMode(rec.modelMode, live2);
    if ("model" in rec)
      rec.model = slug;
    return rec.modeId !== before || rec.modelMode !== beforeMode || "model" in rec && rec.model !== beforeModel;
  }
  function patchSendArgs(args, live2) {
    const first = args[0];
    if (!first || typeof first !== "object")
      return;
    patchPayload(first, live2);
  }
  function rewriteJsonBody(text, live2) {
    let parsed;
    try {
  ...
  function holdQueueEvent(cid, event) {
    ...
    if (!id)
      return false;
    if (type === QUEUE_ADD) {
      const saved3 = pendingEnqueue?.intent ?? (intent.modeId ? intent : liveIntent());
      if (saved3.modeId)
        itemIntent.set(id, { ...saved3 });
      const body = (pendingEnqueue?.args.text || eventText(event)).trim();
      if (body)
        itemBody.set(id, body);
      schedulePaint();
      if (diverting) {
        mapGetOrCreate(held, cid, () => []).push({ id, args: diverting, intent: { ...saved3 } });
        diverting = null;
        logger32.info("held", id, "for", saved3.modeId);
        return true;
      }
      return false;
    }
  ...
  function writeMode(rec, live2) {
    ...
    for (const key of GW_MODE_KEYS) {
      rec[key] = key === "modelMode" || key === "model_mode" ? coerceModelMode(rec[key], live2) : slug;
    }
    if ("model" in rec)
      rec.model = slug;
  }
  function patchGwEvent(event, live2) {
    if (onImaginePage4() || !event || typeof event !== "object" || Array.isArray(event) || !live2.modeId)
      return;
    const rec = event;
  ...
  function intentForText(cid, text) {
    ...
    if (conv) {
      for (const q of conv.queue) {
        const id = qid(q);
        const saved2 = id ? itemIntent.get(id) : undefined;
        if (saved2?.modeId && bodyOfItem(conv, id) === body)
          return saved2;
      }
    }
    for (const [id, saved2] of itemIntent) {
      if (saved2.modeId && itemBody.get(id) === body)
        return saved2;
    }
  ...
  function intentFromPayload(raw) {
    ...
    const cid = typeof rec.conversationId === "string" ? rec.conversationId : typeof rec.convId === "string" ? rec.convId : currentCid();
  ...
  function wrapGatewaySend() {
    try {
      ...
      const wrapped = function voidModeSyncGwSend(...args) {
        ...
        if (type === QUEUE_ADD) {
          const saved2 = (typeof cid === "string" ? itemIntent.get(eventQueueId(event)) : undefined) ?? pendingEnqueue?.intent;
          if (saved2?.modeId)
            patchGwEvent(event, saved2);
          return orig.apply(mgr, args);
        }
      ...
      logger32.debug("gateway wrap failed", e);
    }
  }
  function unwrapGatewaySend() {
    try {
      ...
      logger32.debug("gateway unwrap failed", e);
    }
    origGwSend = null;
    wrappedGwSend = null;
    gwHost = null;
  ...
  function unwrapStore(getState, setState, keys3, prefix) {
    let state2;
    try {
      state2 = getState();
    } catch {
    ...
    for (const key of keys3) {
      ...
      if (orig && state2[key] === wrappedFns2.get(label))
        next[key] = orig;
    }
  ...
  function rewriteIfChatPost(url, method, text) {
    ...
    if (!CHAT_POST.test(url) || STOP_URL.test(url) || text == null)
      return null;
    const live2 = intentForBody(text);
    if (!live2.modeId)
      return null;
    const next = rewriteJsonBody(text, live2);
    if (!next || next === text)
      return null;
    applyIntent(live2);
    logger32.info("rewrite", live2.modeId, url.replace(/^https?:\/\/[^/]+/, ""));
    return next;
  }
  ...
  function hookFetch() {
    ...
    pageWindow.fetch = function voidModeSyncFetch(input, init) {
      try {
        ...
        logger32.debug("fetch patch failed", e);
      }
  ...
  function hookXhr() {
    ...
    XHR.prototype.open = function voidModeSyncOpen(method, url, ...rest) {
      try {
        ...
        logger32.debug("xhr open failed", e);
      }
  ...
  function paintGlyph(host2, modeId) {
    ...
    host2.innerHTML = ICONS[modeId] || ICONS.fast;
  }
  function closeMenu() {
    menu?.remove();
    menu = null;
  ...
  function setItemMode(id, modeId) {
    ...
    logger32.info("queue item", id, "->", next.modeId);
    schedulePaint();
  }
  function openMenu(chip, id) {
    ...
    for (const choice of modeChoices()) {
      ...
      span2.textContent = choice.label;
      opt.append(span2);
  ...
  function rowBody(row) {
    const clamp2 = row.querySelector(".line-clamp-2")?.textContent?.trim();
    if (clamp2)
      return clamp2;
  ...
  function mountChip(row, id) {
    if (!itemIntent.has(id)) {
      const saved2 = pendingEnqueue?.intent?.modeId ? pendingEnqueue.intent : intent.modeId ? intent : undefined;
      if (saved2?.modeId)
        itemIntent.set(id, { ...saved2 });
    }
  ...
  function paint2() {
    paintRaf = 0;
    if (!settings23.store.showQueueMode || onImaginePage4()) {
      unpaint();
      return;
    }
    const card = trayCard();
  ...
  var modeSync_default = definePlugin({
    ...
    start() {
      ...
      try {
        ...
        logger32.warn("Failed to hook send path", e);
      }
      if (intent.modeId)
        applyIntent(intent);
    },
...
`);
  ...
  var cl26 = classNameFactory("void-ci-");
  ...
  function PresetEditor({ preset, onUpdate, onClose }) {
    ...
    return /* @__PURE__ */ React.createElement("div", {
      ...
      className: cl26("textarea-wrap", { "textarea-wrap-error": overLimit })
    }, /* @__PURE__ */ React.createElement(Textarea, {
      ...
      className: cl26("editor-footer")
    }, /* @__PURE__ */ React.createElement(Text2, {
      size: "xs",
      color: overLimit ? undefined : "muted",
      className: overLimit ? cl26("error-text") : undefined
...
`));
  ...
  // voidpp-css:/Users/zhutaiyu/Downloads/Cursor Workspace/Void Fork/src/plugins/quoteJump/styles.css
...
`);
  ...
  var logger33 = new Logger("QuoteJump");
  ...
  function idsFrom(el, extra) {
    const out = [];
    if (el) {
      const host2 = el.closest("[id^='response-']");
      if (host2) {
        const m = host2.id.match(UUID);
        if (m)
          out.push(m[0]);
      }
  ...
  function chatPane() {
    ...
    if (msg) {
      const col = msg.closest(OVERFLOW_SEL);
      if (col && !skip(col))
        return col;
    }
    let best = null;
    let bestScore = 0;
    for (const n of main.querySelectorAll(OVERFLOW_SEL)) {
      if (skip(n))
        continue;
  ...
  function paneOf(el) {
    ...
    for (let n = el.parentElement;n && n !== document.documentElement; n = n.parentElement) {
      if (n.closest(PANE_SKIP))
        continue;
      if (n.closest("pre, code, table, details") && !n.querySelector(MSG2))
        continue;
      if (n.matches(OVERFLOW_SEL))
        return n;
    }
    return null;
  }
  ...
  function storeNeedle(needle) {
    ...
    try {
      ...
      logger33.debug("store search failed", e);
    }
    return null;
  }
  ...
  function composerChip(el) {
    ...
    while (n && n !== bar) {
      ...
      if (n.offsetHeight > 0 && n.offsetHeight <= 72 && nodeHasNeedle(n, needle)) {
        ...
        if (action && isBarAction(action) && !n.contains(el.closest("button, [role='button']") ?? el))
          return null;
        return n;
      }
      n = n.parentElement;
  ...
  function hiddenHost(el, allowThink) {
    if (el.closest("button, svg, [role='toolbar']"))
      return true;
    if (!allowThink && el.closest(THINK_SEL) && !el.closest("summary"))
      return true;
    const d = el.closest("details");
    if (d instanceof HTMLDetailsElement && !d.open && !el.closest("summary"))
      return true;
    try {
  ...
  function rawIndexForNorm(raw, normIdx) {
    let i = 0;
    let n = 0;
  ...
  function collectParts(root, allowThink) {
    ...
    while (node = walker.nextNode()) {
      ...
      if (!el || hiddenHost(el, allowThink))
        continue;
      if (blob)
        blob += " ";
  ...
  function clearHighlight() {
    ...
    flashTimer = 0;
    flashing?.classList.remove(cl27("hit"));
    flashing = null;
  ...
  function visibleMidY(pane) {
    const top = pane?.getBoundingClientRect().top ?? 0;
    const bar = document.querySelector(QUERY2);
  ...
  async function hydrate(cid) {
    ...
    try {
      ...
      logger33.debug("loadResponses failed", e);
    }
    try {
      await ResponseStore.useResponseStore.getState().loadMoreResponses?.(cid);
    } catch (e) {
      logger33.debug("loadMoreResponses failed", e);
    }
  }
  function resolveNeedle(origin) {
    const live2 = quotedText();
    if (origin) {
      ...
      const text = sent || live2 || prefixOf(origin.textContent || "");
      const ids = [parent, ...idsFrom(origin, row)].filter(Boolean);
      return { needle: text, ids };
    }
    return { needle: live2, ids: idsFrom(null) };
  }
  ...
  async function jump(origin) {
    ...
    if (!el || !findHit(el, needle) && !nodeHasNeedle(el, needle)) {
      const hit2 = storeNeedle(needle);
      if (hit2) {
        if (hit2.id)
          ids.unshift(hit2.id);
        await hydrate(hit2.cid || conversationId());
        if (mine !== gen)
          return;
    ...
    if (mine !== gen)
      return;
    if (!el) {
      logger33.debug("no source message");
      return;
    }
  ...
  function onClick2(e) {
    if (!e.isTrusted || e.button !== 0 || onImaginePage5())
      return;
    const t = e.target;
    if (!(t instanceof Element))
      return;
  ...
  var quoteJump_default = definePlugin({
    ...
    start() {
      ...
      document.addEventListener("click", onClick2, { capture: true, signal: abort3.signal });
    },
    stop() {
      abort3?.abort();
      abort3 = null;
...
`);
  ...
  var logger34 = new Logger("BetterSidebar");
  var cl28 = classNameFactory("void-sidebar-");
  ...
  function applyHeaderHover() {
    if (settings26.store.titleRowHover)
  ...
  function startBotsCollapse() {
    stopBotsCollapse();
    if (!settings26.store.botsDefaultCollapsed)
      return;
    let done = false;
    const tick = () => {
      if (done)
  ...
  function resetChatsCollapsedStorage() {
    if (!settings26.store.chatsDefaultExpanded)
      return;
    try {
  ...
  function resetProjectsCollapsedStorage() {
    if (!settings26.store.projectsDefaultCollapsed)
      return;
    try {
  ...
  var ChatsPlus = ErrorBoundary.wrap(function ChatsPlusButton() {
    if (!settings26.use(["chatsPlus"]).chatsPlus)
      return null;
    return /* @__PURE__ */ React.createElement("button", {
      type: "button",
      className: BTN_CLASS,
  ...
  async function deleteConversations(ids) {
    ...
    await Promise.allSettled(ids.map((id) => fetchSoftDeleteConversation(id).catch((e) => logger34.error("Failed to delete", id, e))));
  }
  function SelectCheckbox({ id, route }) {
    const enabled = settings26.use(["batchSelect"]).batchSelect;
  ...
  var betterSidebar_default = definePlugin({
    ...
    _wrapSidebarClick(onClick3, id, route) {
      return (e) => {
        if (id && settings26.store.batchSelect && isConversationRoute(route) && (e.ctrlKey || e.metaKey)) {
          e.preventDefault();
          e.stopPropagation();
    ...
    _defaultOpen() {
      return !settings26.store.defaultCollapsed;
    },
    _botsDefaultCollapsed() {
      return settings26.store.botsDefaultCollapsed;
    },
    _chatsCollapsedInit() {
      resetChatsCollapsedStorage();
      return false;
    ...
    _projectsAutoExpand() {
      return !settings26.store.projectsDefaultCollapsed;
    },
    _onSidebarClick() {
      if (!settings26.store.clickToToggle)
        return;
      return (e) => {
...
`);
  ...
  var cl29 = classNameFactory("void-ph-");
...
`, "\\A ");
  ...
  function ImaginePhrasesEditor() {
    const { imaginePhrases } = settings27.use(["imaginePhrases"]);
    return /* @__PURE__ */ React.createElement(Flex, {
      flexDirection: "column",
      gap: "0.5rem",
      className: cl29("root")
      ...
      onChange: (e) => {
        settings27.store.imaginePhrases = e.target.value;
      },
...
Neon rain in a quiet city`
  ...
  function imaginePhrases() {
    try {
      const lines = parsePhrases(settings27.store.imaginePhrases);
      return lines.length ? lines : null;
    } catch {
      return null;
    }
  ...
  function rotateMode() {
    const value = String(settings27.store.mode ?? "refresh");
    if (value === "interval" || value === "manual")
      return value;
    return "refresh";
  }
  function rotateOrder() {
    return settings27.store.order === "random" ? "random" : "sequential";
  }
  function intervalMs() {
    return clamp(Number(settings27.store.intervalSec ?? 10), 1, 3600) * 1000;
  }
  ...
  function pickNextIndex(listLen, advance) {
    if (listLen <= 0)
      return 0;
    const current = Number(settings27.store.greetIndex ?? -1);
    const last = Number(settings27.store.lastRandom ?? -1);
    if (listLen === 1) {
      if (current !== 0)
        settings27.store.greetIndex = 0;
      if (last !== 0)
        settings27.store.lastRandom = 0;
      return 0;
    }
    if (!advance)
      return current >= 0 && current < listLen ? current : 0;
    if (rotateOrder() === "random") {
      const prev2 = current >= 0 && current < listLen ? current : last;
      let next2 = Math.floor(Math.random() * listLen);
      let guard = 0;
      while (next2 === prev2 && guard++ < 10)
        next2 = Math.floor(Math.random() * listLen);
      settings27.store.greetIndex = next2;
      settings27.store.lastRandom = next2;
      return next2;
    }
    const prev = current >= -1 && current < listLen ? current : -1;
    const next = (prev + 1) % listLen;
    settings27.store.greetIndex = next;
    return next;
  }
...
`);
  ...
  var logger35 = new Logger("MessageTimestamps");
  var STAMP_MAX = 5000;
  ...
  function stamps() {
    if (cache)
      return cache;
    cache = new Map;
    const raw = settings28.plain.stamps;
    if (raw && typeof raw === "object") {
  ...
  function persistNow() {
    ...
    for (const [id, ms] of stamps())
      next[id] = ms;
    settings28.store.stamps = next;
  }
  var persist2 = debounce(persistNow, 400);
  function remember2(id, ms, sender2, state2, force = false) {
    if (!id)
      return false;
    const map = stamps();
  ...
  function conversationIdOf(id, rec) {
    ...
    try {
      ...
      logger35.debug("conversation id lookup failed", e);
    }
    return "";
  }
  function userKeys(rec, id) {
    ...
    if (typeof parentResponseId === "string" && parentResponseId)
      keys3.push(`u:p:${parentResponseId}`);
    ...
    if (fp) {
      ...
      keys3.push(cid ? `u:t:${cid}:${fp}` : `u:t:${fp}`);
    }
    return keys3;
  }
  function gatewayRecords(cid) {
    ...
    try {
      ...
      logger35.debug("message store unavailable", e);
    }
    return out;
  }
  ...
  function extraKeys(rec, id) {
    ...
    try {
      ...
      for (const [cid, list] of Object.entries(byConversationId ?? {})) {
        ...
        if (index < 0)
          continue;
        keys3.push(`h:${cid}:${index}`);
        break;
      }
    } catch (e) {
      logger35.debug("stable key lookup failed", e);
    }
    return keys3;
  }
  function storedMs(id, rec, user) {
    const map = stamps();
    const keys3 = user ? userKeys(rec, id) : [id, ...extraKeys(rec, id)];
    for (const key of keys3) {
  ...
  function rememberKeys(id, rec, ms, sender2, user, force = false) {
    ...
    if (user) {
      for (const key of userKeys(rec, id)) {
        if (remember2(key, ms, "human", state2, force))
          changed = true;
      }
      return changed;
    }
    if (remember2(id, ms, sender2, state2, force))
      changed = true;
    for (const key of extraKeys(rec, id)) {
      if (remember2(key, ms, sender2, state2, force))
        changed = true;
    }
    return changed;
  }
  function storeRecords(id) {
    try {
      ...
      logger35.debug("response store unavailable", e);
      return [];
    }
  }
  function borrowedMs(id, cid) {
    ...
    try {
      ...
      logger35.debug("node neighbor lookup failed", e);
    }
  ...
  function conversationCreateTime(id) {
    try {
      ...
      logger35.debug("conversation time lookup failed", e);
    }
    return null;
  }
  function fullRecord(id, rec) {
    ...
    try {
      ...
      logger35.debug("byId lookup failed", e);
    }
    return rec;
  }
    ...
    pageWindow.fetch = function voidMessageTimestampsFetch(input, init) {
      ...
      return promise.then((res) => {
        try {
          ...
          logger35.debug("fetch ingest failed", e);
        }
        return res;
      });
    };
  ...
  function hookXhr2() {
    ...
    XHR.prototype.open = function voidMessageTimestampsOpen(method, url, ...rest) {
      try {
        ...
        logger35.debug("xhr open failed", e);
      }
      ...
      if (RESPONSE_URL.test(url)) {
        this.addEventListener("load", () => {
          try {
            ...
            logger35.debug("xhr ingest failed", e);
          }
        }, { once: true });
      }
  ...
  function hookListResponses() {
    ...
    try {
      ...
      origList = null;
      logger35.debug("chatListResponses wrap skipped", e);
    }
  }
  function unhookListResponses() {
    ...
    try {
      ...
      logger35.debug("chatListResponses unwrap skipped", e);
    }
    origList = null;
  }
  ...
  var messageTimestamps_default = definePlugin({
    ...
    start() {
      try {
        ...
        logger35.warn("Failed to hook network", e);
      }
    },
    stop() {
      unhookFetch2();
      ...
      ResponseStore: {
        ...
        handler() {
          try {
            ...
            logger35.debug("store ingest failed", e);
          }
        }
      },
      MessageStore: {
    ...
    _renderTimestamp: ErrorBoundary.wrap(({ response, isUser }) => {
      ...
      if (settings28.store.hideOwnMessages && human)
        return null;
      ...
      }, formatTimestamp(ms, settings28.store.showDate));
    }),
    patches: [
      {
...
`);
  ...
  var logger36 = new Logger("BetterNavigator");
  var cl30 = classNameFactory("void-bn-");
  ...
  var THINK_SEL2 = "[data-testid*='think'], [class*='thinking'], [class*='Thought'], [aria-label*='Thought']";
  var STOP_SEL = [
    'button[aria-label="Stop model response"]',
  ...
  var HIDE_CLASS2 = "void-bn-hidetip";
  var LIVE_LABEL = "正在输出…";
  ...
  var HYDRATE_MS = 2400;
  var HYDRATE_STEP = 80;
  var LIVE_NODE2 = new Set(["streaming", "optimistic", "reconnecting", "send-sent", "ack-pending", "send-queued", "skeleton"]);
  ...
  function isDeadResponse3(r) {
    if (!r)
      return false;
    if (isUserInterrupt5(r))
      return true;
    const state2 = (r.state ?? "").trim().toLowerCase();
    return DEAD2.has(state2) || r.error != null && !LIVE2.has(state2);
  }
  function nodeTerminal(node) {
    ...
    if (node.status === "complete" || node.status === "stream-error" || node.status === "send-error")
      return true;
    return isDeadResponse3(node.content);
  }
  ...
  function storeLive() {
    try {
      ...
      if (genNode && nodeTerminal(genNode)) {} else if (LIVE_PHASE2.has(phase)) {
        return true;
      ...
      if (streamedId) {
        ...
        if (genNode && nodeTerminal(genNode) && sameGen)
          return false;
        if (streamed && !isDeadResponse3(streamed)) {
          const state2 = (streamed.state ?? "").trim().toLowerCase();
          if (LIVE2.has(state2) || streamed.partial)
            return true;
        }
      }
      return false;
  ...
  function liveAssistantEl() {
    ...
    if (stopVisible())
      return last;
    const live2 = storeLive();
    if (live2)
      return last;
    if (live2 == null && last.querySelector(THINK_SEL2) && !lookSettled(last))
      return last;
    return null;
  }
  function currentCid2() {
    try {
      ...
      logger36.debug("chat page unavailable:", e);
      return "";
    }
  }
  function gatewayOf2(cid) {
    if (!cid)
      return;
    try {
      return MessageStore.useMessageStore.getState().conversations?.[cid];
  ...
  function extendPath(gw, path) {
    ...
    const gen2 = gw.activeGeneration;
    if (!gen2?.assistantId)
      return out;
    const assistant = nodes[gen2.assistantId];
    if (assistant && nodeTerminal(assistant))
      return out;
    const user = gen2.userId ? nodes[gen2.userId] : undefined;
  ...
  function liveAssistantId(gw, path) {
    ...
    const phase = generationPhase(gw);
    if (genNode && genNode.role === "assistant" && !nodeTerminal(genNode) && (LIVE_PHASE2.has(phase) || nodeLive(genNode))) {
  ...
  function responseIdOf(el) {
    if (el.id.startsWith("response-"))
      return el.id.slice("response-".length);
    const host3 = el.closest("[id^='response-']");
    if (host3?.id.startsWith("response-"))
      return host3.id.slice("response-".length);
    return;
  }
  ...
  function plain(text) {
    ...
    if (!t || NOISE_TEXT.test(t))
      return "";
    return t.length > SUMMARY_MAX ? `${t.slice(0, SUMMARY_MAX)}…` : t;
  }
  function labelFromResponse(role, rec) {
    if (!rec || rec.isControl)
      return "";
    const raw = role === "user" ? rec.query || rec.message || "" : rec.message || "";
    const text = plain(raw);
    if (text)
      return text;
    if (rec.generatedImageUrls?.length || rec.imageAttachments?.length || rec.imageEditUri || rec.imageEditUris?.length)
      return "图片";
    if (rec.fileAttachments?.length || rec.fileUris?.length || rec.fileAttachmentsMetadata?.length)
      return "附件";
    return "";
  }
  function contentOf(cid, node) {
    if (node.content)
      return node.content;
    try {
      return MessageStore.nodeToResponse?.(cid, node);
    } catch (e) {
      logger36.debug("nodeToResponse failed:", e);
      return;
    }
  }
  function nodeLive(node) {
    if (LIVE_NODE2.has(node.status))
      return true;
    if (node.content?.partial && !isDeadResponse3(node.content))
      return true;
    const state2 = (node.content?.state ?? "").trim().toLowerCase();
    return LIVE2.has(state2) && !isDeadResponse3(node.content);
  }
  function collectDom() {
    const root = chatPane2() ?? document;
    const showAsst = settings29.store.showAssistant;
    ...
    for (const el of root.querySelectorAll(MSG_SEL)) {
      ...
      if (!showAsst && role === "assistant")
        continue;
      const live2 = el === liveEl;
      const text = summarize(el) || (live2 ? LIVE_LABEL : "");
      if (!text)
        continue;
      out.push({ id: responseIdOf(el), el, role, text, live: live2 });
    }
    return out;
  }
  function itemFromNode(cid, node, liveId, liveEl) {
    ...
    if (!settings29.store.showAssistant && role === "assistant")
      return null;
    const rec = contentOf(cid, node);
    if (rec?.isControl)
      return null;
    const el = elForId(node.id);
    const live2 = role === "assistant" && (node.id === liveId || !!el && el === liveEl);
    ...
    if (!text)
      text = live2 ? LIVE_LABEL : LOADING_LABEL;
    if (text !== LOADING_LABEL && text !== LIVE_LABEL)
      labelCache.set(key, text);
    return { id: node.id, el, role, text, live: live2 };
  }
  ...
  function clearFlash() {
    ...
    flashTimer2 = 0;
    flashing2?.classList.remove("void-bn-flash");
    flashing2 = null;
  }
  function flash(el) {
    clearFlash();
    if (settings29.store.jumpEffect !== "border")
      return;
    flashing2 = el;
    el.classList.add("void-bn-flash");
    flashTimer2 = window.setTimeout(clearFlash, reduceMotion() ? FLASH_REDUCED_MS : FLASH_MS2);
  }
  ...
  function assistantPool(tickCount) {
    const all = responseIdxs();
    const mounted = mountedAssistantIndexes();
    if (!tickCount || tickCount === all.length)
      return all;
    if (mounted.length && tickCount === mounted.length)
      return mounted;
    if (mounted.length && Math.abs(tickCount - mounted.length) < Math.abs(tickCount - all.length))
      return mounted;
    return all;
  }
  ...
  function navIndexFromTick(tick2, tickIndex) {
    ...
    if (n != null && pool[n - 1] != null)
      return pool[n - 1];
    if (pool[tickIndex] != null)
  ...
  async function hydrateJump(item, index) {
    const gen2 = ++hydrateGen;
    lockIdx = index;
    lockUntil = performance.now() + HYDRATE_MS + LOCK_MS;
    applyActive(index);
    const tick2 = item.role === "assistant" ? nativeTickFor(item, index) : undefined;
    if (tick2)
      tick2.click();
  ...
  function applyActive(index, source = "list") {
    ...
    const tick2 = host2?.querySelectorAll(".void-bn-tick")[index];
  ...
  function tickMarkedCurrent(btn) {
    ...
    if (btn.getAttribute("aria-pressed") === "true")
      return true;
    const state2 = (btn.getAttribute("data-state") || "").toLowerCase();
    return state2 === "active" || state2 === "current" || state2 === "on";
  }
  ...
  function nativeCurrentIndex() {
    if (!settings29.store.showAssistant)
      return null;
  ...
  function edgePick(nav) {
    ...
    if (!edge)
      return null;
    const ticks = nativeTicks();
    if (ticks.length && settings29.store.showAssistant) {
  ...
  function syncNativeEdge(nav, index) {
    ...
    for (const tick2 of ticks) {
      if (tick2 === mapped) {
        if (!item.live)
          tick2.classList.add("void-bn-native-edge");
        continue;
      }
      if (!tick2.classList.contains("void-bn-native-live"))
        tick2.classList.add("void-bn-native-dim");
    }
  }
  ...
  function clampMenu() {
    const menu2 = host2?.querySelector(".void-bn-menu");
    if (!menu2 || !host2)
      return;
    const origin = rail ?? host2;
    const cap2 = Math.max(120, composerTop() - 16);
    menu2.style.maxHeight = `${Math.min(cap2, window.innerHeight * 0.7)}px`;
    menu2.style.top = "";
    const originRect = origin.getBoundingClientRect();
    const mh = menu2.offsetHeight;
    const viewTop = 8;
    ...
    menu2.style.marginTop = Math.abs(delta) < 1 ? "" : `${delta}px`;
  }
  function alignMenu(index) {
    const menu2 = host2?.querySelector(".void-bn-menu");
    if (!menu2 || !host2)
      return;
    const row = menu2.querySelector(`.void-bn-item[data-void-bn-i="${index}"]`);
  ...
  function menuEl(nav) {
    ...
    menu2.className = cl30("menu");
    menu2.addEventListener("pointerenter", () => {
      overMenu = true;
      markAim(-1);
    });
    menu2.addEventListener("pointerleave", () => {
      overMenu = false;
      markAim(-1);
    });
    const meta = document.createElement("div");
    meta.className = cl30("meta");
    meta.textContent = metaLabel(0);
    const ul = document.createElement("ul");
    ul.className = cl30("list");
    nav.forEach((item, i) => {
      const li = document.createElement("li");
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = cl30("item");
      btn.dataset.voidBnI = String(i);
      const emoji = document.createElement("span");
      emoji.className = cl30("emoji");
      emoji.textContent = item.role === "user" ? "❓" : "\uD83E\uDD16";
      const label = document.createElement("span");
      label.className = cl30("label");
    ...
    menu2.append(meta, ul);
    return menu2;
  }
  function tickRail(nav) {
    const wrap = document.createElement("div");
    wrap.className = cl30("ticks", { dense: nav.length > DENSE_N });
    nav.forEach((item, i) => {
      const tick2 = document.createElement("button");
      tick2.type = "button";
      tick2.className = cl30("tick", item.role === "user" ? "tick-user" : "tick-asst", { "tick-live": item.live });
  ...
  function syncHideTip() {
    document.documentElement.classList.toggle(HIDE_CLASS2, !!settings29.store.hideNativeHover);
  }
  function setOpen(on) {
  ...
  function onPointerLeaveRail(e) {
    const next = e.relatedTarget;
    if (next instanceof Element && (next.closest(".void-bn-host") || next.closest(".void-bn-rail") || next.closest(".void-bn-menu")))
      return;
    markAim(-1);
  }
  ...
  function messageKey2(s) {
    try {
      ...
      const path = extendPath(gw, pathToLeaf(gw));
      const gen2 = gw.activeGeneration;
      const genNode = gen2?.assistantId ? gw.nodes?.[gen2.assistantId] : undefined;
      const phase = generationPhase(gw);
      const lastAsst = [...path].reverse().find((n) => n.role === "assistant");
      const lastKey2 = lastAsst ? `${lastAsst.id}:${lastAsst.status}:${lastAsst.content?.state ?? ""}` : "";
      const genKey = gen2 ? `${gen2.userId}:${gen2.assistantId}:${genNode?.status ?? ""}:${phase}` : "";
      return `${cid}|${gw.defaultLeafId ?? ""}|${genKey}|${lastKey2}|${path.map((n) => `${n.id}:${n.status}`).join(",")}`;
    } catch (e) {
      logger36.debug("message key failed:", e);
      return "";
    }
  }
  function onStreamEnd8(_data) {
    paint3();
  }
  function start() {
    if (ac)
  ...
  function stop() {
    ...
    lastNav = [];
    lastPath = "";
    document.documentElement.classList.remove(HIDE_CLASS2);
  }
  ...
  function applyImagineUpgrade() {
    if (settings30.store.hideImagineUpgrade)
  ...
  function shouldReplay(officialCount, saved2, now) {
    return officialCount === 0 && freshSnaps(saved2, now).length > 0;
  }
  function intentOf(intent2) {
    if (!intent2?.modeId)
      return;
    return {
      modeId: intent2.modeId,
      modelMode: intent2.modelMode || "",
      activeModelId: intent2.activeModelId || ""
    };
  }
  function bindPending(saved2, official, pending2, now) {
    const known = new Set(saved2.map((item) => item.id));
    const left = pending2.filter((item) => now - item.savedAt < PENDING_MS);
    const extra = [];
    ...
    return { saved: saved2.concat(extra), pending: left };
  }
  function projectQueue(saved2, official, pending2, hydrated, now) {
    const fresh = freshSnaps(saved2, now);
    const bound = bindPending(fresh, official, pending2, now);
    if (!official.length) {
  ...
  var logger37 = new Logger("QueuePersist");
  ...
  function liveIntent2() {
    const host3 = pageWindow;
    const forced = host3[ENQUEUE_FORCE2] ?? host3[Symbol.for("voidpp.modeSync.intent")];
    if (forced?.modeId) {
      return {
  ...
  function withIntent(intent2, fn) {
    if (!intent2?.modeId)
      return fn();
    let modes = null;
    let chat = null;
    ...
    try {
      ...
      if (prevMode !== intent2.modeId)
        modes.setSelectedModeId(intent2.modeId, { source: "sync" });
      if (intent2.modelMode && prevModel !== intent2.modelMode)
        chat.setModelMode(intent2.modelMode);
      if (intent2.activeModelId && prevActive !== intent2.activeModelId)
        chat.setActiveModelId(intent2.activeModelId);
    } catch (e) {
      logger37.debug("intent apply failed", e);
    }
    try {
      if (intent2?.modeId)
        pageWindow[ENQUEUE_FORCE2] = intent2;
      return fn();
    } finally {
      ...
      try {
        ...
        logger37.debug("intent restore failed", e);
      }
    }
  }
  function qid2(item) {
  ...
  function remember3(cid, next) {
    const prev = memory2.get(cid) ?? [];
    if (sameQueue(prev, next))
      return;
    if (next.length)
      memory2.set(cid, next);
    else
      memory2.delete(cid);
    persistSoon();
  }
  function syncOne(cid, hydrated) {
    ...
    const bound = bindPending(freshSnaps(memory2.get(cid) ?? [], now), official, pending2.get(cid) ?? [], now);
  ...
  function bucketsToMemory(account) {
    ...
    const local = account === LOCAL_ACCOUNT2 ? {} : doc.buckets[LOCAL_ACCOUNT2] ?? {};
    const cids = new Set([...Object.keys(primary), ...Object.keys(local), ...memory2.keys()]);
    const now = Date.now();
    for (const cid of cids) {
      ...
      const live2 = memory2.get(cid) ?? [];
      if (!live2.length && disk.length)
        memory2.set(cid, disk);
    }
  }
  function recordBuckets() {
    const now = Date.now();
    const out = {};
    for (const [cid, items] of memory2) {
  ...
  async function persist3() {
    ...
    if (account !== LOCAL_ACCOUNT2 && doc.buckets[LOCAL_ACCOUNT2]) {
      for (const cid of Object.keys(doc.buckets[account] ?? {}))
        delete doc.buckets[LOCAL_ACCOUNT2][cid];
      if (!Object.keys(doc.buckets[LOCAL_ACCOUNT2]).length)
        delete doc.buckets[LOCAL_ACCOUNT2];
    }
    try {
      await idbSet(DB_KEY, doc);
    } catch (e) {
      logger37.debug("persist failed", e);
    }
  }
  async function load() {
    try {
  ...
  function loadBusy(cid) {
    try {
      const state2 = ResponseStore.useResponseStore.getState();
      return !!(state2.initialResponsesPromisesByConversationId?.[cid] || state2.nodesPromisesByConversationId?.[cid] || state2.inflightPromisesByConversationId?.[cid]);
    } catch {
      return false;
    }
  }
  ...
  async function replay(cid, saved2) {
    const placed = [];
    const ordered = freshSnaps(saved2, Date.now()).slice().sort((a, b) => a.position - b.position);
    for (const item of ordered) {
      ...
      try {
        ...
        logger37.debug("replay failed", e);
      } finally {
        suppress = false;
      }
  ...
  async function restore2(cid) {
    ...
    const saved2 = freshSnaps(memory2.get(cid) ?? [], Date.now());
    if (!shouldReplay(0, saved2, Date.now())) {
      syncOne(cid, true);
      decided.add(cid);
      return;
    }
    ...
    let retry2 = false;
    try {
      const placed = await replay(cid, saved2);
      const after = readOfficial(cid);
      if (!after.length) {
        memory2.set(cid, saved2);
        persistSoon();
        if (!retried.has(cid)) {
          retried.add(cid);
          waits.set(cid, Date.now());
        ...
        memory2.set(cid, placed);
        persistSoon();
        decided.add(cid);
      }
    } catch (e) {
      logger37.debug("restore failed", e);
      retry2 = true;
    } finally {
      replaying = false;
      restoring.delete(cid);
    }
    if (retry2 || !decided.has(cid))
      scheduleCurrent();
  ...
  function wrapQueue() {
    let state2;
    try {
      state2 = MessageStore.useMessageStore.getState();
    } catch {
      return;
    }
    const current = state2.queueMessage;
  ...
  function unwrapQueue() {
    ...
    try {
      const state2 = MessageStore.useMessageStore.getState();
      if (state2.queueMessage === wrappedQueue)
  ...
  function wrapReconnect() {
    let state2;
    try {
      state2 = ChatPageStore.useChatPageStore.getState();
    } catch {
      return;
    }
    const current = state2.reconnectToInflightResponses;
  ...
  function unwrapReconnect() {
    ...
    try {
      const state2 = ChatPageStore.useChatPageStore.getState();
      if (state2.reconnectToInflightResponses === wrappedReconnect) {
...
`);
  ...
  var cl31 = classNameFactory("void-rt-");
  var HOME_KEY = "home";
  var HOME_SEP = "home:";
  ...
  function maxCount() {
    const n = Number(settings31.store.maxRecent);
    return Number.isFinite(n) && n > 0 ? n : 5;
  }
  function capVisits(ids) {
    const allowHome = settings31.store.includeHome;
    const current = currentVisit();
    const dirtyGlobalWs = asWorkspaceId2(settings31.plain.workspaceByConv?.[HOME_KEY]);
    const seen = new Set;
    const out = [];
  ...
  function sameRecord(a, b) {
    const src = a ?? {};
    const keys4 = Object.keys(b);
    if (Object.keys(src).length !== keys4.length)
      return false;
    return keys4.every((k) => src[k] === b[k]);
  }
  function assignRecord(key, next) {
    if (sameRecord(settings31.plain[key], next))
      return false;
    settings31.store[key] = next;
    return true;
  }
  ...
  function initEffect() {
    ...
    if (disk) {
      ...
      const fromSettings = {
        visits: asStringList(settings31.plain.visits),
        deniedIds: asStringList(settings31.plain.deniedIds),
        deniedAt: asStringRecord(settings31.plain.deniedAt)
      };
      effect = {
        v: 1,
  ...
  function commitVisits(next) {
    ...
    const rawWs = pruneRecord(settings31.plain.workspaceByConv, visits);
    ...
    const pages = pruneRecord(settings31.plain.pages, visits);
    ...
    for (const [id, name] of Object.entries(settings31.plain.projectNames ?? {})) {
    ...
    for (const [id, snap] of Object.entries(settings31.plain.projectIcons ?? {})) {
    ...
    const titles = {};
    for (const [id, name] of Object.entries(pruneRecord(settings31.plain.titles, visits))) {
  ...
  function rememberTitle(id, title) {
    ...
    if (id === chatIdFromUrl() && isAccessDeniedPage())
      return;
    const prev = settings31.plain.titles ?? {};
    if (prev[id] === t)
      return;
    settings31.store.titles = { ...prev, [id]: t };
  }
  ...
  function hrefFor2(id, workspaceId) {
    if (isHomeId(id)) {
      const ws2 = workspaceFromHomeId(id) || asWorkspaceId2(workspaceId);
  ...
  function titleOf2(id) {
    if (!id || isHomeId(id))
      return "New chat";
    if (isDenied(id))
      return usableTitle(settings31.plain.titles?.[id]) || "Untitled";
    const conv = lookup(id);
    return usableTitle(conv?.title) || usableTitle(settings31.plain.titles?.[id]) || titleFromPage(id) || "Untitled";
  }
...
`) : "";
  ...
  function pickProjectSvg(el) {
    for (const svg2 of el.querySelectorAll("svg")) {
      const host4 = svg2.closest("a[href]");
      if (host4 && host4 !== el) {
        const { chat } = hrefParts(host4.getAttribute("href"));
        if (chat)
          continue;
      }
      if (encodeIcon(svg2))
  ...
  function rememberProjectIcon(ws) {
    ...
    const prev = settings31.plain.projectIcons ?? {};
    if (snap) {
      wsIcons[ws] = snap;
      if (prev[ws] !== snap)
        settings31.store.projectIcons = { ...prev, [ws]: snap };
      return;
    }
    if (prev[ws] && isChromeSnap(prev[ws])) {
      ...
      delete next[ws];
      delete wsIcons[ws];
      settings31.store.projectIcons = next;
    }
  }
  ...
  function dropWorkspace(id) {
    const prev = settings31.plain.workspaceByConv ?? {};
    if (!prev[id])
      return;
    const next = { ...prev };
    delete next[id];
    settings31.store.workspaceByConv = next;
  }
  function workspaceOf2(id) {
    ...
    if (fromSidebar)
      return fromSidebar;
    const cached = asWorkspaceId2(settings31.plain.workspaceByConv?.[id]);
    if (cached)
      return cached;
  ...
  function readOpenProjectName() {
    const idx = sidebarIndex();
    const live2 = liveWorkspaceId();
    if (live2) {
      const n = usableName(idx.nameByWs[live2]);
      if (n)
        return n;
    }
    const current = currentVisit();
  ...
  function projectNameOf(id) {
    ...
    if (!ws)
      return "";
    const idx = sidebarIndex();
    const named = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || wsNames[ws] || settings31.plain.projectNames?.[ws] || "");
    if (!named)
      return "";
    const live2 = liveWorkspaceId();
    const liveName = readOpenProjectName();
    if (live2 && ws !== live2 && liveName && named === liveName)
      return "";
    return named;
  }
  function rememberProject(id) {
    ...
    if (!ws)
      return;
    const prevWs = settings31.plain.workspaceByConv ?? {};
    if (prevWs[id] !== ws)
      settings31.store.workspaceByConv = { ...prevWs, [id]: ws };
    const idx = sidebarIndex();
    const sidebarName = usableName(idx.nameByConv[id] || idx.nameByWs[ws] || "");
    const liveName = ws === liveWorkspaceId() ? readOpenProjectName() : "";
    const cached = usableName(wsNames[ws] || settings31.plain.projectNames?.[ws] || "");
    ...
    if (!name)
      return;
    wsNames[ws] = name;
    const prevNames = settings31.plain.projectNames ?? {};
    if (prevNames[ws] !== name)
      settings31.store.projectNames = { ...prevNames, [ws]: name };
  }
  function reconcileSidebarCache() {
    const idx = sidebarIndex();
    const prevWs = { ...settings31.plain.workspaceByConv };
    const prevNames = { ...settings31.plain.projectNames };
    const prevIcons = { ...settings31.plain.projectIcons };
    let wsChanged = false;
    ...
    if (wsChanged)
      settings31.store.workspaceByConv = prevWs;
    if (namesChanged)
      settings31.store.projectNames = prevNames;
    if (iconsChanged)
      settings31.store.projectIcons = prevIcons;
  }
  function requestWorkspace(id) {
    ...
    try {
      ...
      fetchConv(id).then((conv) => {
        ...
        const prev = settings31.plain.workspaceByConv ?? {};
        if (prev[id] !== ws)
          settings31.store.workspaceByConv = { ...prev, [id]: ws };
        const live2 = liveWorkspaceId();
        const liveName = usableName(readOpenProjectName());
        const names = settings31.plain.projectNames ?? {};
        if (live2 && ws !== live2 && liveName && names[ws] === liveName) {
          const next = { ...names };
          delete next[ws];
          settings31.store.projectNames = next;
          delete wsNames[ws];
        }
        maybePaint();
  ...
  function snapOf(id) {
    if (!id || isHomeId(id))
      return null;
    const snap = thumbs.get(id) ?? parseSnap(settings31.plain.pages?.[id]);
    if (!snap)
      return null;
  ...
  function rememberPage(id, snap) {
    const json = JSON.stringify(snap);
    const prev = settings31.plain.pages ?? {};
    if (prev[id] === json)
      return;
    settings31.store.pages = { ...prev, [id]: json };
  }
  ...
  function buildPageShot(snap) {
    const page = node("span", cl31("page"));
    page.dataset.theme = snap.theme;
    for (const line of lastRound(snap.lines)) {
      const el = node("span", cl31("page-line", line.role === "user" && "page-line-user"), line.text);
  ...
  function captureId(id) {
    ...
    if (!lines.length)
      return;
    const prev = thumbs.get(id) ?? parseSnap(settings31.plain.pages?.[id]);
  ...
  function bump(id) {
    if (!id)
      return;
    if (isHomeId(id) && !settings31.store.includeHome)
      return;
  ...
  function fillShot(box, id) {
    ...
    if (!snap) {
      const fallback = node("span", cl31("fallback"));
      fallback.append(faviconImg(cl31("favicon")));
      box.append(fallback);
      return;
    }
    const shot = node("span", cl31("shot"));
  ...
  function detectTheme() {
    const html = document.documentElement;
    const { body } = document;
    const tokens2 = `${html.className} ${body?.className ?? ""} ${html.getAttribute("data-theme") ?? ""} ${html.getAttribute("data-color-scheme") ?? ""}`.toLowerCase();
    if (/(^|[\s_-])(dark|night)([\s_-]|$)/.test(tokens2) || html.classList.contains("dark") || html.getAttribute("dark") != null)
      return "dark";
    if (/(^|[\s_-])(light|day)([\s_-]|$)/.test(tokens2) || html.classList.contains("light"))
      return "light";
    try {
  ...
  function folderIcon() {
    const svg2 = document.createElementNS("http://www.w3.org/2000/svg", "svg");
    svg2.setAttribute("class", cl31("folder"));
    svg2.setAttribute("viewBox", "0 0 24 24");
    svg2.setAttribute("fill", "none");
    svg2.setAttribute("stroke", "currentColor");
    svg2.setAttribute("stroke-width", "2");
...
`)) {
  ...
  function projectIconOf(ws) {
    const raw = ws ? wsIcons[ws] || settings31.plain.projectIcons?.[ws] || liveIconSnap(ws) || "" : "";
  ...
  function renderList(items) {
    if (!host3)
      return;
    const panel = host3.querySelector(`.${cl31("panel")}`);
    if (!panel)
      return;
    let list = panel.querySelector(`.${cl31("list")}`);
    if (!list) {
      panel.replaceChildren();
      list = node("div", cl31("list"));
      panel.append(list);
    }
    list.replaceChildren();
    items.forEach((topic, i) => {
      const btn = node("button", cl31("card"));
      btn.type = "button";
      btn.tabIndex = -1;
      ...
      const shot = node("span", cl31("thumb"));
      shot.setAttribute("aria-hidden", "true");
      fillShot(shot, topic.id);
      const meta = node("span", cl31("meta"));
      meta.append(node("span", cl31("name"), topic.title));
      if (topic.project) {
        const proj = node("span", cl31("host"));
        proj.append(projectIconOf(topic.ws), node("span", cl31("host-name"), topic.project));
        meta.append(proj);
      }
  ...
  function patchList(items) {
    if (!host3)
      return;
    const cards = [...host3.querySelectorAll(`.${cl31("card")}`)];
    ...
    items.forEach((topic, i) => {
      ...
      const name = card.querySelector(`.${cl31("name")}`);
      if (name)
        name.textContent = topic.title;
      const meta = card.querySelector(`.${cl31("meta")}`);
      if (!meta)
        return;
      let row = meta.querySelector(`.${cl31("host")}`);
      if (!topic.project) {
        row?.remove();
        return;
      }
      if (!row) {
        row = node("span", cl31("host"));
        row.append(projectIconOf(topic.ws), node("span", cl31("host-name"), topic.project));
        meta.append(row);
        return;
      }
      const label = row.querySelector(`.${cl31("host-name")}`);
      if (label)
  ...
  function paint4() {
    ...
    if (!items.length) {
      if (paintedIds !== "__empty__") {
        panel.replaceChildren(node("div", cl31("empty"), "Open a few chats, then hold Ctrl+` to switch."));
    ...
    if (paintedIds === "__empty__" || !panel.querySelector(`.${cl31("list")}`)) {
      panel.replaceChildren(node("div", cl31("list")));
      paintedIds = "";
      paintedMeta = "";
    }
    if (paintedIds !== ids) {
  ...
  function apply7() {
    const pct = clamp(settings32.store.opacity, 0, 100);
    const blur = clamp(settings32.store.blur, 0, 40);
    const alpha = pct / 100;
  ...
  betterCanvas_default.updatedAt = 1790133349000;
...
`);
  ...
  function showNotice(options) {
    closeNotice();
    const { toast: toast2 } = Toaster;
    if (!toast2)
      return -1;
    activeNoticeId = toast2.custom((id) => /* @__PURE__ */ React.createElement(Notice, {
      ...options,
      onClose: () => {
        toast2.dismiss(id);
        activeNoticeId = null;
